//! `wrokbot-server` —— Server transport：Axum HTTP/SSE/WS、static GUI、health/readiness。
//!
...
//!   `wrokbot-application::ApplicationService`。
...
//!   `wrokbot-server::http::channels::list`，[`http::channels::list`] 逐字兑现它。
//! - `GET /health` —— 台账 `health-get`（parity），落点 `wrokbot-server::http::health`，
...
//!   `wrokbot-server::http::metrics`。不另开监听端口，见 [`http::metrics`] 模块文档。
...
//! - 独立 `wrokbot-migrate` binary 承载 R68 Intelligence import/FK finalize；Server main 零调用点。
//!
//! # 还没有的东西（不要当成"已经有了"）
//!
...
pub use wrokbot_infra::auth::single_user::SINGLE_USER_ACTOR_ID;
