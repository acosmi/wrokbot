//! WrokBot PostgreSQL/cutover migration binary；不进入最终 runtime request path。
...
use wrokbot_infra::intelligence_import::PostgresIntelligenceImportStore;
use wrokbot_server::config::{env_map_from_process, preflight_audit_retention};
...
) -> Result<wrokbot_application::IntelligenceImportReport, MigrationFailure> {
...
-> Result<wrokbot_application::IntelligenceToolRunFkReport, MigrationFailure> {
    let pool = connect_migration_database().await?;
    let store = PostgresIntelligenceImportStore::new(pool.clone());
...
async fn connect_migration_database() -> Result<deadpool_postgres::Pool, MigrationFailure> {
    ...
    if wrokbot_server::database::initialize(&pool).await.is_err() {
        pool.close();
