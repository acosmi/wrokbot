...
use wrokbot_application::{
    ...
    MutateMemoryRequest, WrokBotApplication, PeopleAdministration, PeoplePageRequest,
...
use wrokbot_contracts::audit::{AuditEventView, AuditPage};
...
use wrokbot_contracts::error::IdentityConflictReason;
use wrokbot_contracts::ids::{
...
use wrokbot_contracts::ui::{UiPreferences, UpdateUiPreferences};
use wrokbot_domain::audit::hash::Sha256Digest;
use wrokbot_domain::identity::roles::AdminFloor;
use wrokbot_domain::identity::session::{
    SessionHashKey, SessionState, SessionToken, SessionTokenHash, TrustedOrigins, evaluate_session,
};
use wrokbot_domain::tool::approval::{ApprovalTarget, PolicyVersionTag};
use wrokbot_domain::tool::metadata::{ApprovalClass, Effect, ToolName};
use wrokbot_domain::vault::{KeyVersion, WrappingKey};
use wrokbot_infra::auth::config::default_session_lifetime;
use wrokbot_infra::auth::sso::DynamicSsoService;
use wrokbot_infra::db::{baseline, native, pool};
use wrokbot_infra::net::safe_http::{EgressPolicy, SafeDialer};
use wrokbot_infra::policy::PolicyStore;
use wrokbot_infra::repo::people_admin::PostgresPeopleAdministration;
use wrokbot_infra::tool_approval::{DurableHumanDecision, PostgresToolApprovalCoordinator};
use wrokbot_server::auth::SESSION_COOKIE_NAME;
use wrokbot_server::{
...
const FIXTURE_APPROVAL_DATABASE_URL: &str = "WROK_BOT_UI_APPROVAL_DATABASE_URL";
const FIXTURE_AUTH_JOURNEY_ENV: &str = "WROK_BOT_UI_AUTH_FIXTURE";
const FIXTURE_APPROVAL_DATABASE_PREFIX: &str = "wrokbot_ui_approval_fixture_";
...
const FIXTURE_SSO_PUBLIC_URL: &str = "https://fixture.wrokbot.test";
const FIXTURE_SSO_ROUTE_COOKIE: &str = "wrokbot_sso_route=fixture-auth-route";
...
impl ThreadDirectory for FixtureThreads {
    ...
    async fn begin_thread_run(
        ...
        if let wrokbot_contracts::command::ThreadRunAnchor::Channel { channel_id } =
    ...
    async fn subscribe_channel_activity(
        &self,
        _request: wrokbot_application::ChannelActivitySubscription,
    ) -> Result<AppEventStream, ThreadDirectoryError> {
...
impl AuthResolver for FixtureAuthResolver {
    ...
    ) -> Result<AuthContext, wrokbot_contracts::error::AppError> {
        if self.revoked.load(Ordering::SeqCst) {
            Err(wrokbot_contracts::error::AppError::Unauthenticated)
        } else {
    ...
    async fn resolve_with_assurance(
        &self,
        _parts: &axum::http::request::Parts,
    ) -> Result<ResolvedAuth, wrokbot_contracts::error::AppError> {
        if self.revoked.load(Ordering::SeqCst) {
            Err(wrokbot_contracts::error::AppError::Unauthenticated)
        } else {
    ...
    async fn revoke_session(
        &self,
        resolved: &ResolvedAuth,
    ) -> Result<(), wrokbot_contracts::error::AppError> {
        if !resolved.has_revocable_session() {
            return Err(wrokbot_contracts::error::AppError::RequestConflict {
                resource: "session",
            });
        }
...
async fn assemble_approval_fixture(
    ...
    let config = database_url.parse::<wrokbot_infra::db::pool::DatabaseConfig>()?;
...
fn validate_fixture_database(
    config: &wrokbot_infra::db::pool::DatabaseConfig,
) -> Result<(), std::io::Error> {
...
async fn auth_journey_middleware(
    ...
    if method == Method::POST && path == "/api/auth/sso/start" {
        ...
        response.headers_mut().insert(
            ...
            HeaderValue::from_static(
                "wrokbot_sso_route=fixture-auth-route; Path=/; HttpOnly; SameSite=Lax",
            ),
        );
        response
            .headers_mut()
    ...
    if method == Method::GET && path == "/api/auth/sso/continue" {
        ...
        response.headers_mut().insert(
            SET_COOKIE,
            HeaderValue::from_static(
                "wrokbot_sso_route=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0",
            ),
        );
        response
            .headers_mut()
...
mod approval_pg_tests {
    ...
    use wrokbot_domain::routing::RoutingReasonCode;
    ...
    async fn direct_bot_fixture_distinguishes_minted_from_persisted_threads() {
        ...
        threads
            .begin_thread_run(BeginThreadRunRequest {
                ...
                tenant,
                actor,
                command: wrokbot_contracts::command::BeginThreadRun {
    ...
    fn postgres_request_seed_and_proof_states_are_one_closed_fixture_contract() {
        ...
        let valid = wrokbot_infra::db::pool::DatabaseConfig::new(
            "127.0.0.1",
            5432,
            "postgres",
            "wrokbot_ui_approval_fixture_test",
        );
        assert!(validate_fixture_database(&valid).is_ok());
        assert!(validate_fixture_database(&valid.clone().with_dbname("postgres")).is_err());
        let remote = wrokbot_infra::db::pool::DatabaseConfig::new(
            "db.example.test",
            5432,
            "postgres",
            "wrokbot_ui_approval_fixture_test",
