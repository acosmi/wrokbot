...
//!
//! `wrokbot_contracts::auth::AuthContext` **刻意既不 `Serialize` 也不 `Deserialize`**
...
//!
//! [`PostgresSessionAuthResolver`] 是 W-4 production 实现：只认 `wrokbot_session` cookie，
...
//!
//! ```text
//! wrokbot-server（本 crate）      定义 AuthResolver port
//!         ↑ 实现
...
pub const SESSION_COOKIE_NAME: &str = "wrokbot_session";
...
impl SingleUserAuthResolver {
    ...
    pub fn from_verified_principal(
        principal: wrokbot_infra::auth::single_user::VerifiedSingleUserPrincipal,
        lifetime: SessionLifetimePolicy,
    ) -> Self {
        Self {
