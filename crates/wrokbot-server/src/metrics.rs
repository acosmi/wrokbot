...
//! `wrokbot_contracts::telemetry::METRICS_LABEL_ALLOWLIST` 是**关联字段**那一侧的白名单
...
//! `wrokbot_application::service::TRACE_ONLY_SPAN_FIELDS` 的手法把这件事做成可判定的：
//!
...
use wrokbot_contracts::telemetry::Transport;
...
pub const HTTP_REQUEST_DURATION_SECONDS: &str = "wrokbot_http_request_duration_seconds";

/// 在飞请求数（不是连接数，见模块文档）。
pub const HTTP_REQUESTS_IN_FLIGHT: &str = "wrokbot_http_requests_in_flight";
...
mod tests {
    use super::*;
    use wrokbot_contracts::telemetry::{
    ...
    fn correlation_field_names() -> BTreeSet<String> {
        let populated = CorrelationFields {
            deployment_id: Some(wrokbot_contracts::ids::DeploymentId::new("d")),
            tenant_id: Some(wrokbot_contracts::ids::TenantId::new("t")),
            request_id: Some("r".to_owned()),
            actor_id: Some(wrokbot_contracts::ids::ActorId::new("a")),
            bot_id: Some(wrokbot_contracts::ids::BotId::new("b")),
            channel_id: Some(wrokbot_contracts::ids::ChannelId::new("c")),
            thread_id: Some(wrokbot_contracts::ids::ThreadId::new("th")),
            run_id: Some(wrokbot_contracts::ids::RunId::new("run")),
            tool_call_id: Some(wrokbot_contracts::ids::ToolCallId::new("tc")),
            computer_id: Some(wrokbot_contracts::ids::ComputerId::new("cm")),
            generation: Some(wrokbot_contracts::ids::ComputerGeneration::new(1)),
            policy_decision_id: Some(wrokbot_contracts::ids::PolicyDecisionId::new("pd")),
