...
use wrokbot_computer::screen::{
    DEFAULT_SCREEN_VIEWERS_PER_STREAM, ScreenHub, ScreenSessionService,
};
use wrokbot_contracts::ids::{ActorId, DeploymentId, TenantId};
use wrokbot_domain::identity::groups::IdentityProviderId;
use wrokbot_domain::identity::session::TrustedOrigins;
use wrokbot_domain::remote_callback::RemoteRunAssertionSigner;
...
use wrokbot_infra::agent_tools::{PostgresAgentAuthorizationSource, PostgresAgentToolSequence};
use wrokbot_infra::application_assembly::{
    ChannelRoutingProviderInput, PostgresApplicationAssemblyInput, assemble_postgres_application,
};
use wrokbot_infra::auth::config::{
...
use wrokbot_infra::auth::oidc::coordinator::{DEFAULT_IDP_TIMEOUT, DEFAULT_METADATA_MAX_BYTES};
use wrokbot_infra::auth::oidc::{
...
use wrokbot_infra::auth::single_user::{initialize_single_user, load_single_user_principal};
use wrokbot_infra::auth::sso::DynamicSsoService;
use wrokbot_infra::component_catalogue::PostgresComponentAdministration;
use wrokbot_infra::db::pool::DatabaseConfig;
use wrokbot_infra::db::{native, pool};
...
use wrokbot_infra::policy::{PolicyListener, PolicyStore};
...
use wrokbot_infra::provider::custom::PostgresCustomModelProvider;
...
use wrokbot_infra::run_runtime::RunRelay;
use wrokbot_infra::sandboxed_components::PostgresSandboxedComponentAdministration;
use wrokbot_infra::tenant::{PostgresTenantPackageSynchronizer, load_tenant_package};
use wrokbot_infra::ui_preferences::PostgresUiPreferenceAdministration;
use wrokbot_infra::vault::CredentialRecordVault;
use wrokbot_server::config::{
...
use wrokbot_server::readiness::ReadinessVerdict;
use wrokbot_server::telemetry::{self, LogFormat};
use wrokbot_server::{
...
type AuthAssembly = (
    ...
    Option<wrokbot_domain::identity::roles::AdminFloor>,
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let local = match arguments.as_slice() {
        ...
        _ => return Err(startup_error("wrokbot_arguments_invalid").into()),
    };
    telemetry::init(LogFormat::Json)?;
    let env = wrokbot_server::config::launch::prepare_environment(env_map_from_process(), local)?;
    ...
    let database_url = wrokbot_server::config::env::optional(&env, "DATABASE_URL")
    ...
    wrokbot_server::database::initialize(&pool).await?;
    ...
    let policy_listener = PolicyListener::start(
        database
            .clone()
            .with_application_name("wrokbot-action-policy-listener"),
    ...
    let raw_master = wrokbot_server::config::env::optional(&env, "KEY_ENCRYPTION_KEY")
    ...
    if local && !single_user {
        return Err(wrokbot_server::config::launch::LaunchError::LocalModeConflict.into());
    }
    ...
    let run_relay = RunRelay::start_with_database(
        ...
        database
            .clone()
            .with_application_name("wrokbot-run-control"),
    );
...
fn configured_origins(
    env: &wrokbot_server::config::EnvMap,
) -> Result<TrustedOrigins, Box<dyn Error>> {
    let configured = wrokbot_server::config::env::comma_separated(env, "TRUSTED_ORIGINS");
    let entries = if configured.is_empty() {
        vec![wrokbot_infra::auth::config::DEFAULT_TRUSTED_ORIGIN.to_owned()]
    } else {
        configured
    };
        ...
        for (provider, model, base, use_responses_api) in cases {
            let adapter = build_managed_provider(
                ManagedProviderConfig {
                    ...
                    api_key: wrokbot_server::config::Secret::new("test-key"),
                    base_url: wrokbot_server::config::DeploymentAddress::parse(base).unwrap(),
                    use_responses_api,
