...
//! # 边界：这里放什么，不放什么
//!
//! 放的是 `parity/env.yaml` 里 `target: "wrokbot-server::config::ServerConfig"` 的那些条目。
//! 刻意**不**放的三类：
//!
//! - `DATABASE_URL` —— 台账把它指给 `wrokbot-infra::db::DatabaseConfig`。同一个变量两个
//!   解析器就是两个答案，而先坏掉的那个不会有人发现。
//! - 认证面（session secret、`KEY_ENCRYPTION_KEY`、三家 OAuth、管理员名单、可信来源）
//!   —— 在 `wrokbot_infra::auth::config`。
//! - `wrokbot-computer` / `wrokbot-agent` 归属的那一大批（容器镜像、超时、provider key、
...
//! `wrokbot-infra`。两个 crate 是兄弟（谁也不依赖谁），所以**不能**共用一个类型。
//!
...
use wrokbot_domain::audit::retention::{RetentionPolicy, parse_retention_days};
use wrokbot_domain::policy::ActionPolicy;
...
#[cfg(test)]
mod tests {
    use wrokbot_domain::policy::PolicyMode;
    ...
    fn public_url_must_be_answerable() {
        ...
        for bad in ["not a URL", "wrokbot.example.com", "ftp://x.test"] {
