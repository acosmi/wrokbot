...
//! | 档 | 机制 | 落点 |
//! | --- | --- | --- |
//! | preserve | 照常读那个名字 | [`ServerConfig`]、`wrokbot_infra::auth::config::AuthConfig`、`wrokbot_infra::db` |
...
//! # 谁不在这里
//!
//! - `DATABASE_URL` → `wrokbot-infra::db`（台账 `database-url` 的 `target` 就是那里）。
//! - 认证面全部 → `wrokbot_infra::auth::config`：session secret、`KEY_ENCRYPTION_KEY`、
//!   三家 OAuth、`INITIAL_ADMIN_EMAILS`、`TRUSTED_ORIGINS`、`OPENBOT_SINGLE_USER`。
//! - `wrokbot-computer` / `wrokbot-agent` 归属的那一大批 → G4/G5。本模块只收 Server
