...
pub const LOCAL_PACKAGE_DIRECTORY: &str = "examples/wrokbot";
...
impl fmt::Display for LaunchError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str(match self {
            Self::VariableCollision => "wrokbot_env_collision",
            Self::UnknownVariable => "wrokbot_env_unknown",
            Self::LocalModeConflict => "wrokbot_local_mode_conflict",
            Self::LocalPortInvalid => "wrokbot_local_port_invalid",
        })
    }
}
...
mod tests {
    ...
    fn local_package_loads_without_environment_credentials_or_remote_slots() {
        ...
        let loaded =
            wrokbot_infra::tenant::load_tenant_package(&root, &TenantPackageEnvironment::default())
                .unwrap();
        assert_eq!(loaded.package.tenant_id, "wrokbot-local");
