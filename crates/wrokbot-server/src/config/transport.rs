...
//! 而那一档的暴露面由**绑定地址**管住（`wrokbot_infra::auth::config::single_user_binding_verdict`），
...
mod tests {
    ...
    fn the_three_shapes_land_in_three_different_places() {
        assert_eq!(
            PublicTransport::classify(Some(&address("https://wrokbot.example.com"))),
        ...
        assert_eq!(
            PublicTransport::classify(Some(&address("http://wrokbot.example.com"))),
