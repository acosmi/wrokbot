...
//! # 为什么不引第三方 URL 解析器
//!
//! `wrokbot-server` 的 `Cargo.toml` 里没有 `url`，而依赖清单不在本轮的可动文件里。
//! 兄弟 crate `wrokbot-infra` 有 `url`（OIDC 那条链路需要真正的 WHATWG 解析器），
...
mod tests {
    ...
    fn parses_scheme_and_host() {
        let https = DeploymentAddress::parse("https://wrokbot.example.com/api/").expect("合法");
        assert_eq!(https.scheme(), Scheme::Https);
        assert_eq!(https.host(), "wrokbot.example.com");
        // 尾斜杠在解析时就剥掉了，之后没有第二个地方会再剥一次。
        assert_eq!(https.as_str(), "https://wrokbot.example.com/api");
    ...
    fn case_is_normalised_on_both_halves() {
        let address = DeploymentAddress::parse("HTTPS://WrokBot.Example.COM").expect("合法");
        assert_eq!(address.scheme(), Scheme::Https);
        assert_eq!(address.host(), "wrokbot.example.com");
        // 原串**不**归一：它要和厂商处登记的 redirect URI 逐字符比对，
        // 我们无权替操作员改写它。
        assert_eq!(address.as_str(), "HTTPS://WrokBot.Example.COM");
    }
    ...
    fn a_query_string_terminates_the_authority() {
        let address = DeploymentAddress::parse("https://wrokbot.test?a=1").expect("合法");
        assert_eq!(address.host(), "wrokbot.test");
    }
    ...
    fn refuses_what_it_cannot_answer_questions_about() {
        for bad in [
            "not a URL",
            "wrokbot.example.com",
            "ftp://wrokbot.example.com",
            "https://",
            "://wrokbot.test",
            "",
            "\u{0085}https://wrokbot.test\u{0085}",
        ] {
            assert_eq!(
        ...
        assert!(DeploymentAddress::parse("https://wrokbot.example.com").is_ok());
    }
