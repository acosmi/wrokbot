...
(()=>{
  ...
  const receive=(event)=>{
    if(event.data!=="wrokbot_sandbox_init:"+payload.capability)return;
    ...
    try{
      ...
      document.getElementById("wrokbot-sandbox-root").innerHTML=payload.html;
...
pub async fn document() -> Result<Response<Body>, HttpError> {
    ...
    let body = format!(
        "<!doctype html><html><head><meta charset=\"utf-8\"><meta name=\"referrer\" content=\"no-referrer\"></head><body><div id=\"wrokbot-sandbox-root\"></div><script nonce=\"{nonce}\">{SANDBOX_RUNNER_JS}</script></body></html>"
    );
    Response::builder()
