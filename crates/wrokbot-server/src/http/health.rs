...
//! 拓扑（`wrokbot_contracts::command::HealthReport` 为同一理由拒绝在探活应答里列依赖）。
...
//! 响应体**，不折叠成 ready 也不折叠成 `not_ready`。`wrokbot-infra` 已经用类型把
...
/// `/health` 的响应体。
///
/// 刻意**不**复用 `wrokbot_contracts::command::HealthReport`：那是 `AppCommand::Health`
