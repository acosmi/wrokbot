...
mod tests {
    ...
    use wrokbot_application::{
        AppEventStream, ApplicationService, ChannelReader, WrokBotApplication, PortError,
        ToolApprovalAdministration, ToolApprovalAdministrationError,
    };
    use wrokbot_contracts::auth::{AuthContext, AuthGeneration, Role};
    use wrokbot_contracts::command::AppEvent;
    use wrokbot_contracts::ids::{ActorId, BotId, DeploymentId, RunId, TenantId, ToolCallId};
    ...
    impl ChannelReader for EmptyChannels {
        ...
        ) -> Result<Vec<wrokbot_contracts::command::ChannelSummary>, PortError> {
            Ok(Vec::new())
        }
    }
    ...
    fn router(approvals: FakeApprovals) -> Router {
        ...
        let application: Arc<dyn ApplicationService> = Arc::new(
            WrokBotApplication::new(EmptyChannels).with_tool_approvals(Arc::new(approvals)),
        );
        crate::router(
