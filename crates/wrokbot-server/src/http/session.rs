...
use wrokbot_contracts::ui::SessionStatus;
...
mod tests {
    ...
    use wrokbot_application::{ApplicationService, ChannelReader, WrokBotApplication, PortError};
    ...
    fn app(revocable: bool, secure: bool) -> (axum::Router, Arc<AtomicUsize>) {
        ...
        let application: Arc<dyn ApplicationService> =
            Arc::new(WrokBotApplication::new(EmptyChannels));
    ...
    async fn status_origin_revoke_and_cookie_clear_are_exact() {
        ...
        assert_eq!(
            headers[SET_COOKIE],
            "wrokbot_session=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0; Secure"
        );
    }
