...
//!
//! 台账原文把落点钉成 `wrokbot-server::http::channels::list (GET /api/channels)`，
...
//!   契约，之后换排序键会成为破坏性变更（`wrokbot_contracts::command` 的字段文档逐字写着
//!   这条）。
...
/// `wrokbot_contracts::command::AppCommand` 上那条 `deny_unknown_fields` 逐字相同：
...
pub struct ListChannelsQuery {
    ...
    /// `wrokbot_contracts::command::MAX_CHANNEL_PAGE` 由 **application** 截断。
    ///
...
async fn drive_channel_websocket(
    ...
    mut stream: wrokbot_application::AppEventStream,
) {
    loop {
        tokio::select! {
...
mod tests {
    ...
    use wrokbot_application::{AppEventStream, ChannelCursor, WrokBotApplication};
    use wrokbot_contracts::auth::{AuthContext, AuthGeneration, Role};
    use wrokbot_contracts::command::{ChannelActivityEvent, ChannelDetail, ChannelSummary};
    use wrokbot_contracts::ids::{ActorId, BotId, ChannelId, DeploymentId, TenantId, ThreadId};
    ...
    {
        let application = Arc::new(WrokBotApplication::new(channels).with_threads(directory));
    ...
    fn app_with_administration(
        ...
        let application = Arc::new(
            WrokBotApplication::new(EmptyChannels)
    ...
    fn app_without_administration(resolver: FixedAuthResolver) -> Router {
        let application = Arc::new(WrokBotApplication::new(EmptyChannels));
        ServerBuilder::new(application, Arc::new(resolver))
            .with_sensitive_write_security(SensitiveWriteSecurity::new(
                default_session_lifetime(),
                TrustedOrigins::from_configured(["https://app.example.test"]).unwrap(),
    ...
    async fn create_uses_auth_scope_canonical_ids_and_returns_only_the_channel_dto() {
        ...
        assert_eq!(
            ...
            [ChannelCreateRequest {
                scope: wrokbot_application::ChannelCreateScope {
