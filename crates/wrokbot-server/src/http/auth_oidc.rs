...
//!
//! handler 不做协议判定；全部顺序在 `wrokbot-infra::auth::oidc::OidcLoginCoordinator`。
...
use wrokbot_infra::auth::oidc::{OidcLoginError, ProviderId};
...
pub(crate) fn issued_session_response(
    state: &ServerState,
    issued: &wrokbot_infra::auth::oidc::IssuedSession,
    now: OffsetDateTime,
) -> Response {
...
pub(crate) fn dynamic_login_error(error: wrokbot_infra::auth::sso::DynamicSsoError) -> Response {
    if error.rate_limited() {
        auth_failure(StatusCode::TOO_MANY_REQUESTS, "authentication_rate_limited")
    } else if error.dependency_unavailable() {
        auth_failure(
...
mod tests {
    use super::*;
    use wrokbot_infra::auth::oidc::OidcError;
    ...
    fn cookie_is_host_only_http_only_lax_and_secure_iff_configured() {
        ...
        assert_eq!(
            secure,
            "wrokbot_session=safe_token-123; Path=/; HttpOnly; SameSite=Lax; Max-Age=3600; Secure"
        );
