...
use wrokbot_contracts::ui::{UiLocale, UiPreferences, UiTheme};
use tower_http::services::ServeDir;

const INDEX_MAX_BYTES: u64 = 1024 * 1024;
const HTML_ROOT_MARKER: &str = "<html lang=\"en\">";
const UI_COOKIE_NAME: &str = "wrokbot-ui";
...
fn validate_index(index: &str) -> io::Result<()> {
    ...
    while let Some(start) = rest.find("<script") {
        ...
        if opening.matches(" src=").count() != 1
            || !(opening.contains(" src=\"/wrokbot-bootstrap.mjs\"")
                || opening.contains(" src=\"./wrokbot-bootstrap.mjs\""))
        {
            return Err(invalid_data(
                "static index must load the one same-origin WrokBot bootstrap",
            ));
        }
...
mod tests {
    ...
    const INDEX: &str = "<!doctype html><html lang=\"en\"><head>\
        <script type=\"module\" src=\"/wrokbot-bootstrap.mjs\"></script>\
        </head><body></body></html>";
    ...
    fn index_requires_external_same_origin_empty_scripts() {
        ...
        assert!(
            validate_index(
                "<html lang=\"en\"><script src=\"//example.test/wrokbot-bootstrap.mjs\"></script>"
            )
            .is_err()
        );
        assert!(
            validate_index(
                "<html lang=\"en\"><script src=\"/wrokbot-bootstrap.mjs\" src=\"//example.test/x.js\"></script>"
            )
            .is_err()
        );
        assert!(
            validate_index(
                "<html lang=\"en\"><script src=\"/wrokbot-bootstrap.mjs\"></script><script src=\"/wrokbot-bootstrap.mjs\"></script>"
            )
            .is_err()
        );
    }
    ...
    fn closed_cookie_wins_and_invalid_cookie_falls_back_to_accept_language() {
        ...
        headers.insert(
            COOKIE,
            HeaderValue::from_static("session=x; wrokbot-ui=v1.dark.zh-CN"),
        );
        headers.insert(
        ...
        headers.insert(COOKIE, HeaderValue::from_static("wrokbot-ui=v2.dark.en"));
        ...
        assert_eq!(
            mirrored,
            "wrokbot-ui=v1.light.en; Path=/; Max-Age=31536000; SameSite=Lax; HttpOnly; Secure"
        );
    }
    ...
    async fn mounted_serve_dir_rewrites_routes_but_never_hides_missing_api_or_assets() {
        ...
        let root = router
            ...
            .oneshot(
                Request::builder()
                    .uri("/")
                    .header(COOKIE, "wrokbot-ui=v1.dark.zh-CN")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
