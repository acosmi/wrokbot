...
#[cfg(test)]
use wrokbot_contracts::people::{CurrentUser, Person};
use wrokbot_domain::text::is_ecmascript_whitespace;
...
pub async fn credentials_list(
    ...
    if !auth.has_role(wrokbot_contracts::auth::Role::Admin) {
...
mod tests {
    ...
    use wrokbot_application::{
        ...
        WrokBotApplication, PeopleAdministration, PeoplePageRequest, PeoplePortError, PortError,
    };
    ...
    use wrokbot_contracts::ids::{AuditEventId, DeploymentId, TenantId};
    ...
    fn application(people: FakePeople) -> Arc<dyn wrokbot_application::ApplicationService> {
    ...
    fn application_with_audit(
        people: FakePeople,
        audit: FakeAudit,
    ) -> Arc<dyn wrokbot_application::ApplicationService> {
        Arc::new(
    ...
    async fn caller_cannot_ask_for_the_whole_deployment_in_one_page() {
        ...
        assert_eq!(
            ...
            [PeoplePageRequest {
                search: None,
                cursor: None,
                limit: wrokbot_application::MAX_PEOPLE_PAGE,
            }],
