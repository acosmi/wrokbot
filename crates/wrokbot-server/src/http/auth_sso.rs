...
use wrokbot_contracts::auth::{EnterpriseSsoRoutingAccepted, EnterpriseSsoStartRequest, Role};
...
use wrokbot_infra::auth::oidc::ProviderId;
use wrokbot_infra::auth::sso::{DynamicSsoStart, RegisterIdentityProviderInput, SamlStart};
...
const ROUTE_COOKIE_NAME: &str = "wrokbot_sso_route";
...
fn require_admin(auth: &wrokbot_contracts::auth::AuthContext) -> Result<(), HttpError> {
...
fn clear_route_cookie() -> HeaderValue {
    HeaderValue::from_static(
        "wrokbot_sso_route=; Path=/api/auth/sso/; HttpOnly; SameSite=Strict; Max-Age=0",
    )
}
...
mod tests {
    ...
    fn html_form_values_and_duplicate_route_cookies_are_closed() {
        ...
        let duplicate = HeaderMap::from_iter([(
            COOKIE,
            HeaderValue::from_static("wrokbot_sso_route=one; wrokbot_sso_route=two"),
        )]);
        assert_eq!(unique_cookie(&duplicate, ROUTE_COOKIE_NAME), None);
        let one = HeaderMap::from_iter([(
            COOKIE,
            HeaderValue::from_static("x=1; wrokbot_sso_route=one"),
        )]);
        assert_eq!(
        ...
        assert_eq!(
            ...
            "wrokbot_sso_route=opaque-route-ticket; Path=/api/auth/sso/; HttpOnly; SameSite=Strict; Max-Age=120; Secure"
        );
