...
use wrokbot_application::AppEventStream;
...
use wrokbot_contracts::ids::{RunId, ThreadId};
...
pub async fn cancel_run(
        ...
        .await?
    {
        AppReply::ThreadRunCancellation(reply) => {
            ...
            metrics::counter!(
                "wrokbot_agent_run_cancel_requests_total",
                "outcome" => outcome
            )
            .increment(1);
...
mod tests {
    ...
    use wrokbot_application::{ChannelCursor, WrokBotApplication};
    ...
    use wrokbot_contracts::ids::{ActorId, DeploymentId, RunId, TenantId};
    ...
    fn app(directory: FakeThreadDirectory, resolver: FixedAuthResolver) -> Router {
        let application = Arc::new(WrokBotApplication::new(EmptyChannels).with_threads(directory));
    ...
    async fn native_conversation_is_one_no_store_snapshot_scoped_only_by_auth_and_path() {
        ...
        let directory =
            FakeThreadDirectory::new(Ok(true)).with_conversation(Ok(ThreadConversationSnapshot {
                ...
                active_run_state: Some(
                    wrokbot_contracts::command::ThreadForegroundRunState::Running,
                ),
        ...
        assert_eq!(
            ...
            [ThreadConversationRequest {
                deployment: DeploymentId::new("wrokbot-test"),
        ...
        for (replayed, expected) in [(false, StatusCode::CREATED), (true, StatusCode::OK)] {
            ...
            assert_eq!(
                ...
                [BeginThreadRunRequest {
                    auth_generation: wrokbot_contracts::auth::AuthGeneration::new(1),
                    deployment: DeploymentId::new("wrokbot-test"),
                    tenant: TenantId::new("tenant-test"),
                    actor: ActorId::new("u1"),
                    command: BeginThreadRun {
                        model_selection: Some(
                            wrokbot_contracts::model_connections::RunModelSelection {
                        ...
                        thread_id: ThreadId::new(thread),
                        run_id: RunId::new("run-1"),
                        bot_id: wrokbot_contracts::ids::BotId::new("agent-1"),
                        anchor: wrokbot_contracts::command::ThreadRunAnchor::Channel {
    ...
    async fn returns_one_this_deployment_can_recognise_later() {
        ...
        assert!(ThreadIdentity::new(&DeploymentId::new("wrokbot-test")).owns(&thread));
    }
