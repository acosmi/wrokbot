...
mod tests {
    ...
    use wrokbot_application::{
        ChannelReader, WrokBotApplication, PortError, RunCostBudgetAdministration,
        RunCostBudgetAdministrationError, RunCostCap,
    };
    use wrokbot_contracts::auth::{AuthContext, AuthGeneration, Role};
    use wrokbot_contracts::budget::RunCostCapInput;
    ...
    fn router(budgets: FakeBudgets) -> Router {
        ...
        let application = Arc::new(
            WrokBotApplication::new(EmptyChannels).with_run_cost_budgets(Arc::new(budgets)),
        );
