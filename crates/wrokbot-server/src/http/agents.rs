...
mod tests {
    ...
    use wrokbot_application::{
        ...
        ChannelCursor, ChannelReader, WrokBotApplication, PortError,
    };
    use wrokbot_contracts::agent::{
    ...
    fn app(tokens: FakeTokens, session_age: Duration) -> axum::Router {
        ...
        router(
            ServerBuilder::new(
                Arc::new(WrokBotApplication::new(EmptyChannels).with_agent_callback_tokens(tokens)),
                Arc::new(resolver),
            )
    ...
    fn read_app(agents: FakeAgentDirectory, resolver: FixedAuthResolver) -> axum::Router {
        router(
            ServerBuilder::new(
                Arc::new(
                    WrokBotApplication::new(EmptyChannels).with_agent_directory(Arc::new(agents)),
                ),
                Arc::new(resolver),
            )
            .build(),
