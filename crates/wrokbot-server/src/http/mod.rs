...
//!
//! ```text
//! health-get              target: "wrokbot-server::http::health (GET /health)"
//! readiness-get           target: "wrokbot-server::http::readiness (GET /readiness)"
//! metrics-get             target: "wrokbot-server::http::metrics (GET /metrics)"
//! api-channels-list-get   target: "wrokbot-server::http::channels::list (GET /api/channels)"
//! api-threads-mint-post   target: "wrokbot-server::http::threads::mint (POST /api/threads/mint)"
//! api-threads-get         target: "wrokbot-server::http::threads::status (GET /api/threads/{thread_id})"
//! thread-events-sse-get   target: "wrokbot-server::http::threads::events (GET /api/threads/{thread_id}/events)"
//! thread-events-ws-get    target: "wrokbot-server::http::threads::websocket (GET /api/threads/{thread_id}/ws)"
//! screen-session/ws       target: "wrokbot-server::http::screen (POST /api/screen/sessions + GET /api/screen)"
...
use wrokbot_agent::RemoteAgentToolInvoker;
use wrokbot_application::{ApplicationService, McpOAuthCallback, RemoteCallbackAuthenticator};
use wrokbot_computer::screen::ScreenHub;
use wrokbot_domain::identity::session::TrustedOrigins;
use wrokbot_infra::auth::oidc::{OidcLoginCoordinator, PreAuthSurface};
...
impl ServerState {
    ...
    pub async fn authorize_sensitive_write(
        ...
        let Some(security) = &self.inner.sensitive_write else {
            return Err(wrokbot_contracts::error::AppError::DependencyUnavailable {
...
mod tests {
    ...
    use wrokbot_application::ports::{ChannelReader, PortError};
    use wrokbot_application::{AppEventStream, WrokBotApplication};
    ...
    use wrokbot_contracts::ids::{ActorId, BotId, ChannelId, DeploymentId, TenantId};
    use std::sync::Mutex;
    ...
    fn auth() -> AuthContext {
        AuthContext::for_test(
            ...
            wrokbot_contracts::auth::AuthGeneration::new(SENTINEL_AUTH_GENERATION),
            false,
        )
    }
    ...
    /// 这份替身是本 crate 自己的：`wrokbot-application` 的同名 fake 是 `pub(crate)` +
    ...
    impl ChannelReader for FakeChannelReader {
        async fn list_visible_channels(
            ...
            rows.sort_by(|a, b| {
                let ra = wrokbot_application::channel_recency(a);
                let rb = wrokbot_application::channel_recency(b);
                rb.cmp(&ra).then_with(|| b.id.as_str().cmp(a.id.as_str()))
            });
            if let Some(cursor) = cursor {
                rows.retain(|row| {
                    let recency = wrokbot_application::channel_recency(row);
    ...
    struct RecordingApplication {
        inner: WrokBotApplication<FakeChannelReader>,
    ...
    impl RecordingApplication {
        fn new(reader: FakeChannelReader) -> Self {
            Self {
                inner: WrokBotApplication::new(reader),
