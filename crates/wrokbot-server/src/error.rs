...
//! 逐条说明**没有**出去的：
//!
//! - `dependency` / `vendor`：依赖名泄漏部署拓扑（`wrokbot_contracts::command::HealthReport`
...
/// `wrokbot-contracts`，而 contracts 必须 wasm 可编、不许依赖 axum。孤儿规则也不允许
...
mod tests {
    ...
    use wrokbot_contracts::error::{ErrorCode, StaleGenerationSubject};
    use wrokbot_contracts::ids::{ActorId, ComputerGeneration, PolicyDecisionId};
    ...
    fn all_variants() -> Vec<AppError> {
        vec![
            ...
            AppError::IdentityConflict {
                reason: wrokbot_contracts::error::IdentityConflictReason::RoleLastAdmin,
            },
            AppError::SensitiveWriteRefused {
                reason: wrokbot_contracts::error::SensitiveWriteReason::OriginUntrusted,
            },
