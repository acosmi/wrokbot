...
///
/// 与 `wrokbot_application::service::APPLICATION_SPAN_FIELDS` 同样是**台账**：
...
/// - `wrokbot-application` 的 `application.execute` span 是它的子 span，`request_id`
...
mod tests {
    ...
    fn text_and_json_formats_differ_and_json_parses() {
        ...
        tracing::subscriber::with_default(
            ...
            || {
                tracing::info!(marker = "wrokbot-format-probe", "hello");
            },
        );
        let text = text_sink.contents();
        assert!(text.contains("wrokbot-format-probe"), "{text}");
        assert!(
        ...
        assert_eq!(parsed["fields"]["marker"], "wrokbot-format-probe");
        assert_ne!(text, json);
    }
