...
use wrokbot_application::provider::{
...
impl RemoteAguiTransport for UnusedRemote {
    async fn start(
        &self,
        _endpoint: &str,
        _authorization: Option<&wrokbot_application::RemoteAguiAuthorization>,
        _body: Vec<u8>,
...
async fn send(
    ...
    let mut request = Request::builder()
        .method(method)
        .uri(path)
        .header(http::header::COOKIE, format!("wrokbot_session={cookie}"));
