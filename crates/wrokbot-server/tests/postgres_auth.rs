...
use http::Request;
use wrokbot_application::{ApplicationService, WrokBotApplication};
use wrokbot_contracts::auth::Role;
use wrokbot_contracts::error::AppError;
use wrokbot_contracts::ids::{DeploymentId, TenantId};
use wrokbot_domain::identity::session::{SessionHashKey, SessionToken, SessionTokenHash};
        ...
        let outcome = async {
            ...
            let duplicate = resolver
                .resolve(&parts(&format!(
                    "wrokbot_session={RAW_TOKEN}; wrokbot_session={RAW_TOKEN}"
                )))
                .await;
            ...
            client
                ...
                .map_err(|error| error.to_string())?;
            if resolver
                .resolve(&parts("wrokbot_session=legacy-plaintext-token"))
                .await
            ...
            client
                ...
                .map_err(|error| error.to_string())?;
            if resolver
                .resolve(&parts(&format!("wrokbot_session={RAW_TOKEN}")))
                .await
                != Err(AppError::Unauthenticated)
            {
    ...
    with_temp_database(&admin, "server_auth_http", |config| async move {
        ...
        let outcome = async {
            ...
            let application: std::sync::Arc<dyn ApplicationService> = std::sync::Arc::new(
                WrokBotApplication::new(ChannelRepo::new(pool.clone())).with_people(people),
            );
            let auth = resolver(&pool);
            let security = SensitiveWriteSecurity::new(
                default_session_lifetime(),
                wrokbot_domain::identity::session::TrustedOrigins::from_configured([
            ...
            let cookie = format!("wrokbot_session={RAW_TOKEN}");
    ...
    with_temp_database(&admin, "server_auth_signout", |config| async move {
        ...
        let outcome = async {
            ...
            let application: std::sync::Arc<dyn ApplicationService> =
                std::sync::Arc::new(WrokBotApplication::new(ChannelRepo::new(pool.clone())));
            let app = router(
            ...
            let first_cookie = format!("wrokbot_session={RAW_TOKEN}");
            let second_cookie = format!("wrokbot_session={SECOND_RAW_TOKEN}");
            ...
            let clear_cookie = signed_out
                ...
                .unwrap_or("");
            if clear_cookie != "wrokbot_session=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0" {
