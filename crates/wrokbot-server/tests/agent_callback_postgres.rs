...
use wrokbot_agent::{AuthorizedAgentToolGateway, RemoteAgentToolInvoker};
use wrokbot_application::{
    AgentContextSource, ApplicationService, WrokBotApplication, ProviderRoute, RunExecutionLease,
};
...
use wrokbot_domain::remote_callback::{
...
use wrokbot_infra::agent_callback::{
...
use wrokbot_infra::mcp::{McpClientError, SafeRmcpClient};
use wrokbot_infra::mcp_catalog::PostgresMcpCatalog;
use wrokbot_infra::mcp_credentials::{McpCredentialError, PostgresMcpCredentialBroker};
use wrokbot_infra::memory_admin::PostgresMemoryAdministration;
use wrokbot_infra::net::safe_http::{CidrAllowlist, EgressPolicy, SafeDialer, SchemePolicy};
...
use wrokbot_server::{PostgresSessionAuthResolver, SensitiveWriteSecurity, ServerBuilder, router};
...
impl ServerHandler for CallbackMcpServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            .with_server_info(Implementation::new("wrokbot-callback-test", "3.1.4"))
...
async fn send(
    ...
    let mut request = Request::builder().method(method).uri(path).header(
        http::header::COOKIE,
        format!("wrokbot_session={RAW_SESSION}"),
    );
...
async fn production_http_issues_hash_only_token_and_refuses_ungranted_callback() {
    ...
    with_temp_database(&admin, "callbackhttp", |config| async move {
        ...
        let result = async {
            ...
            let unknown = wrokbot_domain::remote_callback::callback_token_from_entropy([0x44; 32]);
            let (status, _) = send(
                app.clone(),
                Method::POST,
                "/api/agent-tools/call",
    ...
    with_temp_database(&admin, "callbackrmcp", |config| async move {
        ...
        let result = async {
            ...
            let lease = RunExecutionLease::new(
                ...
                wrokbot_contracts::ids::ThreadId::new("thread-callback"),
