...
use wrokbot_computer::control::ControlService;
...
use wrokbot_contracts::command::{AppCommand, AppReply, ChannelSummary};
...
use wrokbot_server::{AuthResolver, SensitiveWriteSecurity, ServerBuilder};
...
async fn real_engine_frame_crosses_ticketed_server_binary_websocket() {
    ...
    let root = std::env::temp_dir().join(format!(
        "wrokbot-screen-server-conformance-{}",
        std::process::id()
    ));
