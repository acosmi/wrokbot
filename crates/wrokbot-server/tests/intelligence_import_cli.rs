//! `wrokbot-migrate intelligence-import` 的真实文件/crypto/进程/PG17 竖切。
...
use hkdf::Hkdf;
use wrokbot_application::INTELLIGENCE_SOURCE_COMMIT;
...
async fn cli_rejects_public_secret_file_then_imports_empty_signed_bundle() {
    ...
    with_temp_database(&admin, "intelligence_cli", |config| async move {
        let directory =
            std::env::temp_dir().join(format!("wrokbot-intelligence-cli-{}", uuid::Uuid::now_v7()));
...
fn run_cli(
    ...
    Command::new(env!("CARGO_BIN_EXE_wrokbot-migrate"))
        .env_clear()
