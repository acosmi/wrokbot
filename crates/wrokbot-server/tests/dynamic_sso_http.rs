...
use wrokbot_application::WrokBotApplication;
...
fn request_for(
    ...
    if let Some(token) = token {
        builder = builder.header(http::header::COOKIE, format!("wrokbot_session={token}"));
    }
    if !body.is_empty() {
...
#[tokio::test]
#[ignore = "需要真实 PostgreSQL + xmlsec1：设 WROK_BOT_TEST_DATABASE_URL 后加 --include-ignored"]
async fn admin_routes_require_fresh_origin_and_never_project_saml_material() {
    ...
    with_temp_database(&admin, "dynamic_sso_http", |config| async move {
        ...
        let outcome = async {
            ...
            .map_err(|error| error.to_string())?;
            let app = Arc::new(WrokBotApplication::new(ChannelRepo::new(pool.clone())));
            let router = router(
