...
pub(crate) fn ComputerWorkspace(
    ...
    running: Signal<bool>,
    #[prop(optional, into)] target: MaybeProp<wrokbot_contracts::screen::ScreenSessionTarget>,
) -> impl IntoView {
    let i18n = use_i18n();
    let open = RwSignal::new(false);
    view! {
        <details class="wrokbot-computer-workspace" on:keydown=crate::primitives::dismiss_disclosure on:toggle=move |event| {
            ...
            <section aria-label=move || t_string!(i18n, computer.activity_title).to_owned()>
                ...
                <p class="wrokbot-page-intro">{move || t!(i18n, computer.activity_description)}</p>
                <Show when=move || activity.get().is_empty()><p class="wrokbot-page-empty">{move || t!(i18n, computer.activity_empty)}</p></Show>
                <For each=move || activity.get() key=|item| (item.id.clone(), item.label.clone(), item.output.clone()) children=move |item| view! {
                    <details class="wrokbot-workspace-event"><summary>{item.label}</summary><pre class="wrokbot-transcript-text">{item.output}</pre></details>
                }/>
            </section>
            <a class="wrokbot-plugin-link" href="/approvals">{move || t!(i18n, computer.review_approvals)}</a>
        </details>
    }
}
