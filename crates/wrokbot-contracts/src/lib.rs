//! `wrokbot-contracts` —— 协议层：native/wasm-safe 的 ID、DTO、event、error、schema。
//!
...
//! - 唯一一个既编 native 又编 wasm 的 crate：`wrokbot-ui` 只依赖它（设计系统文档 §13）。
//!
//! 明确**不**负责：
//!
//! - 领域不变量与状态机 —— 在 `wrokbot-domain`。
//! - use case 编排 —— 在 `wrokbot-application`。
//! - 任何 I/O：数据库、HTTP、文件、进程 —— 在 `wrokbot-infra` / `wrokbot-computer`。
...
//! # 依赖面为什么这么窄
//!
//! 本 crate 必须编到 `wasm32-unknown-unknown`（`wrokbot-ui` 是 Leptos CSR/WASM 且只依赖它）。
...
//! `cargo check -p wrokbot-contracts --target wasm32-unknown-unknown`，它必须与
