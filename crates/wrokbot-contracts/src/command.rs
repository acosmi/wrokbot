...
//!
//! §5.2 固定了 `wrokbot-application::ApplicationService` 的两个签名：
//!
//! ```text
...
//! ```
//!
//! trait 本身住在 `wrokbot-application`（它需要 `async_trait` 与 `Stream`，那是 native 侧的
//! 依赖）；本模块只定义穿越边界的**类型**，因为它们必须同时编到 wasm 给 `wrokbot-ui` 用。
//!
//! # 为什么这些 enum 是封闭的
//!
...
pub struct HealthReport {
    ...
    /// §16.4 的 metrics 与 `wrokbot-server` 的 `/readyz`，不是跨边界 DTO 的内容。把依赖
    /// 明细放进公开应答会顺带泄漏部署拓扑。
    pub ok: bool,
}
...
/// 需要 `futures`/`async` 机制，属于 `wrokbot-application`。本 crate 只承载帧的内容。
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(tag = "kind", rename_all = "snake_case", deny_unknown_fields)]
pub enum AppEvent {
