...
pub const DESKTOP_STRUCTURED_OPEN_COMMAND: &str = "wrokbot_structured_events_open";

/// Tauri invoke 中关闭本 window 某条 structured stream 的固定命令名。
pub const DESKTOP_STRUCTURED_CLOSE_COMMAND: &str = "wrokbot_structured_events_close";

/// One local UI operation on the invoking, host-bound native window.
pub const DESKTOP_WINDOW_CHROME_COMMAND: &str = "wrokbot_window_chrome";
...
mod tests {
    ...
    fn window_chrome_wire_contains_one_action_and_fixed_receipt_errors() {
        use serde_json::json;
        assert_eq!(DESKTOP_WINDOW_CHROME_COMMAND, "wrokbot_window_chrome");
