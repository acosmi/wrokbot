//! Built-in Agent dispatch consumer/host；pure decisions remain in `wrokbot-domain::agent`。
...
use wrokbot_contracts::components::is_component_human_decision_name;
use wrokbot_domain::agent::{
    AgentEffect, AgentEvent, AgentFailure, AgentState, AgentTerminal, AgentToolCall, reduce,
};
use wrokbot_domain::tool::metadata::ResourceLockKey;
...
async fn execute_run(inner: &Inner, activation: &mut Activation) {
    ...
    let mut bound_custom_model = None::<Option<wrokbot_application::RunModelBinding>>;
    if inner
        .audit
    ...
    match event {
        ...
        ProviderEvent::Failed(failure) => {
            if failure == ProviderFailure::StreamStalled && {
                metrics::counter!("wrokbot_agent_stream_stalled_total").increment(1);
                sampling
                    .audit
...
async fn cancel_and_commit(
    ...
    if matches!(source, CancellationSource::Deadline) && {
        metrics::counter!("wrokbot_agent_run_deadline_total").increment(1);
        audit
...
fn bind_custom_model(
    bound: &mut Option<Option<wrokbot_application::RunModelBinding>>,
...
mod tests {
    ...
    use wrokbot_contracts::ids::{ActorId, BotId, RunId, ThreadId, ToolCallId};
    ...
    impl AgentContextSource for FakeContext {
        async fn load(
            ...
            Ok(ProviderRequest {
                route: wrokbot_application::ProviderRoute::PackageOpenAi,
    ...
    impl RemoteInterruptCoordinator for ImmediateRemoteCoordinator {
        async fn list_pending(
            &self,
            _auth: &wrokbot_contracts::auth::AuthContext,
    ...
    impl AgentToolInvoker for CooperativeCancellationToolInvoker {
        ...
        async fn invoke_cancellable(
            ...
            tool_name: &str,
            arguments: serde_json::Value,
            mut cancellation: wrokbot_application::ToolExecutionCancellation,
        ) -> Result<crate::AgentToolReply, AgentToolInvokeError> {
            assert_eq!(tool_name, "remember");
            assert_eq!(arguments, serde_json::json!({"content":"tea"}));
            self.started.notify_one();
    ...
    async fn custom_sampling_binding_rejects_promotion_fallback_and_identity_or_revision_drift() {
        ...
        let custom = |revision, generation, model: &str| {
            wrokbot_application::RunModelBinding::from_verified_snapshot(
                &run,
                ...
                wrokbot_application::model_connections::normalize_model_configuration(
                    "name",
