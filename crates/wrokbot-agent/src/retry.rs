...
impl RetryingProvider {
    async fn wait_before_retry(&self, retry_index: u8, retry_after: Option<Duration>) {
        ...
        metrics::counter!("wrokbot_agent_provider_retry_total").increment(1);
        metrics::histogram!("wrokbot_agent_provider_retry_delay_seconds")
...
mod tests {
    ...
    use wrokbot_application::{ProviderMessage, ProviderMessageRole, ProviderRoute};
