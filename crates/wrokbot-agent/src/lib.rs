//! `wrokbot-agent` —— Agent 运行时：built-in loop、AG-UI、tool runtime、MCP、connectors。
//!
//! # 所有权边界（v3 §5.1 / §7 / §8 / §9）
//!
//! 负责：
...
//!   durable run（v3 §7.2）。**reducer 本身是 `wrokbot-domain` 的纯函数**，本 crate 只负责
...
//! 明确**不**负责：
//!
//! - 决定"这次工具调用允不允许" —— 判定规则在 `wrokbot-domain`，执行管线的顺序保证在
//!   `wrokbot-application`（v3 §8.1）。本 crate 不得成为第二个策略脑。
//! - 数据库写入与凭据解密 —— 在 `wrokbot-infra`。
//! - 浏览器 / 文件 / shell 的实际执行与隔离 —— 在 `wrokbot-computer`。
