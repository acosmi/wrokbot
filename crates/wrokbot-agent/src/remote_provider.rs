...
impl ProviderAdapter for RemoteAguiProvider {
    async fn start(
        ...
        forwarded.insert(
            "wrokbotBotId".to_owned(),
            serde_json::Value::String(route.bot_id().to_owned()),
        );
        forwarded.insert(
            "wrokbotDeploymentTools".to_owned(),
...
mod tests {
    ...
    fn request(tools: Vec<wrokbot_application::ProviderToolDefinition>) -> ProviderRequest {
        ProviderRequest {
    ...
    fn request_with_assertion(
        tools: Vec<wrokbot_application::ProviderToolDefinition>,
    ) -> ProviderRequest {
    ...
    async fn remote_text_and_reasoning_map_only_after_exact_lifecycle_validation() {
        ...
        assert_eq!(body["forwardedProps"]["wrokbotBotId"], "bot-1");
    ...
    async fn remote_tool_result_is_projection_only_and_requires_an_offered_completed_call() {
        let tool = wrokbot_application::ProviderToolDefinition {
    ...
    async fn remote_cannot_call_an_unoffered_deployment_tool_without_an_assertion() {
        ...
        assert!(matches!(
            provider
                .start(request(vec![wrokbot_application::ProviderToolDefinition {
