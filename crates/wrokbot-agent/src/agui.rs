...
pub fn encode_run_agent_input_with_resume(
    ...
    for (index, message) in messages.iter().enumerate() {
        ...
        let id = format!("wrokbot:{run_id}:message:{index}");
...
mod tests {
    ...
    fn resumed_run_input_has_new_run_lineage_and_exact_closed_entries() {
        ...
        let encoded = encode_run_agent_input_with_resume(
            ...
            &messages,
            &[],
            json!({"wrokbotRun":"signed-local-run"}),
            Some("run-1"),
            Some(&resume),
        )
        .unwrap();
        ...
        assert_eq!(body["forwardedProps"]["wrokbotRun"], "signed-local-run");
        assert_eq!(
            encode_run_agent_input_with_resume(
                "thread-1",
                "run-2",
