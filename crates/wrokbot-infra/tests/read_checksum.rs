...
use wrokbot_infra::db::{baseline, pool};
...
async fn a_single_mutated_value_makes_the_checksum_disagree() {
    ...
    with_temp_database(&admin, "readsumneg", |config| async move {
        ...
        let outcome = async {
            ...
            let mut decoded = wrokbot_infra::db::tables::users::Row::try_from(&raw)
                .map_err(|e| format!("解 users 失败：{e}"))?;

            // 前提自检：未改动前必须相等，否则"改动后不等"说明不了任何事。
            let sql = reconstruct_checksum_sql(
                wrokbot_infra::db::tables::users::TABLE_NAME,
                wrokbot_infra::db::tables::users::COLUMNS.len(),
            );
            ...
            let differing = pinpoint_columns(
                &client,
                wrokbot_infra::db::tables::users::TABLE_NAME,
                wrokbot_infra::db::tables::users::COLUMNS,
            ...
            let source: String = raw
                .try_get("__row_checksum")
                .map_err(|e| format!("取源侧摘要失败：{e}"))?;
            let mut decoded = wrokbot_infra::db::tables::computer_snapshot::Row::try_from(&raw)
                .map_err(|e| format!("解 computer_snapshot 失败：{e}"))?;
            let sql = reconstruct_checksum_sql(
                wrokbot_infra::db::tables::computer_snapshot::TABLE_NAME,
                wrokbot_infra::db::tables::computer_snapshot::COLUMNS.len(),
            );
            let before: String = client
                .query_one(sql.as_str(), &decoded.as_sql_params())
                .await
            ...
            let differing = pinpoint_columns(
                &client,
                wrokbot_infra::db::tables::computer_snapshot::TABLE_NAME,
                wrokbot_infra::db::tables::computer_snapshot::COLUMNS,
                &decoded.as_sql_params(),
                &source,
            )
            .await;
