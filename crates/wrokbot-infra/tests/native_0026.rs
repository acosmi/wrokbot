...
use wrokbot_infra::run_cost_budget::PostgresRunCostBudgetAdministration;
use wrokbot_infra::run_runtime::{DEFAULT_DISPATCH_CLAIM_DURATION, PostgresRunRuntime};
    ...
    with_temp_database(&admin, "native0026facts", |config| async move {
        ...
        let result = async {
            ...
            let after = schema_facts::fetch(&client)
                .await
                .map_err(|error| error.to_string())?;
            if std::env::var_os("WROK_BOT_REGENERATE_SCHEMA_0026").is_some() {
                let mut encoded =
