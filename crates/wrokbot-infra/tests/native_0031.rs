...
use wrokbot_infra::db::tables::{NATIVE_0031_TABLES, run_model_selections};
use wrokbot_infra::db::{baseline, fresh, native, pool, schema_facts};
...
async fn post_0031_preserves_all_old_facts_and_fresh_matches_upgrade() {
    ...
    harness::with_temp_database(&admin,"model31schema",|config|async move {
        ...
        if std::env::var_os("WROK_BOT_REGENERATE_SCHEMA_0031").is_some() {
