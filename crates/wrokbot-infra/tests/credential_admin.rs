...
use wrokbot_application::credential_admin::{
...
async fn model_create_rotate_revoke_are_atomic_and_the_running_provider_reads_the_current_key() {
    ...
    harness::with_temp_database(&config, "credmodel", |config| async move {
        ...
        let outcome = async {
            ...
            assert!(matches!(rejected.next_event().await.unwrap(), Some(wrokbot_application::ProviderEvent::Failed(wrokbot_application::ProviderFailure::Authentication))));
...
#[tokio::test]
#[ignore = "requires isolated PostgreSQL; set WROK_BOT_TEST_DATABASE_URL"]
