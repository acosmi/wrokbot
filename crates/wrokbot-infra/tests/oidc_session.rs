...
use hmac::{Hmac, Mac};
use wrokbot_contracts::ids::TenantId;
...
use wrokbot_infra::auth::oidc::claims::{
    DirectoryIdTokenClaims, build_verifier, verify_with_group_mapping,
};
use wrokbot_infra::auth::oidc::provider::parse_issuer;
...
fn provider(id: &str, issuer: &str) -> OidcProviderConfig {
    OidcProviderConfig::new(
        ...
        CanonicalRedirectUri::parse(
            ...
            wrokbot_infra::auth::oidc::redirect::HTTPS_ONLY,
        )
        .unwrap(),
        BTreeSet::new(),
        None,
...
fn identity(
    ...
    subject: &str,
    groups_json: &str,
) -> wrokbot_infra::auth::oidc::VerifiedIdentity {
    let claims = format!(
    ...
    with_temp_database(&admin, "oidc_issue", |config| async move {
        ...
        let outcome = async {
            ...
            if first.email().as_str() != "admin@example.com"
                || first.path() != wrokbot_domain::identity::revocation::SignInPath::NewAccount
            {
...
async fn real_tls_discovery_pkce_token_jwks_claims_and_session_is_one_replay_safe_flow() {
    ...
    with_temp_database(&admin, "oidc_real_flow", |config| async move {
        ...
        let outcome = async {
            ...
            if !matches!(
                mismatched,
                Err(wrokbot_infra::auth::oidc::OidcLoginError::Attempt(
                    PostgresAttemptError::ProviderMismatch
                ))
            ) {
                return Err(format!(
