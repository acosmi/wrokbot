...
use wrokbot_contracts::ids::{ActorId, BotId, TenantId, ThreadId};
...
fn request(query: &str) -> RecallMemoriesRequest {
    RecallMemoriesRequest {
        auth_generation: wrokbot_contracts::auth::AuthGeneration::new(0),
        deployment: wrokbot_contracts::ids::DeploymentId::new("dep-a"),
...
#[tokio::test]
#[ignore = "requires isolated PostgreSQL via WROK_BOT_TEST_DATABASE_URL"]
    ...
    with_temp_database(&admin_config("Han fallback authority"), "recallhanauth", |config| async move {
        ...
        let outcome=async {
            ...
            for status in ["deleted", "forbidden", "expired", "superseded"] {
                ...
                match status {
                    "deleted"|"forbidden" => {
                        store.mutate(MutateMemoryRequest{auth_generation:wrokbot_contracts::auth::AuthGeneration::new(0),tenant:TenantId::new("tenant-a"),actor:ActorId::new("actor-a"),memory_id:record.memory_id,mutation:if status=="deleted"{MemoryMutation::Delete}else{MemoryMutation::Forbid}}).await.map_err(|e|e.to_string())?;
                    }
                    "expired" => {
                    ...
                    _ => {
                        store.correct(CorrectMemoryRequest{auth_generation:wrokbot_contracts::auth::AuthGeneration::new(0),tenant:TenantId::new("tenant-a"),actor:ActorId::new("actor-a"),memory_id:record.memory_id,correction:CorrectMemory{content:"改喝白水".to_owned(),tags:Vec::new(),sensitivity:MemorySensitivity::Normal,expires_at:None}}).await.map_err(|e|e.to_string())?;
                    }
                }
    ...
    with_temp_database(&admin_config("memory lifecycle recall baseline"), "recalllifecycle", |config| async move {
        ...
        let outcome = async {
            ...
            for (record,mutation) in [(deleted,MemoryMutation::Delete),(forbidden,MemoryMutation::Forbid)] {
                let erased=store.mutate(MutateMemoryRequest { auth_generation: wrokbot_contracts::auth::AuthGeneration::new(0),tenant:TenantId::new("tenant-a"),actor:ActorId::new("actor-a"),memory_id:record.memory_id,mutation}).await.map_err(|e| e.to_string())?;
                if erased.content.is_some() { return Err("erasure retained content".to_owned()); }
            }
            let new=store.correct(CorrectMemoryRequest { auth_generation: wrokbot_contracts::auth::AuthGeneration::new(0), tenant:TenantId::new("tenant-a"),actor:ActorId::new("actor-a"),memory_id:old.memory_id.clone(),correction:CorrectMemory {content:"lifecycle corrected".to_owned(),tags:Vec::new(),sensitivity:MemorySensitivity::Normal,expires_at:None} }).await.map_err(|e| e.to_string())?;
    ...
    with_temp_database(&admin_config("saved memory source snapshots"), "recallsource", |config| async move {
        ...
        let outcome=async {
            ...
            for record in snapshots {
                let erased=store.mutate(MutateMemoryRequest{auth_generation:wrokbot_contracts::auth::AuthGeneration::new(0),tenant:TenantId::new("tenant-a"),actor:ActorId::new("actor-a"),memory_id:record.memory_id,mutation:MemoryMutation::Delete}).await.map_err(|e|e.to_string())?;
