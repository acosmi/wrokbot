...
use wrokbot_contracts::ids::{ActorId, TenantId};
...
async fn dynamic_saml_is_encrypted_replay_safe_and_delete_cleans_account_anchors() {
    ...
    with_temp_database(&admin, "dynamic_sso", |config| async move {
        ...
        let outcome = async {
            ...
            let issued = service
                .saml_callback(
                    &wrokbot_infra::auth::oidc::ProviderId::parse("acme-saml").unwrap(),
                    &relay_state,
...
#[tokio::test]
#[ignore = "需要真实 PostgreSQL + 本机 TLS：设 WROK_BOT_TEST_DATABASE_URL 后加 --include-ignored"]
    ...
    with_temp_database(&admin, "dynamic_oidc", |config| async move {
        ...
        let outcome = async {
            ...
            *auth_params.lock().unwrap() = Some(OidcAuthorizationParams {
            ...
            let provider = wrokbot_infra::auth::oidc::ProviderId::parse("acme-oidc").unwrap();
            let issued = second
                .oidc_callback(
                    &provider,
                    &state,
