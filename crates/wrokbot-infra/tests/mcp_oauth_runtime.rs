...
use wrokbot_application::{
    ...
    McpOAuthCallbackInput, WrokBotApplication, RunExecutionLease,
};
use wrokbot_contracts::auth::{AuthContextBuilder, AuthGeneration, Role};
use wrokbot_contracts::ids::{ActorId, BotId, DeploymentId, RunId, TenantId, ThreadId};
...
use wrokbot_infra::mcp_catalog::{McpAuthentication, PostgresMcpCatalog};
...
#[tokio::test]
#[ignore = "requires isolated PostgreSQL and loopback sockets; set WROK_BOT_TEST_DATABASE_URL"]
async fn admin_credential_retirement_keeps_exact_oauth_context_through_retry_replacement_and_server_removal()
 {
    use wrokbot_application::credential_admin::CredentialAdministration;
    use wrokbot_contracts::credential_admin::CredentialExternalRevocation;
...
impl ServerHandler for OAuthMcpServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            .with_server_info(Implementation::new("wrokbot-oauth-test", "3.1.4"))
