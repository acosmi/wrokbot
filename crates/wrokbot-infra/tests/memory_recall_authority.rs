...
use wrokbot_application::{
    ApplicationService, MemoryAdministration, MemoryAdministrationError, WrokBotApplication,
...
use wrokbot_contracts::ids::{ActorId, BotId, DeploymentId, TenantId, ThreadId};
...
async fn old_application_auth_is_rejected_after_people_revoke_and_restore() {
    fixture("memory actor revoke", "memoryauth", |pool, _| async move {
        ...
        let app: Arc<dyn ApplicationService> =
            Arc::new(WrokBotApplication::new(ChannelRepo::new(pool.clone())).with_memory(store));
        let old = auth(0);
        let positive = app
