...
mod harness;
use wrokbot_application::model_connections::ModelConnectionAdministration;
use wrokbot_application::{BeginThreadRunRequest, ThreadDirectory, ThreadDirectoryError as Error};
...
async fn both_anchors_three_protocols_snapshot_all_fields_and_keep_private_facts_out_of_history() {
  ...
  for channel in [false,true] {for protocol in [CustomModelProtocol::OpenaiChatCompletions,CustomModelProtocol::OpenaiResponses,CustomModelProtocol::AnthropicMessages] {
   ...
   drop(c);
   let history=directory.thread_history(wrokbot_application::ThreadHistoryRequest{deployment:req.deployment.clone(),tenant:req.tenant.clone(),actor:req.actor.clone(),thread:req.command.thread_id.clone()}).await.unwrap();
...
impl wrokbot_application::ProviderAdapter for LegacyProviderSpy {
    async fn start(
        &self,
        request: wrokbot_application::ProviderRequest,
    ) -> Result<Box<dyn wrokbot_application::ProviderSession>, wrokbot_application::ProviderPortError>
    {
        self.0.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
        assert!(matches!(
            request.route,
            wrokbot_application::ProviderRoute::PackageOpenAi
        ));
        Ok(Box::new(LegacyProviderSession(
            vec![
                wrokbot_application::ProviderEvent::ResponseStarted {
                    response_id: "legacy-positive".into(),
                },
                wrokbot_application::ProviderEvent::TextDelta {
                    index: 0,
                    delta: "Legacy route remains usable".into(),
                },
                wrokbot_application::ProviderEvent::Usage(wrokbot_application::ProviderUsage {
                    input_tokens: 2,
                    output_tokens: 3,
                    total_tokens: 5,
                }),
                wrokbot_application::ProviderEvent::Completed,
            ]
            .into(),
        )))
    }
}
struct LegacyProviderSession(std::collections::VecDeque<wrokbot_application::ProviderEvent>);
#[async_trait::async_trait]
impl wrokbot_application::ProviderSession for LegacyProviderSession {
    async fn next_event(
        &mut self,
    ) -> Result<Option<wrokbot_application::ProviderEvent>, wrokbot_application::ProviderPortError>
    {
        Ok(self.0.pop_front())
    }
}
...
async fn custom_binding_requires_both_halves_and_missing_adapter_never_starts_legacy_provider() {
    ...
    harness::with_temp_database(&admin, "selectionguard", |config| async move {
        ...
        for mode in 0..5 {
            ...
            drop(c);
            let lease = RunExecutionLease::new(req.command.run_id.clone(), req.command.thread_id.clone(), req.command.bot_id.clone(), req.actor.clone(), wrokbot_domain::thread::FencingToken::new(1).unwrap(), 0).unwrap();
            if mode == 0 {
                assert!(matches!(context.load(&lease).await.unwrap().route, wrokbot_application::ProviderRoute::CustomModel(_)));
            } else if mode < 4 {
                assert_eq!(context.load(&lease).await.unwrap_err(), AgentContextError::Corrupt { field: "model_selection" });
            } else {
                assert!(matches!(context.load(&lease).await.unwrap().route, wrokbot_application::ProviderRoute::PackageOpenAi));
            }
        ...
        let agent = wrokbot_agent::BuiltInAgentRuntime::start(
            runtime.clone(), context, Arc::new(wrokbot_agent::ProviderRouter::new(provider.clone(), None)), Arc::new(wrokbot_agent::NoAgentToolInvoker),
            Arc::new(wrokbot_infra::agent_audit::PostgresAgentAudit::new(pool.clone(), vec![0x52;32]).unwrap()),
            wrokbot_agent::BuiltInAgentConfig { queue_capacity: 8, max_concurrency: 2, max_tool_concurrency: 1,
...
async fn custom_binding_requires_both_halves_even_when_input_metadata_is_corrupt() {
    ...
    for mode in 0..4 {
        harness::with_temp_database(&admin, "selectionmarker", |config| async move {
            ...
            let runtime: Arc<dyn RunRuntime> = Arc::new(PostgresRunRuntime::new(pool.clone(), "selection-runtime".into(), Duration::seconds(30), DEFAULT_DISPATCH_CLAIM_DURATION).unwrap());
            let provider = Arc::new(LegacyProviderSpy::default());
            let agent = wrokbot_agent::BuiltInAgentRuntime::start(runtime.clone(), context,
                Arc::new(wrokbot_agent::ProviderRouter::new(provider.clone(), None)), Arc::new(wrokbot_agent::NoAgentToolInvoker),
                Arc::new(wrokbot_infra::agent_audit::PostgresAgentAudit::new(pool.clone(), vec![0x52; 32]).unwrap()),
                wrokbot_agent::BuiltInAgentConfig { queue_capacity: 8, max_concurrency: 2, max_tool_concurrency: 1,
                    lease_renew_interval: std::time::Duration::from_secs(1), run_deadline: Some(std::time::Duration::from_secs(10)) },
            ).unwrap();
