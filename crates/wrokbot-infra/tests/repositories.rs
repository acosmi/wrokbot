...
use wrokbot_application::ChannelReader;
use wrokbot_contracts::ids::{ActorId, AuditEventId};
use wrokbot_domain::audit::chain::{ChainAnchor, ChainVerification, verify_chain};
use wrokbot_domain::audit::checkpoint::{AuditCheckpointKind, ChainSegment};
use wrokbot_domain::audit::event::{AuditEvent, AuditEventType};
use wrokbot_domain::audit::payload::{AuditIdentifier, AuditLabel, AuditPayload};
use wrokbot_domain::tool::commit::CommitState;
use serde_json::json;
use time::OffsetDateTime;

use wrokbot_infra::db::tables::{self, tool_attempts, tool_calls};
use wrokbot_infra::db::{InfraError, baseline, native, pool};
use wrokbot_infra::repo::agents::{AgentPreferenceRepo, AgentProfileRepo, AgentRepo};
use wrokbot_infra::repo::audit::AuditEventRepo;
use wrokbot_infra::repo::channels::{
    ChannelAgentRepo, ChannelMembershipRepo, ChannelRepo, LegacyIntelligenceMappingRepo,
};
use wrokbot_infra::repo::components::{
    ComponentExclusionRepo, ComponentFunctionRepo, ComponentRepo, SandboxedComponentRepo,
};
use wrokbot_infra::repo::computer::{ActionPolicyRepo, SnapshotRepo};
use wrokbot_infra::repo::import::ImportCursorRepo;
use wrokbot_infra::repo::memory::{MemoryEventRepo, MemoryRepo};
use wrokbot_infra::repo::outbox::OutboxRepo;
use wrokbot_infra::repo::people::{
    AccountRepo, IdentityProviderRepo, RevokedAccessRepo, RoleRepo, SessionRepo, UserRepo,
    VerificationRepo,
};
use wrokbot_infra::repo::plugins::{
    McpServerRepo, McpToolRepo, McpUserCredentialRepo, PluginGrantRepo, SkillRepo,
};
use wrokbot_infra::repo::run::{RunEventRepo, RunRepo};
use wrokbot_infra::repo::tenant::DeploymentPackageRepo;
use wrokbot_infra::repo::thread::{MessageRepo, ThreadLeaseRepo, ThreadMembershipRepo, ThreadRepo};
use wrokbot_infra::repo::tools::{
    FirstDurableDecision, PersistedToolOutcome, ToolAttemptRepo, ToolCallRepo,
};
use wrokbot_infra::vault::CredentialRepo;
    ...
    with_temp_database(&admin, "repo_people", |config| async move {
        ...
        let outcome = async {
            ...
            let admin_rows = all_roles
                .iter()
                .filter(|row| row.role == wrokbot_infra::db::types::Role::Admin)
                .count();
            if roles
                .count_admins()
                .await
