...
fn retried(inner: Arc<dyn ProviderAdapter>) -> wrokbot_agent::RetryingProvider {
    wrokbot_agent::RetryingProvider::new(
        inner,
        wrokbot_agent::RetryingProviderConfig {
            max_retries: 1,
...
async fn retry_revalidates_revision_and_actor_after_429_without_a_second_post() {
    ...
    harness::with_temp_database(&admin,"customretry",|config|async move {
        ...
        for mode in 0..3 {
            ...
            let router=wrokbot_agent::ProviderRouter::new(legacy.clone(),None)
...
async fn current_authority_ciphertext_and_bidirectional_snapshot_fail_closed_before_tls() {
    use wrokbot_domain::vault::{SecretKind, SecretPrincipal, ServiceId};
...
struct RunningAgent {
    agent: wrokbot_agent::BuiltInAgentRuntime,
    relay: wrokbot_infra::run_runtime::RunRelay,
...
impl RunningAgent {
    fn start(
        ...
        tools: Arc<dyn wrokbot_agent::AgentToolInvoker>,
    ) -> Self {
        use wrokbot_infra::run_runtime::{
            DEFAULT_DISPATCH_CLAIM_DURATION, PostgresRunRuntime, RunRelay,
        };
        let runtime: Arc<dyn wrokbot_application::RunRuntime> = Arc::new(
        ...
        let legacy = Arc::new(NoFallback::default());
        let router =
            wrokbot_agent::ProviderRouter::new(legacy.clone(), None).with_custom(Arc::new(adapter));
        let agent = wrokbot_agent::BuiltInAgentRuntime::start(
            ...
            tools,
            Arc::new(
                wrokbot_infra::agent_audit::PostgresAgentAudit::new(f.pool.clone(), vec![0x72; 32])
                    .unwrap(),
            ),
            wrokbot_agent::BuiltInAgentConfig {
                queue_capacity: 8,
                max_concurrency: 2,
...
async fn real_agent_tool_pipeline_persists_exchange_and_samples_same_custom_model_again() {
    use wrokbot_application::{ApplicationService, ComponentAdministration, WrokBotApplication};
    use wrokbot_infra::{
    ...
    harness::with_temp_database(&admin,"customtoolsampling",|config|async move {
        ...
        components.sync_catalogue(&auth(),&wrokbot_contracts::components::compiled_component_manifest()).await.unwrap();
        let application:Arc<dyn ApplicationService>=Arc::new(WrokBotApplication::new(ChannelRepo::new(f.pool.clone())).with_component_administration(components.clone()));
        let gateway=wrokbot_agent::AuthorizedAgentToolGateway::with_sequence(application,
...
async fn unpriced_custom_run_is_durably_rejected_and_authoritative_cap_cannot_be_omitted() {
    ...
    harness::with_temp_database(&admin,"customunpriced",|config|async move {
        ...
        let running=RunningAgent::start(&f,Arc::new(context),adapter,Arc::new(wrokbot_agent::NoAgentToolInvoker));
        let result=terminal(&f.pool,&begin.command.run_id).await;running.stop().await;
...
async fn durable_cancel_while_headers_pending_stops_agent_and_releases_source() {
    ...
    harness::with_temp_database(&admin, "customdurablecancel", |config| async move {
        ...
        let running = RunningAgent::start(
            ...
            Arc::new(f.context()),
            f.adapter(tls.dialer(), Duration::from_secs(3)),
            Arc::new(wrokbot_agent::NoAgentToolInvoker),
        );
        tls.wait_count(1).await;
        f.directory
            .cancel_thread_run(wrokbot_application::CancelThreadRunRequest {
                deployment: DeploymentId::new(DEP),
                tenant: TenantId::new(TENANT),
                actor: ActorId::new("alice"),
                command: wrokbot_contracts::command::CancelThreadRun {
