...
//!
//! ```text
//! WROK_BOT_TEST_DATABASE_URL="host=127.0.0.1 port=5432 user=postgres password=... dbname=postgres" \
//!   cargo test -p wrokbot-infra --all-features -- --include-ignored
//! ```
//!
//! 环境变量给的是**管理连接**（通常连 `postgres` 库）：每个用例自己 `CREATE DATABASE` 一个
//! 带随机后缀的临时库，跑完 `DROP DATABASE ... WITH (FORCE)` 删掉。用例**从不**改动
//! 环境变量指向的那个库，也从不碰参照库 `wrokbot_ref_0012`。
//!
...
use wrokbot_infra::db::pool;
use wrokbot_infra::db::schema_facts::{SchemaFacts, TableFacts};
use wrokbot_infra::db::tables;
use wrokbot_infra::db::{baseline, compat, schema_facts};
    ...
    with_temp_database(&admin, "advval", |config| async move {
        ...
        let outcome = async {
            ...
            let credential_kinds = client
                ...
                .map(|c| c.kind)
                .collect::<Vec<_>>();
            use wrokbot_infra::db::types::{AgentType, AgentVisibility, CredentialKind, Role};
            for want in [
...
async fn connect_fails_immediately_when_the_database_does_not_exist() {
    ...
    assert!(
        matches!(error, wrokbot_infra::db::InfraError::Connect { .. }),
        "应当报连接失败而不是别的档：{error:?}",
    );
}
...
async fn a_server_side_error_never_carries_the_offending_column_value() {
    ...
    with_temp_database(&admin, "errleak", |config| async move {
        ...
        let outcome = async {
            ...
            let infra = wrokbot_infra::db::InfraError::query("插入会话", raw);
            let rendered = format!(
                "{infra} | {infra:?} | {}",
                wrokbot_infra::db::error_chain(&infra),
            );
...
async fn a_short_drizzle_ledger_is_rejected() {
    ...
    with_temp_database(&admin, "ledgershort", |config| async move {
        ...
        let outcome = async {
            ...
            if !matches!(
                error,
                wrokbot_infra::db::InfraError::IncompatibleDatabase(_)
            ) {
                return Err(format!("应当报「库不兼容」而不是别的档：{error:?}"));
            }
            let rendered = format!("{error} | {}", wrokbot_infra::db::error_chain(&error));
...
async fn a_failed_connection_never_carries_the_password() {
    ...
    let rendered = format!(
        "{error} | {error:?} | {}",
        wrokbot_infra::db::error_chain(&error),
    );
    assert!(
...
macro_rules! decode_tables {
    ($client:expr, $problems:expr, $decoded:expr, $($module:ident),+ $(,)?) => {
        $({
            use wrokbot_infra::db::tables::$module as table;
