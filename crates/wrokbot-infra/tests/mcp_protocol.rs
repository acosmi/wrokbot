...
use wrokbot_agent::{AgentToolInvoker, AuthorizedAgentToolGateway};
use wrokbot_application::{
    AgentContextSource, ApplicationService, McpConnectionAdministration, WrokBotApplication,
...
use wrokbot_contracts::ids::{ActorId, BotId, DeploymentId, RunId, TenantId, ThreadId, ToolCallId};
use wrokbot_contracts::mcp::McpCustomServerRegistration;
...
use wrokbot_infra::mcp::{MAX_MCP_RESULT_CHARS, McpClientError, SafeRmcpClient, normalize_result};
use wrokbot_infra::mcp_catalog::PostgresMcpCatalog;
use wrokbot_infra::mcp_connections::PostgresMcpConnections;
use wrokbot_infra::mcp_credentials::PostgresMcpCredentialBroker;
...
use wrokbot_infra::tool_approval::PostgresToolApprovalCoordinator;
...
const RMCP_RUN_CANCELLATION_FIXTURE_SHA256: &str =
    "3e4788f83960adec748fac5c16068c5e06e652cab104609cb60a2daf35477409";
...
impl ServerHandler for RealMcpServer {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build())
            .with_server_info(Implementation::new("wrokbot-test-rmcp", "3.1.4"))
...
async fn real_rmcp_server_lists_calls_normalizes_and_closes_per_operation() {
    ...
    let expected_schema_hash = wrokbot_domain::audit::hash::Sha256Digest::of(
...
fn rmcp_run_cancellation_fixture_is_closed_and_current() {
    ...
    assert_eq!(
        wrokbot_domain::audit::hash::Sha256Digest::of(RMCP_RUN_CANCELLATION_FIXTURE.as_bytes())
            .to_hex(),
        RMCP_RUN_CANCELLATION_FIXTURE_SHA256
    );
    assert_eq!(fixture["schema"], "wrokbot-rmcp-run-cancellation-v2");
...
async fn cancellation_already_requested_prevents_every_rmcp_network_effect() {
    ...
    let result = client()
        .call_tool_bound_cancellable(
            ...
            None,
            "must_not_send",
            wrokbot_domain::audit::hash::Sha256Digest::of(b"must-not-matter"),
            json!({}),
            cancellation,
        )
        .await;
...
async fn modern_tool_call_cancellation_closes_response_stream_without_notification_post() {
    ...
    let schema = json!({"type":"object","properties":{}})
        ...
        .cloned()
        .unwrap();
    let schema_hash = wrokbot_domain::audit::hash::Sha256Digest::of(
...
#[tokio::test]
#[ignore = "需要真实 PostgreSQL 与本机 TLS：设 WROK_BOT_TEST_DATABASE_URL 后加 --include-ignored 运行"]
        ...
        let outcome = async {
            ...
            if denied_page.servers.len() != 1
                || denied_page.servers[0].authentication
                    != wrokbot_contracts::mcp::McpAdminAuthentication::DeploymentBearer
...
async fn catalog_generation_suspends_missing_and_changed_grants_without_auto_revival() {
    ...
    harness::with_temp_database(&admin, "mcpcatalog", |config| async move {
        ...
        let result = async {
            ...
            let holder = catalog
                .granted_tools(
                    &wrokbot_contracts::ids::BotId::new("holder"),
                    &wrokbot_contracts::ids::ActorId::new("actor-a"),
                )
                .await
                .map_err(|error| error.to_string())?;
            if holder.len() != 1
                ...
                || holder[0].effect != wrokbot_domain::tool::metadata::Effect::Read
            {
                return Err(format!("reviewed grant projection drift: {holder:?}"));
            }
            if !catalog
                .granted_tools(
                    &wrokbot_contracts::ids::BotId::new("stranger"),
                    &wrokbot_contracts::ids::ActorId::new("actor-a"),
                )
                .await
                .map_err(|error| error.to_string())?
            ...
            *listed.lock().unwrap() = vec![changed];
            ...
            if !matches!(
                catalog
                    ...
                    .await,
                Err(wrokbot_infra::mcp_catalog::McpCatalogError::Corrupt {
    ...
    harness::with_temp_database(&admin, "mcptoolplane", |config| async move {
        ...
        let result = async {
            ...
            let cancelled_result = cancelled_call
                .await
                .map_err(|error| error.to_string())?;
            if cancelled_result != Err(wrokbot_agent::AgentToolInvokeError::ReconciliationRequired)
            {
                return Err(format!(
            ...
            let secret_refused = gateway
                .invoke(
                    ...
                    "mcp__notes__search_issues",
                    json!({"query":"WROK_BOT_SECRET_CANARY-do-not-send"}),
                )
                .await
                .map_err(|error| error.to_string())?;
            ...
            if pending.tool_name != "mcp__notes__long_answer"
                ...
                || pending.effect != wrokbot_contracts::tool::ToolApprovalEffect::Execute
