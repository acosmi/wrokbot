...
use wrokbot_application::model_connections::normalize_model_configuration;
use wrokbot_application::{BeginThreadRunRequest, ThreadDirectoryError as Error};
use wrokbot_contracts::{command::ThreadRunAnchor, model_connections::CustomModelProtocol};
...
pub(crate) struct LoadedSelection {
    pub(crate) binding: wrokbot_application::RunModelBinding,
...
) -> Result<Option<wrokbot_application::RunModelBinding>, wrokbot_application::AgentContextError> {
    load_selection(
        client,
        deployment,
        tenant,
...
pub(crate) async fn load_current_selection(
    ...
    tenant: &wrokbot_contracts::ids::TenantId,
    lease: &wrokbot_application::RunExecutionLease,
) -> Result<Option<LoadedSelection>, wrokbot_application::AgentContextError> {
    load_selection(
        tx,
        deployment,
        tenant,
...
async fn load_selection<C: tokio_postgres::GenericClient + Sync>(
    ...
    use wrokbot_application::AgentContextError as ContextError;
    ...
    let binding = wrokbot_application::RunModelBinding::from_verified_snapshot(
        lease,
        deployment.clone(),
        tenant.clone(),
        wrokbot_contracts::auth::AuthGeneration::new(generation),
        selection,
