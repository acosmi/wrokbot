...
use wrokbot_contracts::auth::{AuthContext, AuthContextBuilder, AuthGeneration, Role};
use wrokbot_contracts::ids::{ActorId, ComputerGeneration, DeploymentId, RunId, TenantId};
use wrokbot_contracts::memory::MemoryStatus;
use wrokbot_contracts::tool::ToolInvocation;
use wrokbot_domain::identity::roles::resolve_effective_role;
use wrokbot_domain::policy::context::{
    ActorRef, BotRef, Intent, McpEffect, McpRef, PageRef, PolicyContext, ToolRef,
};
use wrokbot_domain::policy::evaluate;
use wrokbot_domain::tool::approval::{
    ApprovalBinding, ApprovalObservation, ApprovalTarget, PolicyVersionTag,
};
use wrokbot_domain::tool::args::ToolArguments;
use wrokbot_domain::tool::commit::CommitState;
use wrokbot_domain::tool::metadata::{
    ApprovalClass, Effect, EffectClassification, Idempotency, ResourceLockKey, SandboxRequirement,
    ToolLimits, ToolMetadata, ToolName,
};
use wrokbot_domain::tool::pipeline::{ApprovalEvidence, ApprovalOutcome};
use serde_json::json;
...
impl AgentAuthorizationSource for PostgresAgentAuthorizationSource {
    async fn load(
        &self,
        lease: &wrokbot_application::RunExecutionLease,
...
fn different_catalog_generation(
    current: wrokbot_contracts::ids::CatalogGeneration,
) -> wrokbot_contracts::ids::CatalogGeneration {
    wrokbot_contracts::ids::CatalogGeneration::new(if current.get() == u64::MAX {
        0
    } else {
        current.get() + 1
    })
    ...
    M: RememberToolMemory + 'static,
{
    ...
    async fn resolve_scope(
        ...
        let thread_id = wrokbot_contracts::ids::ThreadId::new(thread_id);
        let (target, policy_context, preflight_refusal, approval_presentation) = match kind {
            ResolvedToolKind::Remember(parsed) => {
                let target = match parsed.scope() {
                    wrokbot_application::RememberToolScope::User => ApprovalTarget {
                        kind: "memory_user",
                        id: auth.actor().as_str().to_owned(),
                    },
                    wrokbot_application::RememberToolScope::Bot => ApprovalTarget {
                        kind: "memory_bot",
                        id: invocation.bot_id.as_str().to_owned(),
                    },
                    wrokbot_application::RememberToolScope::Thread => ApprovalTarget {
                        kind: "memory_thread",
        ...
        match result {
            ...
            Ok(Err(error)) => {
                let (commit, code) = match error {
                    wrokbot_application::MemoryAdministrationError::CommitUnknown => {
                        (CommitState::Unknown, "memory_commit_unknown")
                    }
                    wrokbot_application::MemoryAdministrationError::InvalidInput { .. } => {
                        (CommitState::NotCommitted, "invalid_arguments")
                    }
                    wrokbot_application::MemoryAdministrationError::NotVisible => {
                        (CommitState::NotCommitted, "not_visible")
                    }
                    wrokbot_application::MemoryAdministrationError::Conflict => {
                        (CommitState::NotCommitted, "memory_conflict")
                    }
                    wrokbot_application::MemoryAdministrationError::WritesDisabled => {
                        (CommitState::NotCommitted, "memory_writes_disabled")
                    }
                    wrokbot_application::MemoryAdministrationError::Unavailable
                    | wrokbot_application::MemoryAdministrationError::Corrupt { .. } => {
...
fn is_known_secret_value(value: &str) -> bool {
    if value.contains("-----BEGIN PRIVATE KEY-----")
        ...
        || value.contains("WROK_BOT_SECRET_CANARY")
