...
use wrokbot_domain::run::{Run, RunEvent, RunEventKind};
use wrokbot_domain::thread::{FencingToken, Message, MessageId, MessageRole};
    ...
    async fn thread_conversation(
        ...
        Ok(ThreadConversationSnapshot {
            ...
            active_run_id: active
                .into_iter()
                .next()
                .map(wrokbot_contracts::ids::RunId::new),
            active_run_state,
...
fn decode_history_message(
    ...
    let selected_skill_slugs = match value.get("selectedSkillSlugs") {
        ...
        Some(slugs) if role == ThreadHistoryRole::User => {
            ...
            if !wrokbot_contracts::command::valid_selected_skill_slugs(&slugs) {
                return Err(ThreadDirectoryError::Corrupt {
                    field: "selected_skill_slugs",
                });
            }
...
fn decode_thread_event(
    ...
    Ok(ThreadRunEvent {
        ...
        run_id: wrokbot_contracts::ids::RunId::new(decode::<String>(row, "run_id")?),
        event_sequence,
        event_type,
        payload,
        terminal,
