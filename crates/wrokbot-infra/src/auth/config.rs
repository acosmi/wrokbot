...
//! `wrokbot_server::config::migration`，因为那是启动期唯一一次看得见整张环境表的地方。
...
//! # 已知的重复与偏差，交付时请一并看
//!
//! - [`Secret`] 与 `wrokbot_server::config::Secret` 同名但不再同形：本类型已以内层
//!   [`SecretBytes`] 获得 zeroize/no-Clone 边界；server 配置 secret 的统一属于它自己的消费者
//!   批次，不能靠复制本实现制造第二份秘密原语。
//! - `KEY_ENCRYPTION_KEY` 的台账 `target` 写的是 `wrokbot-infra::vault::KeyEncryptionKey`，
...
//!
//! # TrimString、规范化与集合类型一律复用 `wrokbot-domain`，本模块不再自造
//!
//! `INITIAL_ADMIN_EMAILS` 落成 [`AdminFloor`]（内含 [`NormalizedEmail`](wrokbot_domain::identity::email::NormalizedEmail)），
...
use wrokbot_domain::identity::session::{OriginMalformed, SessionLifetimePolicy, TrustedOrigins};
use wrokbot_domain::text::trim_ecmascript;
use wrokbot_domain::vault::{SecretBytes, WrappingKey};
use time::Duration;

/// 一次启动看见的全部环境变量。
///
/// 与 `wrokbot_server::config::EnvMap` 是**同一个类型**（两边都是
...
///
/// ```compile_fail
/// use wrokbot_infra::auth::config::Secret;
...
/// 这与 `wrokbot_server::readiness` 里 `DataMigrationVerdict -> ReadinessVerdict` 是同一个
/// 手法，也是同一个理由。
///
...
impl KeyEncryptionKey {
    ...
    /// 借出包装密钥，交给 `wrokbot_domain::vault` 的 AEAD。
    #[must_use]
...
pub struct AuthConfig {
    ...
    ///
    /// 由启动层从 `wrokbot_server::config::ServerConfig::public_url` 传入，**本模块不自己读**
    ...
    /// [`NormalizedEmail`](wrokbot_domain::identity::email::NormalizedEmail) 之间，
...
/// 认证配置里的**一个**毛病。
///
/// 每个字段都是 `&'static str` —— 与 `wrokbot_application::ports::PortError` 同一条决定：
...
/// 一次认证配置解析的**全部**失败。
///
/// 聚合而不是"抛第一个就停"，理由与 `wrokbot_server::config::error` 那边相同：
...
///
/// 它是 `wrokbot_server::config::ServerConfig::public_url` 已经解析、校验、剥过尾斜杠的
...
#[cfg(test)]
mod tests {
    use wrokbot_domain::identity::email::NormalizedEmail;
    use wrokbot_domain::identity::session::SessionPolicyInvalid;
    ...
    fn signed_in_env() -> EnvMap {
        env(&[
            ...
            ("INITIAL_ADMIN_EMAILS", "admin@wrokbot.test"),
        ])
    }
    ...
    fn a_complete_configuration_starts() {
        ...
        assert!(
            config
                .admin_floor
                .contains(&NormalizedEmail::normalize("admin@wrokbot.test").expect("非空"))
        );
    ...
    fn a_public_url_alone_is_not_a_reason_to_refuse() {
        ...
        assert!(
            auth_config(
                &single_user,
                Some("https://wrokbot.example.com"),
                default_session_lifetime()
            )
            .expect("应当放行")
            .is_none()
    ...
    fn administrator_addresses_are_normalised_the_way_upstream_compares_them() {
        let listed = with(
            ...
            &[(
                ...
                " Admin@WrokBot.Test , ,OWNER@wrokbot.test ",
            )],
        );
        ...
        assert_eq!(
            config
                ...
                .collect::<Vec<_>>(),
            vec!["admin@wrokbot.test", "owner@wrokbot.test"]
        );

        // 比对两侧都规范化：大小写与空白都不该把人挡在外面。
        let admin = NormalizedEmail::normalize("  ADMIN@WrokBot.TEST ").expect("非空");
        assert!(config.admin_floor.contains(&admin));
        // 负向对照：不在名单里的人确实不是管理员。
        let stranger = NormalizedEmail::normalize("stranger@wrokbot.test").expect("非空");
    ...
    fn a_byte_order_mark_does_not_silently_disable_the_admin_floor() {
        let with_bom = with(
            &signed_in_env(),
            &[("INITIAL_ADMIN_EMAILS", "\u{FEFF}admin@wrokbot.test")],
        );
        ...
        assert_eq!(
            "\u{FEFF}admin@wrokbot.test".trim(),
            "\u{FEFF}admin@wrokbot.test"
        );
    }
    ...
    /// 逐字节相同。旧名本身的拒绝由 `wrokbot_server::config::migration` 负责。
    #[test]
    ...
    /// `wrokbot_server::config::migration` 那条 `renamed_single_user_flag` 承担：
