//! `ChannelRepo` —— `wrokbot_application::ChannelReader` 的 PostgreSQL 实现。
//!
//! 落点由 `parity/tables.yaml::tbl-channels` 的 notes 里 `repo=wrokbot-infra::repo::channels`
//! 钉死，逐字兑现。
//!
...
use wrokbot_contracts::command::{ChannelDetail, ChannelSummary};
...
use wrokbot_domain::channel::{PRIVATE_AGENT_CHANNEL_DESCRIPTION, derive_channel_name};
...
fn summary_from_row(row: &tokio_postgres::Row) -> Result<ChannelSummary, PortError> {
    ...
    Ok(ChannelSummary {
        ...
        created_at,
        thread_id: thread_id.map(wrokbot_contracts::ids::ThreadId::new),
        active,
    })
}
