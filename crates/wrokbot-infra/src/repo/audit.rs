...
use wrokbot_domain::audit::chain::{PrevRowHash, StoredAuditRow};
use wrokbot_domain::audit::checkpoint::{
    AuditCheckpoint, AuditCheckpointKind, CheckpointSignature,
};
use wrokbot_domain::audit::event::AuditEvent;
...
pub(crate) async fn next_event_coordinates(
    transaction: &tokio_postgres::Transaction<'_>,
) -> Result<(wrokbot_contracts::ids::AuditEventId, OffsetDateTime), InfraError> {
    transaction
        .query_one("SELECT pg_advisory_xact_lock($1)", &[&AUDIT_CHAIN_LOCK_KEY])
        .await
    ...
    Ok((
        wrokbot_contracts::ids::AuditEventId::new(id.to_string()),
        created_at,
    ))
}
