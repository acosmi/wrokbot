...
use wrokbot_application::provider::RemoteAguiTransport;
use wrokbot_application::{
    ApplicationService, WrokBotApplication, ProviderAdapter, RunRuntime,
...
pub async fn assemble_postgres_application(
    ...
    let thread_directory = PostgresThreadDirectory::with_runtime(
        ...
        listener_database
            .clone()
            .with_application_name("wrokbot-thread-events"),
        runtime_owner,
        DEFAULT_THREAD_LEASE_DURATION,
    )
    ...
    let application = WrokBotApplication::new(channels.clone())
        .with_people(people)
