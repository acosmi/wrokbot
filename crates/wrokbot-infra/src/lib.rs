//! `wrokbot-infra` —— 适配器层：PostgreSQL、vault、HTTP safe dialer、provider adapters。
//!
...
//! 明确**不**负责：
//!
//! - 业务规则与 use case 编排 —— 在 `wrokbot-application`；本 crate 只提供 port 的实现。
//! - 领域不变量判定 —— 在 `wrokbot-domain`。
//! - Agent loop、AG-UI 事件语义、tool runtime、MCP 会话 —— 在 `wrokbot-agent`。
...
//!   [`repo::channels::ChannelRepo`] 同时实现 `wrokbot_application::ChannelReader`。
