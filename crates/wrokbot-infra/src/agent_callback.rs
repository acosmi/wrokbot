...
use wrokbot_contracts::agent::{CallbackTokenIssued, CallbackTokenRevoked};
use wrokbot_contracts::auth::{AuthContext, AuthContextBuilder, AuthGeneration, Role};
use wrokbot_contracts::ids::{BotId, DeploymentId, TenantId, ThreadId};
...
impl RemoteCallbackAuthenticator for PostgresRemoteCallbackAuthenticator {
    async fn authorize(
        ...
        let stored_hash = wrokbot_domain::audit::hash::Sha256Digest::parse_hex(&stored_hash)
            .map_err(|_| RemoteCallbackAuthError::Corrupt {
                field: "callback_token_hash",
            })?;
        ...
        if requested_tool.is_empty()
            || requested_tool.len() > wrokbot_domain::remote_callback::MAX_REMOTE_TOOL_NAME_BYTES
