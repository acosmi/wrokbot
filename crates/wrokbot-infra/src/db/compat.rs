//! 迁移边界检查（v3 §14.1 逐字：「升级前先要求旧 WrokBot 把数据库迁到当前第 13 条 migration
...
//!
//! 1. 28 张表一张不少 —— 缺表说明库比 0012 老，或者根本不是 WrokBot 的库。
        ...
        match self {
            ...
            Self::IncompleteMigrationLedger {
                ...
                "迁移账本不足：{ledger_table} 只有 {entries} 条记录，期望 {expected} 条。\
                 ...
                 处置：用旧版 WrokBot 跑 `bun drizzle-kit migrate` 把剩余 migration 补完"
            ),
        }
    }
}
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(
            ...
            "数据库 schema 不是上游第 13 条 migration（0012）的终态，拒绝启动。\
             请先用旧版 WrokBot 把库迁到 0012 再升级（v3 §14.1：Rust 不接收更早 schema）。\
             共 {} 处不兼容：",
            self.findings.len(),
        )?;
