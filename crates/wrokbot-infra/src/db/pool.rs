...
/// startup-only [`wrokbot_domain::vault::SecretBytes`] owner, avoiding a second public config path
...
mod tests {
    ...
    fn debug_never_prints_the_password() {
        let config = DatabaseConfig::new("127.0.0.1", 5432, "postgres", "wrokbot")
            .with_password("hunter2-绝对不能出现在日志里")
            .with_application_name("wrokbot-test");
        ...
        assert!(rendered.contains("wrokbot-test"));
        assert!(rendered.contains("5432"));
    }

    #[test]
    fn debug_distinguishes_no_password_from_a_redacted_one() {
        let without = DatabaseConfig::new("127.0.0.1", 5432, "postgres", "wrokbot");
        assert!(format!("{without:?}").contains("password: None"));
    }

    #[test]
    fn builders_keep_the_other_fields_untouched() {
        let base = DatabaseConfig::new("db.internal", 6432, "wrokbot", "wrokbot")
            .with_password("s3cret")
            .with_application_name("wrokbot-server")
    ...
    fn pg_config_carries_every_field_that_was_set() {
        let config = DatabaseConfig::new("db.internal", 6432, "wrokbot", "wrokbot_main")
            .with_password("s3cret")
            .with_application_name("wrokbot-server");
        let pg = config.to_pg_config();
        assert_eq!(pg.get_ports(), &[6432]);
        assert_eq!(pg.get_user(), Some("wrokbot"));
        assert_eq!(pg.get_dbname(), Some("wrokbot_main"));
        assert_eq!(pg.get_password(), Some(&b"s3cret"[..]));
        assert_eq!(pg.get_application_name(), Some("wrokbot-server"));
    ...
    fn database_url_and_keyword_forms_share_one_parser_and_redact_password() {
        for raw in [
            "postgresql://wrokbot:secret@127.0.0.1:5544/wrokbot",
            "host=127.0.0.1 port=5544 user=wrokbot password=secret dbname=wrokbot",
        ] {
            let config: DatabaseConfig = raw.parse().unwrap();
            assert_eq!(config.host, "127.0.0.1");
            assert_eq!(config.port, 5544);
            assert_eq!(config.user, "wrokbot");
            assert_eq!(config.dbname, "wrokbot");
