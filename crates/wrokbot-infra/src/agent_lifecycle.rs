...
#[cfg(test)]
use wrokbot_application::RemoteAguiEventStream;
...
impl PostgresAgentAdministration {
    ...
    fn prepare_credential(
        ...
        agent_id: &str,
        owner: &str,
        auth: &wrokbot_contracts::agent::AgentAuthInput,
        ...
        let encrypted = self
            ...
            .seal(
                &id,
                SecretKind::Agent,
                SecretPrincipal::Actor(wrokbot_contracts::ids::ActorId::new(owner)),
...
async fn probe_agent_connection(
    ...
    let thread_id = format!("wrokbot-connection-test-{}", Uuid::now_v7());
    let run_id = format!("wrokbot-connection-test-{}", Uuid::now_v7());
    ...
    let body = serde_json::to_vec(&json!({
        ...
        "messages": [{
            "id": message_id,
            "role": "user",
            "content": "WrokBot connection test. Reply briefly."
        }],
        "tools": [],
        "context": [],
        "state": {},
...
mod tests {
    ...
    use super::*;
    use wrokbot_contracts::agent::AgentAuthInput;
