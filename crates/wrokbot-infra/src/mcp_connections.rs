...
use wrokbot_domain::agent::profile_policy::{AgentActor, AgentProfileFacts, can_access_agent};
...
fn validate_skill_slug(slug: &str) -> Result<(), McpConnectionError> {
    if !wrokbot_contracts::mcp::valid_skill_slug(slug) {
...
struct StoredAgentFacts {
    ...
    visibility: wrokbot_contracts::agent::AgentVisibility,
    system_owned: bool,
    deleted: bool,
}
...
fn decode_agent_facts(row: &tokio_postgres::Row) -> Result<StoredAgentFacts, McpConnectionError> {
    ...
    let visibility = match row
        ...
        .as_str()
    {
        "public" => wrokbot_contracts::agent::AgentVisibility::Public,
        "private" => wrokbot_contracts::agent::AgentVisibility::Private,
