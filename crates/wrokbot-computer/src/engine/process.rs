...
fn spawn_engine(
    ...
    .map_err(|_| EngineProcessError::SandboxUnavailable)?;
    wrokbot_windows_sandbox::spawn_restricted(&policy)
...
impl RuntimeDirectory {
    fn create(token: &BootToken) -> Result<Self, EngineProcessError> {
        ...
        let root = base.join(format!("wrokbot-eng-{}", &token.hex()[..16]));
        fs::create_dir(&root)?;
        use std::os::unix::fs::PermissionsExt as _;
...
impl RuntimeDirectory {
    fn create(token: &BootToken) -> Result<Self, EngineProcessError> {
        ...
        Ok(Self {
            control_pipe: PathBuf::from(format!(r"\\.\pipe\wrokbot-eng-{nonce}.control")),
            frame_pipe: PathBuf::from(format!(r"\\.\pipe\wrokbot-eng-{nonce}.frame")),
        })
    }
}
...
mod tests {
    ...
    fn macos_main_read_policy_blocks_sibling_symlink_and_data_volume_alias() {
        ...
        assert_eq!(fixture["schema"], "wrokbot-macos-engine-main-read-v1");
        assert!(
            fixture["remaining"]
                .as_object()
        ...
        let root =
            std::env::temp_dir().join(format!("wrokbot-macos-read-policy-{}", std::process::id()));
        assert!(
            !root.exists(),
    ...
    fn macos_main_network_policy_only_connects_its_two_owned_pipes() {
        ...
        assert_eq!(fixture["schema"], "wrokbot-macos-engine-main-network-v1");
        assert!(
        ...
        let root = Path::new("/private/tmp").join(format!("wrokbot-net-{}", std::process::id()));
        assert!(!root.exists());
        ...
        let invoke = |kind: &str, address: &str, confined: bool| {
            ...
            command
                ...
                .env_clear()
                .env("PATH", "/usr/bin:/bin")
                .env("WROK_BOT_NATIVE_NETWORK_PROBE_KIND", kind)
                .env("WROK_BOT_NATIVE_NETWORK_PROBE_ADDRESS", address)
                .current_dir(&profile)
                .output()
    ...
    fn macos_network_probe_child() {
        let kind = std::env::var("WROK_BOT_NATIVE_NETWORK_PROBE_KIND").expect("owned probe kind");
        let address =
            std::env::var("WROK_BOT_NATIVE_NETWORK_PROBE_ADDRESS").expect("owned probe address");
    ...
    fn macos_helper_rejects_loader_injection_after_parent_profile_release() {
        ...
        let root = Path::new("/private/tmp").join(format!("wrokbot-helper-{}", std::process::id()));
        assert!(!root.exists());
        ...
        let workspace = Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .expect("workspace");
        let bundle = std::env::var_os("WROK_BOT_ENGINE_LOADER_FIXTURE_DIR")
            .map(PathBuf::from)
    ...
    fn bundle_rejects_legacy_schema_missing_helper_and_modified_helper_bytes() {
        ...
        let workspace = Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .and_then(Path::parent)
            .expect("workspace");
        let source = std::env::var_os("WROK_BOT_ENGINE_LOADER_FIXTURE_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|| {
        ...
        let original: serde_json::Value =
            serde_json::from_slice(&fs::read(source.join("manifest.json")).expect("real bundle"))
                .expect("manifest");
        let root = std::env::temp_dir().join(format!("wrokbot-bundle-integrity-{}", std::process::id()));
        assert!(!root.exists());
