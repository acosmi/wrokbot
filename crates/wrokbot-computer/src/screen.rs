...
use wrokbot_application::{ScreenSessionAdministration, ScreenSessionAdministrationError};
use wrokbot_contracts::auth::{AuthContext, AuthGeneration};
use wrokbot_contracts::engine::MAX_ENGINE_IMAGE_BYTES;
use wrokbot_contracts::ids::{ActorId, ComputerGeneration, ComputerId, TabId, TenantId};
...
mod tests {
    ...
    async fn two_viewers_share_latest_frames_and_generation_invalidation_closes_both() {
        ...
        hub.attach(source).await.expect("attach");
        let actor = auth("actor", 4);
        let binding_a = ScreenViewerBinding::verified_desktop("wrokbot://localhost", "a", 1)
            .expect("binding a");
        let binding_b = ScreenViewerBinding::verified_desktop("wrokbot://localhost", "b", 2)
            .expect("binding b");
        let ticket_a = hub
    ...
    async fn application_port_resolves_target_and_never_logs_issued_protocol() {
        ...
        let request = ScreenSessionRequest {
            target: wrokbot_contracts::screen::ScreenSessionTarget {
