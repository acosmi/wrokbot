//! `wrokbot-computer` —— 计算机控制层：manager、supervisor、browser protocol、screen、file/shell。
//!
...
//! 明确**不**负责：
//!
//! - 决定某次操作允不允许 —— policy 判定在 `wrokbot-domain`，管线顺序在 `wrokbot-application`。
//! - 直接对外提供 HTTP/WS 端点 —— Server 面在 `wrokbot-server`，Desktop 面在 `wrokbot-desktop`。
