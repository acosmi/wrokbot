...
use wrokbot_computer::browser::{BrowserInput, ModifierMask, MouseButton};
use wrokbot_computer::control::{ControlError, ControlService, HumanInputTicket};
use wrokbot_computer::engine::{
...
use wrokbot_computer::screen::coordinates::{CanvasRect, DecodedFrameSize, ScreenCoordinateMap};
use wrokbot_computer::screen::{ScreenHub, ScreenHubError, ScreenViewerBinding};
use wrokbot_contracts::auth::{AuthContext, AuthGeneration, Role};
...
fn demand_fixture_keeps_protocol_and_production_boundary_explicit() {
    ...
    assert_eq!(
        ...
        wrokbot_contracts::engine::ENGINE_PROTOCOL_VERSION
    );
...
fn engine_input_fixture_locks_protocol_and_unfinished_platform_boundaries() {
    ...
    assert_eq!(fixture["schema"], "wrokbot-engine-input-wire-v2");
...
fn screencast_fixture_locks_ack_order_latest_buffer_and_remaining_screen_boundary() {
    ...
    assert_eq!(fixture["schema"], "wrokbot-screencast-backpressure-v3");
...
fn screen_hub_fixture_locks_ticket_and_production_transport_boundary() {
    ...
    assert_eq!(fixture["schema"], "wrokbot-screen-hub-ticket-core-v1");
...
fn screen_coordinate_fixture_locks_units_journeys_and_hardware_boundary() {
    ...
    assert_eq!(
        fixture["schema"],
        "wrokbot-screen-coordinate-input-journey-v1"
    );
    assert_eq!(
...
async fn run_demand_role(role: EngineRole) {
    use wrokbot_computer::screen::engine_owner::{ScreenEngineOwner, ScreenEngineState};
    ...
    assert_eq!(
        owner
            ...
            .await,
        Err(wrokbot_computer::screen::engine_owner::ScreenEngineError::InputRefused)
    );
    let ticket = {
...
async fn wait_demand_state(
    state: &mut tokio::sync::watch::Receiver<
        wrokbot_computer::screen::engine_owner::ScreenEngineState,
    >,
    expected: wrokbot_computer::screen::engine_owner::ScreenEngineState,
) {
...
async fn demand_viewer(
    ...
    computer: &ComputerId,
    tab: &TabId,
) -> wrokbot_computer::screen::ScreenViewer {
    let binding =
...
#[cfg(target_os = "macos")]
const PARENT_ENVIRONMENT_CANARY: &str = "WROK_BOT_ENGINE_PARENT_ENVIRONMENT_CANARY";
...
fn both_roles_exclude_the_parent_environment_from_main_and_renderer() {
    ...
    assert_eq!(fixture["schema"], "wrokbot-engine-child-environment-v1");
    assert!(
...
fn fixture_bundle_root() -> PathBuf {
    ...
    std::env::var_os("WROK_BOT_ENGINE_LOADER_FIXTURE_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| {
...
async fn run_role(role: EngineRole) {
    ...
    let binding_a = if tag == "browser" {
        ...
        ScreenViewerBinding::verified_desktop("wrokbot://localhost", "component-a", 1)
            .expect("desktop binding")
    };
    let binding_b = if tag == "browser" {
        binding_a.clone()
    } else {
        ScreenViewerBinding::verified_desktop("wrokbot://localhost", "component-b", 2)
            .expect("desktop binding")
    };
    assert!(matches!(
        hub.issue_ticket(
...
) -> wrokbot_computer::engine::ScreenIngressStats {
    for _ in 0..100 {
...
impl TestDirectories {
    fn new(tag: &str) -> Self {
        let root = std::env::temp_dir().join(format!(
            "wrokbot-engine-conformance-{tag}-{}",
            std::process::id()
        ));
