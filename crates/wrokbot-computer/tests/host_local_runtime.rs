...
use wrokbot_computer::browser::BrowserInput;
use wrokbot_computer::engine::{
    ComputerSecurityScope, EngineBundle, EngineBundleDigest, ScreenAudience, WorkspaceScope,
};
use wrokbot_computer::host_local_runtime::{
    HostLocalBrowserRuntime, HostLocalRuntimeConfig, HostLocalRuntimeError,
};
use wrokbot_computer::manager::{BrowserInstance, BrowserRuntimeBudget};
use wrokbot_computer::screen::{ScreenHub, ScreenViewerBinding};
...
fn bundle() -> EngineBundle {
    let workspace = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .unwrap()
        .to_path_buf();
    let root = std::env::var_os("WROK_BOT_ENGINE_LOADER_FIXTURE_DIR")
        .map(PathBuf::from)
    ...
    fn new() -> Self {
        ...
        Self(
            std::env::temp_dir()
                .canonicalize()
                .unwrap()
                .join(format!("wrokbot-host-live-{}-{nonce}", std::process::id())),
        )
    }
}
impl Drop for Root {
...
async fn failed_profile_replacement_does_not_restore_an_already_retired_engine() {
    use wrokbot_computer::engine::EngineRole;
