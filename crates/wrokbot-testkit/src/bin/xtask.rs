//! `xtask` —— 仓库闸门驱动器。
//!
//! 落点是 `wrokbot-testkit` 的 bin target 而不是第 11 个 crate：v3 §5.1 只允许四个理由建
//! crate（独立安全边界 / 独立发布单元 / 明显不同的 feature graph / 可单独复用的纯协议），
//! xtask 一个都不满足；而 §5.1 的 crate 表里 `wrokbot-testkit` 的职责原文就写着 "xtask"。
...
// `test-inventory` 的实现落在 `crates/wrokbot-testkit/src/xtask/test_inventory.rs`。
...
/// The workspace members outside the parity-owner domain: R127's Win32 unsafe FFI boundary, plus
/// R228/R282's macOS-only independent security-boundary members. None contains migrated product
/// behavior; each exists solely because it hits §5.1's "independent security boundary / distinct
/// feature graph" crate-split criteria.
#[cfg(test)]
const NON_PARITY_WORKSPACE_MEMBERS: [&str; 3] = [
    "wrokbot-windows-sandbox",
    "wrokbot-macos-local-auth",
    "wrokbot-macos-process",
];
...
/// v3 §1.2 固定源码基线里的 CopilotKit/WrokBot commit。
/// 只用于**告警**，不是 8 条硬规则之一。
...
用法：
  ...
  cargo xtask ui-assets               用与 wrokbot-ui build.rs 同一生成器物化 ignored tokens.css
...
/// `CARGO_MANIFEST_DIR/../..`（`crates/wrokbot-testkit` 的祖父目录 = 仓根）。
///
    ...
    if driver_conflicts_with_child_target(&exe, &child_target) {
        bail!(
            "驱动器自己就落在子构建的 target 树里，第 3 步 `cargo test` 会去重链它：\n\
             ...
             不要用 `cargo run -p wrokbot-testkit --features xtask --bin xtask -- ci`。",
            exe.display(),
...
mod tests {
    ...
    fn rule3_rejects_owner_outside_ten_crates() {
        // `wrokbot-core` 不在 v3 §5.1 的十个 crate 里；建第 11 个 crate 要过四条理由的评审。
        let entry = VALID_ENTRY.replace("    owner: wrokbot-server\n", "    owner: wrokbot-core\n");
    ...
    fn rule4_done_with_evidence_is_accepted() {
        let entry = format!(
            "{}    done_evidence: cargo test -p wrokbot-server thread_get\n",
            VALID_ENTRY.replace("    status: todo\n", "    status: done\n")
        );
        let (violations, _) = run(&doc(&entry));
    ...
    fn accepts_migration_rule_with_colon_suffix() {
        let entry = VALID_ENTRY.replace(
            "    migration_rule: preserve\n",
            "    migration_rule: \"rename: WROKBOT_ 前缀统一\"\n",
        );
        let (violations, warnings) = run(&doc(&entry));
    ...
    fn owners_match_workspace_members() {
        // 十个 parity OWNERS + R127/R228/R282 三个窄化安全边界必须恰好覆盖 workspace；新增
        // 未登记的例外或把安全边界误列成产品迁移 owner 都会判红。
        let manifest = include_str!("../../../../Cargo.toml");
        for owner in OWNERS {
            // The UI directory was renamed; the Cargo package and parity owner stay stable.
            let directory = if owner == "wrokbot-ui" {
                "wrokbot-ui"
            } else {
                owner
            };
        ...
        for member in NON_PARITY_WORKSPACE_MEMBERS {
            assert!(
                ...
                "非 parity 安全边界 {member} 不在 workspace members 里"
            );
        ...
        let member_count = manifest
            ...
            .filter(|l| {
                ...
                line.starts_with("\"crates/wrokbot-") || line.starts_with("\"crates/wrokbot-ui\"")
            })
            .count();
        assert_eq!(
            member_count,
            OWNERS.len() + NON_PARITY_WORKSPACE_MEMBERS.len(),
            "workspace 必须恰为十个 parity owner + R127/R228/R282 三个安全边界"
        );
    }
    ...
    fn driver_in_a_sibling_tree_is_not_a_conflict() {
        ...
        repo.write("target/debug/wrokbot-server.exe", "child build output");
