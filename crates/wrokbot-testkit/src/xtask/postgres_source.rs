...
fn source_pin(root: &Path) -> Result<SourcePin> {
    ...
    let engine_source = fs::read_to_string(root.join("crates/wrokbot-contracts/src/engine.rs"))
    ...
    let desktop = fs::read_to_string(root.join("crates/wrokbot-desktop/src/postgres_sidecar.rs"))
