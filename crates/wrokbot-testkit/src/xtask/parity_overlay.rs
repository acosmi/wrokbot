...
fn path_target_prefix(path: &str) -> Option<String> {
    let path = path.strip_prefix("crates/wrokbot-")?;
    let (crate_name, source) = path.split_once("/src/")?;
    let mut prefix = format!("wrokbot_{}", crate_name.replace('-', "_"));
...
"#;
    ...
    fn targets() -> BTreeMap<String, String> {
        [
            ("T-BROP-0046", "wrokbot_computer::control::ControlService"),
            (
                "T-CMP-0015",
                "wrokbot_ui::features::gallery::sandboxed::renderer",
            ),
        ]
        .into_iter()
    ...
    fn renderer_prefix() -> BTreeSet<String> {
        BTreeSet::from(["wrokbot_ui::features::gallery::sandboxed::renderer".to_owned()])
    }
    ...
    fn untouched_split_requires_no_evidence_and_existing_revalidate_still_works() {
        ...
        let mut violations = Vec::new();
        validate_document(&document, &test_ids(), 10, &mut violations);
        let prefixes = BTreeSet::from(["wrokbot_computer::control".to_owned()]);
        assert_eq!(
    ...
    fn other_legal_desktop_splits_can_revalidate_only_with_their_own_evidence() {
        ...
        done_targets.insert("T-EXTRA".to_owned(), "wrokbot_desktop::example".to_owned());
        let prefixes = BTreeSet::from(["wrokbot_desktop::example".to_owned()]);
        let mut violations = Vec::new();
    ...
    fn rust_source_path_becomes_ledger_target_prefix() {
        assert_eq!(
            super::path_target_prefix("crates/wrokbot-computer/src/control.rs").as_deref(),
            Some("wrokbot_computer::control")
        );
        assert_eq!(
            super::path_target_prefix("crates/wrokbot-testkit/src/xtask/engine.rs").as_deref(),
            Some("wrokbot_testkit::xtask::engine")
        );
    }
}
