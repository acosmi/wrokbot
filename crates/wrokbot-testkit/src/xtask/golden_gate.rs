...
fn validate_image_dependency(workspace: &Path) -> Result<()> {
    ...
    let testkit_path = workspace.join("crates/wrokbot-testkit/Cargo.toml");
    ...
    for entry in fs::read_dir(workspace.join("crates"))? {
        let entry = entry?;
        if !entry.file_type()?.is_dir() || entry.file_name() == "wrokbot-testkit" {
            continue;
        }
        ...
        if manifest_uses_dependency(&value, "image") {
            bail!("image dependency escaped wrokbot-testkit into a product crate");
        }
    }
...
fn load_masks(
    ...
    if document.schema != "wrokbot-golden-mask-resolution-v1"
...
mod tests {
    use super::*;
    use wrokbot_testkit::golden::FULL_DIFFERENCE_BLOCK_SIZE;
    ...
    impl TempRoot {
        fn new() -> Self {
            ...
            let path = std::env::temp_dir().join(format!(
                "wrokbot-golden-gate-test-{}-{sequence}",
                std::process::id()
            ));
    ...
    fn unreviewed_mask_sidecar_is_rejected_when_manifest_allowlist_is_empty() {
        ...
        fs::write(
            &sidecar,
            br#"{"schema":"wrokbot-golden-mask-resolution-v1","image":"web/a.png","page":"/","rectangles":[{"selector":"[data-golden-mask]","x":0,"y":0,"width":1,"height":1}]}"#,
        )
        .unwrap();
        let rules = ManifestRules::load(&workspace().join(DEFAULT_MANIFEST)).unwrap();
