...
fn check_dist_script_policy(dist: &Path) -> Result<usize> {
    ...
    // HTML minification (`[build] minify = "on_release"` in Trunk.toml) drops attribute quotes
    // when the value has no whitespace/special characters, so `src` must be matched in both its
    // quoted (`src="..."`/`src='...'`) and bare (`src=/path`) forms; either is valid HTML.
    let src = Regex::new(r#"(?i)\bsrc\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s"'>]+))"#)?;
    let mut count = 0;
    ...
    for capture in script.captures_iter(&index) {
        ...
        let Some(src_match) = src.captures(attributes) else {
            bail!("compiled index.html contains an inline script");
        };
        let src_value = src_match
            .get(1)
            .or_else(|| src_match.get(2))
            .or_else(|| src_match.get(3))
            .expect("src regex always captures one alternative")
            .as_str();
        if src_value == "/wrokbot-bootstrap.mjs" {
            bootstrap_found = true;
        }
    }
    if count != 1 || !bootstrap_found {
        bail!("compiled index.html must contain exactly one external WrokBot bootstrap script");
    }
    let bootstrap_path = dist.join("wrokbot-bootstrap.mjs");
...
fn check_icon_mapping_join(
    ...
    for (tabler, target_name) in &documented {
        ...
        if tabler == "IconBrandGoogleDrive" {
            if target != "wrokbot_ui::icons::brand::GoogleDrive" || status != "todo" {
        ...
        let expected_target = format!("wrokbot_ui::icons::Icon::{}", icon_variant(target_name));
...
mod tests {
    ...
    fn compiled_css_class_parser_unescapes_tailwind_selectors() {
        let classes =
            compiled_css_classes(".wrokbot-card{display:block}.md\\:grid{display:grid}").unwrap();
        assert!(classes.contains("wrokbot-card"));
