...
pub(crate) fn bundle(root: &Path) -> Result<()> {
    ...
    let shim = root.join("crates/wrokbot-desktop/engine-shim");
...
pub(crate) fn verify_if_required(root: &Path) -> Result<()> {
    if !root.join("crates/wrokbot-desktop/engine-shim").is_dir() {
        return Ok(());
    }
    let pins = crate::engine::pins(root)?;
    let electron = crate::engine::electron(&pins)?;
...
fn write_asar_integrity(layout: &BundleLayout, hash: &str, platform: &str) -> Result<()> {
    ...
    if platform == "windows-x64" {
        #[cfg(windows)]
        {
            ...
            wrokbot_windows_sandbox::replace_pe_resource(
                &layout.executable,
                "Integrity",
                "ElectronAsar",
                &payload,
            )?;
            let actual = wrokbot_windows_sandbox::read_pe_resource(
                &layout.executable,
                "Integrity",
                "ElectronAsar",
            )?;
...
fn verify_windows(root: &Path, manifest: &BundleManifest) -> Result<()> {
    ...
    #[cfg(windows)]
    {
        ...
        let resource =
            wrokbot_windows_sandbox::read_pe_resource(&executable, "Integrity", "ElectronAsar")?;
