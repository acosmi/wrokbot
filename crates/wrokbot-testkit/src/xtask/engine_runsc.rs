...
const PROBE_SCHEMA: &str = "wrokbot-runsc-engine-spike-v1";
...
pub(crate) fn run(
    ...
    let container_id = format!("wrokbot-p1-{}", std::process::id());
...
fn oci_config(
    ...
    Ok(json!({
        ...
        "process": {
            ...
            "user": { "uid": 0, "gid": 0 },
            "args": [
                "/opt/wrokbot/bin/engine-runsc-probe",
                "--bundle", "/opt/wrokbot/engine",
                "--manifest-sha256", manifest_digest,
                "--xvfb-sha256", xvfb_sha256
            ],
            "env": [
                "HOME=/tmp/wrokbot-home",
        ...
        "mounts": [
            ...
            { "destination": "/opt/wrokbot/bin", "type": "bind", "source": utf8(probe)?, "options": ["rbind", "ro", "nosuid", "nodev"] },
            { "destination": "/opt/wrokbot/engine", "type": "bind", "source": utf8(engine)?, "options": ["rbind", "ro", "nosuid", "nodev"] }
        ],
        "linux": {
            "devices": [
