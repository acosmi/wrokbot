...
const FILE_RULES: [FileRule; UPSTREAM_TEST_FILE_COUNT] = [
    ...
    FileRule { file: "agent-computer/tests/aria-snapshot.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::browser::aria", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §11.1 单一 engine / §11.2 Engine 协议：aria snapshot 解析是 engine 侧纯函数，判据逐条移植。" },
    FileRule { file: "agent-computer/tests/authorisation.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::manager::authorisation", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §10.3 Desktop 与 Server 安全表述：manager 侧共享密钥校验与未认证可达面逐条移植。" },
    FileRule { file: "agent-computer/tests/bot-id.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::identity::bot_id", tier: Tier::Ported, label: Label::Substitute,
        reason: "v3 §10.1 明写 bot_id 不足以定界，Rust 侧隔离键换成 ComputerSecurityScope；id 合法性与 profile 目录派生的判据本身逐条移植。" },
    FileRule { file: "agent-computer/tests/browser-eviction.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::browser::eviction", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §11.3 Browser 安全配置：并发上限、闲置回收与运营方配额读取判据逐条移植。" },
    FileRule { file: "agent-computer/tests/control.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::screen::handover", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §12.5 Input / §12.4 Viewer ticket：take-the-wheel 的交接、双驾驶冲突与密钥不外泄判据逐条移植。" },
    FileRule { file: "agent-computer/tests/egress.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::egress", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §10.5 Network egress：代理变量命名、按 Bot 解析与对外展示面判据逐条移植。" },
    FileRule { file: "agent-computer/tests/shell.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::shell", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §8.1 唯一执行管线：命令继承什么、实际执行什么、不能对宿主做什么，判据逐条移植。" },
    FileRule { file: "agent-computer/tests/workspace.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::files::workspace", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §10.2 Engine 能看到什么：工作区内读写、列举与越界拒绝判据逐条移植。" },

    // --- agent-langgraph（1）：v3 §3.5 框架样例不进第一方生产仓 ------------------------
    FileRule { file: "agent-langgraph/tests/history.test.ts", owner: "wrokbot-testkit", target_module: "wrokbot_testkit::fixtures::remote_agent::langgraph", tier: Tier::CoveredByGolden, label: Label::Substitute,
    ...
    FileRule { file: "app/src/components/channels/composer/draft.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::channels::composer::draft", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §13.1 GUI：草稿构造、单 Agent 约束与命令 chip 应用是纯状态函数，判据逐条移植。" },
    FileRule { file: "app/src/components/channels/composer/queue.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::channels::composer::queue", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §13.1 GUI：发送队列的提交/落定/移除是纯状态函数，判据逐条移植。" },

    // --- app/tests（18）：v3 §13 Tauri/Leptos ---------------------------------------
    FileRule { file: "app/tests/agents.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::coworkers", tier: Tier::Ported, label: Label::Substitute,
        reason: "v3 §13.2 typed in-process transport 取代 TanStack Query 的 query key 体系（key 相等性判据在 Rust 侧无对应物）；coworker 表单校验判据逐条移植。" },
    FileRule { file: "app/tests/audit-silence.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::audit::stalled_turn", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §8.6 Audit：停滞回合在审计页留下什么，判据逐条移植。" },
    FileRule { file: "app/tests/auth-client.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::auth::client", tier: Tier::Ported, label: Label::Substitute,
        reason: "v3 §6.2 必须实现的认证面：Better Auth 客户端被 Rust 自建认证面取代；登录入口与 provider 显示名判据逐条移植。" },
    FileRule { file: "app/tests/auth-queries.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::auth::queries", tier: Tier::Ported, label: Label::Substitute,
        reason: "v3 §13.2：query key 稳定性是 TanStack Query 的机制属性，Leptos + typed in-process 侧无对应物；当前用户读取语义逐条移植。" },
    FileRule { file: "app/tests/bot-thread.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::threads::bot_thread", tier: Tier::Ported, label: Label::Substitute,
        reason: "v3 §13.2：botThreadKey 是 query key（换机制）；threadToUse 的线程选择判据逐条移植。" },
    FileRule { file: "app/tests/compose-state.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::channels::compose_state", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §13.1 GUI：收件人增删与可发送判据是纯状态函数，逐条移植。" },
    FileRule { file: "app/tests/computer-activity.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::computer::activity", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §3.3 Generative UI：调用输出的读取与活动面板呈现判据逐条移植。" },
    FileRule { file: "app/tests/credential-form.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::credentials::form", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §6.4 Vault：必填四项（类型/供应商/key id/密文）的校验规则不变，zod 换成 Rust 校验器不改变判据。" },
    FileRule { file: "app/tests/new-id.test.ts", owner: "wrokbot-contracts", target_module: "wrokbot_contracts::ids::mint", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §5.3：ID 是 string newtype，创建端可用 UUIDv7/ULID、兼容端必须接受上游既有字符串；铸造函数的唯一性与形态判据逐条移植。" },
    FileRule { file: "app/tests/query-client.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::data::client", tier: Tier::Ported, label: Label::Substitute,
        reason: "v3 §13.2：TanStack QueryClient 的默认重试配置在 Leptos + typed in-process transport 下无对应物，判据搬到 Rust 侧的请求重试策略。" },
    FileRule { file: "app/tests/repair-history.test.ts", owner: "wrokbot-domain", target_module: "wrokbot_domain::agent::history_repair", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §7.2 Agent reducer：未应答 tool call 的历史修复是 reducer 前置不变量，落在 domain，判据逐条移植。" },
    FileRule { file: "app/tests/router.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::shell::router", tier: Tier::Ported, label: Label::Substitute,
        reason: "v3 §13.1：TanStack Router 生成路由表换成 leptos_router；路由存在性与 fullPath 判据搬到 parity/routes.yaml 的同一批 31 条落点上。" },
    FileRule { file: "app/tests/stopped-turn.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::threads::stopped_turn", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §7.4 Retry、Cancel、Budget 与 Commit：回合结束原因的呈现判据逐条移植。" },
    FileRule { file: "app/tests/take-the-wheel.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::computer::take_the_wheel", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §12.5 Input：页面坐标换算是纯几何函数，判据逐条移植。" },
    FileRule { file: "app/tests/theme-preference.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::theme", tier: Tier::Ported, label: Label::Substitute,
        reason: "设计系统文档 §17 记录上游主题不跟随系统（prefers-color-scheme 零命中），Rust 侧主题是三态（含 system），故偏好读写语义换了一个；两态之间的持久化判据逐条移植。" },
    FileRule { file: "app/tests/tool-name.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::threads::tool_name", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §8.2 Tool metadata：工具调用显示名派生判据逐条移植。" },
    FileRule { file: "app/tests/tool-result.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::threads::tool_result", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §8.2 Tool metadata：工具结果的读取与展示判据逐条移植。" },
    FileRule { file: "app/tests/transcript-messages.test.ts", owner: "wrokbot-ui", target_module: "wrokbot_ui::features::threads::transcript", tier: Tier::Ported, label: Label::Substitute,
    ...
    // --- server/tests（65） ----------------------------------------------------------
    FileRule { file: "server/tests/agent-callback-token.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::callback::token", tier: Tier::Ported, label: Label::Parity,
    ...
    FileRule { file: "server/tests/agent-connection-live.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::registry::connection_probe", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §7.5 Remote AG-UI：注册时的真实连通性探测（答得好 / 答得坏）判据逐条移植。" },
    FileRule { file: "server/tests/agent-endpoint.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::registry::endpoint", tier: Tier::Ported, label: Label::Substitute,
    ...
    FileRule { file: "server/tests/agent-profile-policy.test.ts", owner: "wrokbot-domain", target_module: "wrokbot_domain::agent::profile_policy", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §5.2 Hexagonal ownership：agent profile 权限判据是纯领域规则，逐条移植。" },
    FileRule { file: "server/tests/agent-profile-store.integration.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::store::agent_profile", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §14.1 单数据库裁决：agent profile 的持久化往返判据逐条移植到 PostgreSQL 后端。" },
    FileRule { file: "server/tests/agent-registry.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::registry", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §7.1 产品中存在两类 Agent：内建与远程 Agent 的可用性汇报且不泄漏密钥，判据逐条移植。" },
    FileRule { file: "server/tests/agent-routes.test.ts", owner: "wrokbot-server", target_module: "wrokbot_server::routes::agents", tier: Tier::Ported, label: Label::Parity,
    ...
    FileRule { file: "server/tests/audit-retention.integration.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::store::audit_retention", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §16.5 Retention：审计留存策略的裁剪判据逐条移植。" },
    FileRule { file: "server/tests/audit.test.ts", owner: "wrokbot-application", target_module: "wrokbot_application::audit", tier: Tier::Ported, label: Label::Substitute,
        reason: "v3 §8.6 Audit：上游自由 JSON + 敏感键黑名单改为封闭 AuditFact 字段 allowlist；事件目录、不可变与管理端读取判据逐条移植（写/读面均穿 ApplicationService，v3 §5.2）。" },
    FileRule { file: "server/tests/bot-access.test.ts", owner: "wrokbot-server", target_module: "wrokbot_server::routes::bot_access", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §3.2 无权访问统一 404 + §10.1：Bot 对 computer / 工具 / component 面的可达性判据逐条移植。" },
    FileRule { file: "server/tests/bot-lifecycle-audit.test.ts", owner: "wrokbot-application", target_module: "wrokbot_application::bot::lifecycle_audit", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §8.6 Audit：Bot 隐藏/恢复等生命周期动作在审计上留下什么，判据逐条移植。" },
    FileRule { file: "server/tests/channel-activity.integration.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::store::channel_activity", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §3.2 Channel + §14.1：某人可读哪些 channel、channel 活动如何汇总，判据逐条移植。" },
    FileRule { file: "server/tests/channel-events.integration.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::events::channel_hub", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §4.3 Native thread/realtime：channel 事件枢纽与投递判据逐条移植到 Rust/PostgreSQL 真源。" },
    FileRule { file: "server/tests/channel-routes.test.ts", owner: "wrokbot-server", target_module: "wrokbot_server::routes::channels", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §15.1 + §21.1 条 1：channel 路由输入解析、路由组合与并发写判据逐条移植。" },
    FileRule { file: "server/tests/component-decision.test.ts", owner: "wrokbot-domain", target_module: "wrokbot_domain::components::decision", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §3.3 Generative UI 与 Components：选用哪个 component 是纯领域裁决，判据逐条移植。" },
    FileRule { file: "server/tests/component-store.integration.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::store::components", tier: Tier::Ported, label: Label::Parity,
    ...
    FileRule { file: "server/tests/computer-client.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::client", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §11.2 Engine 协议 + §7.4 Cancel：元素不存在、调用方 Stop 与调用时限判据逐条移植。" },
    FileRule { file: "server/tests/computer-fleet-route.test.ts", owner: "wrokbot-server", target_module: "wrokbot_server::routes::computer_fleet", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §10.4 Server v1 + §15.1：列举部署内全部 computer 的路由判据逐条移植。" },
    FileRule { file: "server/tests/computer-gateway.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::gateway", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §8.1 唯一执行管线 + §10.4：网关裁决、人工输入命名手势而非路径、跨副本解析 ref，判据逐条移植。" },
    FileRule { file: "server/tests/computer-policy-route.test.ts", owner: "wrokbot-server", target_module: "wrokbot_server::routes::computer_policy", tier: Tier::Ported, label: Label::Substitute,
        reason: "v3 §15.1 + §8.3 + R57：部署边界读写判据逐条移植；PUT 额外要求 fresh admin + trusted Origin，且 Axum 精确路由取代 Hono wildcard exemption。" },
    FileRule { file: "server/tests/computer-policy.test.ts", owner: "wrokbot-domain", target_module: "wrokbot_domain::policy::computer", tier: Tier::Ported, label: Label::Substitute,
    ...
    FileRule { file: "server/tests/computer-provider.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::provider", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §10.2 / §10.4：隔离级别描述、共享 provider 与工厂选择判据逐条移植。" },
    FileRule { file: "server/tests/computer-routes.test.ts", owner: "wrokbot-server", target_module: "wrokbot_server::routes::computer", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §15.1 + §21.1 条 1：computer 路由、fleet 列举与人工输入路由判据逐条移植。" },
    FileRule { file: "server/tests/computer-snapshot-store.integration.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::store::computer_snapshot", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §10.4 Server v1：一台服务器上拍的快照在另一台上可读，判据逐条移植到 PostgreSQL 后端。" },
    FileRule { file: "server/tests/computer-snapshot-store.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::snapshot::memory_store", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §12.3 Frame contract：内存快照存储的读写与淘汰判据逐条移植。" },
    FileRule { file: "server/tests/computer-supervisor.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::supervisor", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §10.4：定位 Bot 的 computer 与 Docker supervisor provider 判据逐条移植。" },
    FileRule { file: "server/tests/computer-target.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::target", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §11.3 Browser 安全配置：导航目标与 computer 地址校验判据逐条移植。" },
    FileRule { file: "server/tests/config.test.ts", owner: "wrokbot-server", target_module: "wrokbot_server::config", tier: Tier::Ported, label: Label::Substitute,
    ...
    FileRule { file: "server/tests/copilot.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::compat::copilotkit_facade", tier: Tier::CoveredByGolden, label: Label::Substitute,
    ...
    FileRule { file: "server/tests/credentials.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::vault::credentials", tier: Tier::Ported, label: Label::Substitute,
    ...
    FileRule { file: "server/tests/database.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::db::client", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §14.1 单数据库裁决：构造数据库边界时不发查询，判据逐条移植。" },
    FileRule { file: "server/tests/deployment-route-bot-end-to-end.test.ts", owner: "wrokbot-server", target_module: "wrokbot_server::routes::deployment_route_bot", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §3.2 Routing：以部署路由命名的 Bot 端到端判据逐条移植。" },
    FileRule { file: "server/tests/dev-actor.integration.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::auth::single_user", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §6.1 Desktop 与 Server 身份模型：开发态 actor 的持久化判据逐条移植。" },
    FileRule { file: "server/tests/encrypt-sso-config.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::vault::sso_config", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §6.4 Vault：身份提供方配置在数据库里的加密形态判据逐条移植。" },
    FileRule { file: "server/tests/entra-profile.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::auth::oidc::claims", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §6.2 必须实现的认证面：Entra 档案映射判据逐条移植。" },
    FileRule { file: "server/tests/google-drive-rest.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::connectors::google_drive", tier: Tier::Ported, label: Label::Substitute,
    ...
    FileRule { file: "server/tests/guards.test.ts", owner: "wrokbot-application", target_module: "wrokbot_application::authz::guards", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §5.2：授权守卫是 ApplicationService 的入口不变量，判据逐条移植。" },
    FileRule { file: "server/tests/health.test.ts", owner: "wrokbot-server", target_module: "wrokbot_server::health", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §16.1 Server 发行物（health/readiness）：健康端点、运行时能力、认证可用性与 IdP 注册判据逐条移植。" },
    FileRule { file: "server/tests/human-input-end-to-end.test.ts", owner: "wrokbot-server", target_module: "wrokbot_server::routes::human_input", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §12.5 Input + §8.5 Approval：人工输入端到端判据逐条移植。" },
    FileRule { file: "server/tests/jsonb-encoding.integration.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::db::jsonb", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §21.1 条 3（关键 JSON canonical hash 差异为 0）：jsonb 列实际存了什么，判据逐条移植。" },
    FileRule { file: "server/tests/mcp-protocol.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::mcp::protocol", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §9.1 首版 MCP runtime 的精确范围：列举、调用与不可达服务器判据逐条移植。" },
    FileRule { file: "server/tests/mcp-result.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::mcp::result", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §9.1 + §28.4 记录的 MAX_RESULT_CHARS = 20_000：空结果、有内容结果与超长结果的处理判据逐条移植。" },
    FileRule { file: "server/tests/people-paging.integration.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::store::people", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §6.1：部署内人员分页读取判据逐条移植。" },
    FileRule { file: "server/tests/people-routes.test.ts", owner: "wrokbot-server", target_module: "wrokbot_server::routes::people", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §15.1 + §21.1 条 1：people 路由判据逐条移植。" },
    FileRule { file: "server/tests/plugin-catalogue.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::mcp::catalogue", tier: Tier::Ported, label: Label::Substitute,
    ...
    FileRule { file: "server/tests/plugin-oauth.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::mcp::oauth", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §9.4 OAuth：state、PKCE、去向地址与回调后落点判据逐条移植。" },
    FileRule { file: "server/tests/plugin-store.integration.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::store::plugin_grants", tier: Tier::Ported, label: Label::Substitute,
    ...
    FileRule { file: "server/tests/plugin-user-credential.integration.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::store::plugin_user_credential", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §6.4 Vault + §9.2 连接生命周期：个人凭据的连接、缺失与退役判据逐条移植。" },
    FileRule { file: "server/tests/policy-durability.integration.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::store::policy_durability", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §8.3：运行中设置的边界必须持久，判据逐条移植。" },
    FileRule { file: "server/tests/policy-fanout.integration.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::events::policy_fanout", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §8.3 明写沿用上游 policy-listener.ts 形态（LISTEN/NOTIFY 只做唤醒、每个 replica 整表重读、policy_version 进 decision），故 fanout 判据逐条移植。" },
    FileRule { file: "server/tests/roles.test.ts", owner: "wrokbot-domain", target_module: "wrokbot_domain::identity::roles", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §6.1 / §6.2：按邮箱定角色、设角色、应用配置管理员与种子角色是纯领域规则，判据逐条移植（§6.5 修正的是 group，不是 role）。" },
    FileRule { file: "server/tests/routing-classify.test.ts", owner: "wrokbot-domain", target_module: "wrokbot_domain::routing::classify", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §3.2 Routing：无 @mention 时的分派与按 coworker 可达面分派，判据逐条移植。" },
    FileRule { file: "server/tests/routing-routes.test.ts", owner: "wrokbot-server", target_module: "wrokbot_server::routes::routing", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §15.1 + §3.2：记录消息去了哪个 coworker 的路由判据逐条移植。" },
    FileRule { file: "server/tests/runtime-agents.integration.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::runtime::loading", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §7.1：运行时 agent 装载判据逐条移植。" },
    FileRule { file: "server/tests/sandboxed-components.integration.test.ts", owner: "wrokbot-application", target_module: "wrokbot_application::components::sandboxed", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §3.3：免重建撰写 component、以及不得删除本面不拥有的名字，判据逐条移植。" },
    FileRule { file: "server/tests/schema.test.ts", owner: "wrokbot-infra", target_module: "wrokbot_infra::db::schema", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §14.2 28 表 parity ledger + §21.1 条 3：schema 断言逐条移植，并与 parity/tables.yaml 同一批 28 张表交叉。" },
    FileRule { file: "server/tests/server-side-tools.integration.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::tools::server_side", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §8.1 唯一执行管线：Bot 在服务端被交到手上的工具集判据逐条移植。" },
    FileRule { file: "server/tests/single-user.test.ts", owner: "wrokbot-server", target_module: "wrokbot_infra::auth::config", tier: Tier::Ported, label: Label::Substitute,
    ...
    FileRule { file: "server/tests/skill-ownership.integration.test.ts", owner: "wrokbot-application", target_module: "wrokbot_application::skills::ownership", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §9.6 Skills 与 tool discovery：可见性、归属、编辑保持归属与 HTTP 可做什么，判据逐条移植。" },
    FileRule { file: "server/tests/stall-guard.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::stall_guard", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §7.4 Retry、Cancel、Budget 与 Commit：停流判定、留给人的那句话与不该碰的东西，判据逐条移植。" },
    FileRule { file: "server/tests/tenant-package.test.ts", owner: "wrokbot-application", target_module: "wrokbot_application::tenant::package", tier: Tier::Ported, label: Label::Substitute,
    ...
    FileRule { file: "server/tests/thread-identity.test.ts", owner: "wrokbot-contracts", target_module: "wrokbot_contracts::ids::thread", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §5.3：ThreadId 携带铸造它的 deployment，兼容端必须接受上游既有字符串，判据逐条移植。" },
    FileRule { file: "server/tests/thread-routes.test.ts", owner: "wrokbot-server", target_module: "wrokbot_server::routes::threads", tier: Tier::Ported, label: Label::Substitute,
    ...
    FileRule { file: "server/tests/thread-status.test.ts", owner: "wrokbot-application", target_module: "wrokbot_application::migration::intelligence_thread", tier: Tier::CoveredByGolden, label: Label::Substitute,
        reason: "v3 §4.1 条 3：Intelligence 只用于旧数据导出、迁移核对和一次性导入，最终请求路径不连接它 —— 该文件的被测面整体从运行路径搬进 §20.3 的迁移工具，由迁移 fixture 对照覆盖而非 1:1 移植。" },
    FileRule { file: "server/tests/turn-watchdog.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::turn::watchdog", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §7.4：回合按活性而不是按时长判定、停流只报一次、未配置则不启用，判据逐条移植。" },

    // --- shared（2） -----------------------------------------------------------------
    FileRule { file: "shared/agent-authorisation.test.ts", owner: "wrokbot-domain", target_module: "wrokbot_domain::agent::authorisation", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §5.2 + §7.1：托管 agent 的授权判定是纯领域规则，判据逐条移植。" },
    FileRule { file: "shared/bot-prompt.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::prompt::computer_guidance", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §7.2 Agent reducer：COMPUTER_GUIDANCE 提示词内容契约判据逐条移植。" },

    // --- supervisor（2） -------------------------------------------------------------
    FileRule { file: "supervisor/tests/docker.integration.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::supervisor::docker", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §10.4 Server v1：容器归属规则对真实 daemon 的判据（同名冲突返回 409）逐条移植。" },
    FileRule { file: "supervisor/tests/names.test.ts", owner: "wrokbot-computer", target_module: "wrokbot_computer::supervisor::names", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §10.1：supervisor 接受哪些 id，判据逐条移植。" },

    // --- 仓根 tests（6） --------------------------------------------------------------
    FileRule { file: "tests/agent-bot.test.ts", owner: "wrokbot-agent", target_module: "wrokbot_agent::reference::bootstrap", tier: Tier::Ported, label: Label::Substitute,
    ...
    FileRule { file: "tests/clean-checkout.test.ts", owner: "wrokbot-testkit", target_module: "wrokbot_testkit::repo::clean_checkout", tier: Tier::Ported, label: Label::Substitute,
    ...
    FileRule { file: "tests/compose.test.ts", owner: "wrokbot-testkit", target_module: "wrokbot_testkit::repo::compose", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §14.1 单数据库裁决（PostgreSQL + pgvector）：docker-compose 提供本地开发数据库的判据逐条移植。" },
    FileRule { file: "tests/fintech-package.test.ts", owner: "wrokbot-testkit", target_module: "wrokbot_testkit::fixtures::tenant_package::fintech", tier: Tier::Ported, label: Label::Substitute,
        reason: "v3 §3.2 / §6.5 保留五文件示例与 all audience；§16.2 禁止对外 product name 复用 WrokBot/CopilotKit 标记，所以 fixture 改为 fintech/Ledgerline 并增加反向品牌断言。" },
    FileRule { file: "tests/smoke/journey.test.ts", owner: "wrokbot-testkit", target_module: "wrokbot_testkit::smoke::journey", tier: Tier::Ported, label: Label::Parity,
        reason: "v3 §21.5 性能/稳定性 + §21.1：一次贯穿运行中部署的旅程（服务器→supervisor→网关→浏览器→轨迹）判据逐条移植；上游同样不在默认套件里，按名字调用。" },
    FileRule { file: "tests/workspace.test.ts", owner: "wrokbot-testkit", target_module: "wrokbot_testkit::repo::workspace", tier: Tier::Ported, label: Label::Substitute,
        reason: "v3 §5.1 精简 workspace：bun workspace 的包清单换成 cargo workspace 的十个 crate；'每个成员都声明了名字且被根清单收录'这条判据逐条移植。" },

    // --- worker（1）：v3 §3.5 逐字点名 -----------------------------------------------
    FileRule { file: "worker/tests/status.test.ts", owner: "wrokbot-testkit", target_module: "wrokbot_testkit::retired::worker_status", tier: Tier::NotApplicableWithProof, label: Label::Substitute,
        reason: "v3 §3.5 逐字：'当前 worker 只返回 {status:\"idle\"} 且没有 job，Rust 版不发布空 worker binary；该测试标记 not-applicable-with-proof'。证明面 = 发行物清单里不得出现 worker 二进制（v3 §16.1/§16.2），由 wrokbot_testkit::retired::worker_status 做反向断言，不是把 idle 断言照抄一遍。" },
];
...
fn render_ledger(
    ...
    writeln!(
        w,
        "# 分类真源 = crates/wrokbot-testkit/src/xtask/test_inventory.rs::FILE_RULES（{UPSTREAM_TEST_FILE_COUNT} 行，一行一个上游文件）。"
    )?;
    writeln!(
        w,
    ...
    let recounts: Vec<(String, &str, String)> = vec![
        ...
        (
            ...
            r#"grep -cE '^    FileRule \{ file: "' crates/wrokbot-testkit/src/xtask/test_inventory.rs"#
                .to_string(),
            "repo",
            UPSTREAM_TEST_FILE_COUNT.to_string(),
        ),
...
mod tests {
    ...
    fn oxc_version_matches_workspace_manifest() {
        let manifest = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .expect("crates/wrokbot-testkit 的祖父目录 = 仓根")
            .join("Cargo.toml");
