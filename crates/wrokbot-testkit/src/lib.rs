//! `wrokbot-testkit` —— 测试与闸门工具：golden trace、fault injection、fake provider、xtask。
//!
...
//! - 实现业务规则或绕开 `wrokbot-application`：测试也必须穿过 `ApplicationService`（v3 §5.2
...
//!
//! `tests/transport_parity.rs` 依赖 `wrokbot-application` / `-contracts` / `-server` /
