...
use wrokbot_application::{
    AppEventStream, ApplicationService, BeginThreadRunRequest, CancelThreadRunRequest,
    ComponentAdministration, MemoryAdministrationError, WrokBotApplication, ProviderAdapter,
...
use wrokbot_contracts::auth::{AuthContextBuilder, AuthGeneration, Role};
...
use wrokbot_contracts::ids::thread::ThreadIdentity;
use wrokbot_contracts::ids::{ActorId, BotId, DeploymentId, RunId, TenantId};
use wrokbot_contracts::memory::MemoryRecord;
use wrokbot_contracts::remote_interrupt::{RemoteInterruptAnswer, RemoteInterruptAnswerStatus};
use wrokbot_domain::policy::{ActionPolicy, PolicyMode};
use wrokbot_domain::remote_callback::{RemoteRunAssertionSigner, RemoteToolSet};
use wrokbot_domain::thread::FencingToken;
use wrokbot_domain::vault::{KeyVersion, SecretBytes, SecretKind, SecretPrincipal, WrappingKey};
use wrokbot_infra::agent_audit::PostgresAgentAudit;
use wrokbot_infra::agent_tools::{
    PostgresAgentAuthorizationSource, PostgresBuiltInToolControlPlane,
};
use wrokbot_infra::component_catalogue::PostgresComponentAdministration;
use wrokbot_infra::db::pool::DatabaseConfig;
use wrokbot_infra::db::{baseline, native, pool};
use wrokbot_infra::memory_admin::PostgresMemoryAdministration;
use wrokbot_infra::net::safe_http::{
    CidrAllowlist, EgressPolicy, SafeDialer, SafeHttpBudget, SchemePolicy,
};
use wrokbot_infra::policy::PolicyStore;
use wrokbot_infra::provider::anthropic::{
    AnthropicApiKey, AnthropicProvider, AnthropicProviderConfig,
};
use wrokbot_infra::provider::context::PostgresAgentContextSource;
use wrokbot_infra::provider::credential::PostgresOpenAiCredentialSource;
use wrokbot_infra::provider::google::{GoogleApiKey, GoogleProvider, GoogleProviderConfig};
use wrokbot_infra::provider::openai::{
    OpenAiApiKey, OpenAiCredentialError, OpenAiCredentialSource, OpenAiProtocol, OpenAiProvider,
    OpenAiProviderConfig,
};
use wrokbot_infra::remote_agui::SafeRemoteAguiTransport;
use wrokbot_infra::remote_interrupt::PostgresRemoteInterruptCoordinator;
use wrokbot_infra::repo::ChannelRepo;
use wrokbot_infra::repo::tools::PostgresToolJournal;
use wrokbot_infra::run_runtime::{DEFAULT_DISPATCH_CLAIM_DURATION, PostgresRunRuntime, RunRelay};
use wrokbot_infra::thread_directory::{DEFAULT_THREAD_LEASE_DURATION, PostgresThreadDirectory};
...
impl ApplicationService for ConcurrentComponentApplication {
    async fn execute(
        &self,
        auth: wrokbot_contracts::auth::AuthContext,
        command: AppCommand,
    ) -> Result<AppReply, wrokbot_contracts::error::AppError> {
    ...
    ) -> Result<AppEventStream, wrokbot_contracts::error::AppError> {
...
#[async_trait]
impl wrokbot_application::AgentContextSource for CancellationHoldingContext {
    async fn load(
        &self,
        _lease: &wrokbot_application::RunExecutionLease,
    ) -> Result<ProviderRequest, wrokbot_application::AgentContextError> {
...
#[async_trait]
impl wrokbot_application::AgentContextSource for HoldingAgentContext {
    async fn load(
        &self,
        _lease: &wrokbot_application::RunExecutionLease,
    ) -> Result<ProviderRequest, wrokbot_application::AgentContextError> {
...
#[tokio::test]
#[ignore = "需要真实 PostgreSQL：设 WROK_BOT_TEST_DATABASE_URL 后加 --include-ignored 运行"]
        ...
        let result = async {
            ...
            directory
                .begin_thread_run(BeginThreadRunRequest {
                    auth_generation: wrokbot_contracts::auth::AuthGeneration::new(0),
            ...
            if requests.len() != 1
                ...
                || requests[0].messages[0].role != wrokbot_application::ProviderMessageRole::System
                ...
                || requests[0].messages[1]
                    != (ProviderMessage {
                        role: wrokbot_application::ProviderMessageRole::User,
    ...
    with_temp_database(&admin, "agentparallel", |config| async move {
        ...
        let result = async {
            ...
            let inner: Arc<dyn ApplicationService> = Arc::new(
                WrokBotApplication::new(ChannelRepo::new(pool.clone()))
            ...
            let tool_ids = requests[1]
                .messages
                .iter()
                .filter(|message| message.role == wrokbot_application::ProviderMessageRole::Tool)
        ...
        let result = async {
            ...
            if requests.len() != 2
                ...
                || requests[1].messages.iter().all(|message| {
                    message.role != wrokbot_application::ProviderMessageRole::Assistant
                ...
                || requests[1].messages.iter().all(|message| {
                    message.role != wrokbot_application::ProviderMessageRole::Tool
...
#[tokio::test]
#[ignore = "需要真实 PostgreSQL：设 WROK_BOT_TEST_DATABASE_SOCKET/PORT/USER 后加 --include-ignored 运行"]
    ...
    with_temp_database(&admin, "agentremember", |config| async move {
        ...
        let result = async {
            ...
            let exact_exchange = RunToolExchange::new(
                wrokbot_contracts::ids::ToolCallId::new(internal_call_id),
            ...
            if runtime
                ...
                .await
                != Err(wrokbot_application::RunRuntimeError::Conflict)
            {
...
#[tokio::test]
#[ignore = "需要真实 PostgreSQL 与 loopback socket：设 WROK_BOT_TEST_DATABASE_URL 后加 --include-ignored 运行"]
        ...
        let result = async {
            ...
            let remote_server = tokio::spawn(async move {
                ...
                if body["runId"] != "run-remote"
                    || body["forwardedProps"]["wrokbotBotId"] != "bot-remote"
                    || !offered_quote
                    || !body["forwardedProps"]["wrokbotDeploymentTools"]
                        .as_array()
                        .is_some_and(|tools| {
                ...
                let assertion = body["forwardedProps"]["wrokbotRun"]
                    .as_str()
                    .ok_or_else(|| "signed wrokbotRun missing".to_owned())?;
...
fn batch6_admin_config(test_name: &str) -> DatabaseConfig {
    let Ok(socket) = std::env::var("WROK_BOT_TEST_DATABASE_SOCKET") else {
        return admin_config(test_name);
    };
    let port = std::env::var("WROK_BOT_TEST_DATABASE_PORT")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(5432);
    let user =
        std::env::var("WROK_BOT_TEST_DATABASE_USER").unwrap_or_else(|_| "postgres".to_owned());
...
async fn begin_test_run(
    ...
    run_id: &str,
    message: &str,
) -> Result<wrokbot_contracts::ids::ThreadId, String> {
    begin_test_run_for_bot(
        directory,
        deployment,
        tenant,
