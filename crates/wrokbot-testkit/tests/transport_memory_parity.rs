...
use wrokbot_application::{
    ...
    WrokBotApplication, PortError, RecallMemoriesRequest, RememberMemoryRequest,
...
use wrokbot_server::auth::{FixedAuthResolver, SensitiveWriteSecurity};
use wrokbot_server::{ServerBuilder, router};
...
impl Fixture {
    fn new() -> Self {
        let service: Arc<dyn ApplicationService> =
            Arc::new(WrokBotApplication::new(EmptyChannels).with_memory(FixedMemory));
