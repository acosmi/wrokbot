...
use wrokbot_application::{
    ...
    ChannelReader, WrokBotApplication, PortError,
};
...
use wrokbot_contracts::ids::{ActorId, AuditEventId, DeploymentId, TenantId};
use wrokbot_desktop::InProcessTransport;
use wrokbot_domain::identity::session::SessionLifetimePolicy;
use wrokbot_server::{SINGLE_USER_ACTOR_ID, ServerBuilder, SingleUserAuthResolver, router};
...
async fn audit_page_matches_between_http_and_typed_in_process_on_the_same_arc() {
    ...
    let service: Arc<dyn ApplicationService> =
        Arc::new(WrokBotApplication::new(EmptyChannels).with_audit(audit.clone()));
