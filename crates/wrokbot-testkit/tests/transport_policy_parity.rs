...
use wrokbot_application::{
    ApplicationService, ChannelCursor, ChannelReader, WrokBotApplication, PolicyAdministration,
...
use wrokbot_domain::identity::session::{SessionLifetimePolicy, TrustedOrigins};
...
fn auth() -> AuthContext {
    AuthContext::for_test(
        ...
        [Role::Admin],
        wrokbot_contracts::auth::AuthGeneration::new(0),
        true,
    )
}
...
async fn policy_get_and_put_match_between_http_and_typed_in_process_on_the_same_arc() {
    ...
    let service: Arc<dyn ApplicationService> =
        Arc::new(WrokBotApplication::new(EmptyChannels).with_policy(policies.clone()));
