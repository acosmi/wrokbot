...
//! 「typed in-process，**不复制 JSON-RPC**」；`wrokbot-desktop` 的依赖图里连
...
//! 等价性**，不是 SQL 正确性 —— 后者已由 `wrokbot-infra` 的 `tests/channel_repo.rs`
//! （真 PostgreSQL、`#[ignore]` + `WROK_BOT_TEST_DATABASE_URL`）覆盖。
//!
...
//! `wrokbot_server::http::health`），所以矩阵覆盖的是 `ListVisibleChannels` 一条。
...
use wrokbot_application::{
    ...
    WrokBotApplication, PortError, channel_recency,
};
...
use wrokbot_contracts::ids::{ActorId, ChannelId, DeploymentId, TenantId};
use wrokbot_desktop::InProcessTransport;
use wrokbot_server::auth::FixedAuthResolver;
use wrokbot_server::{ServerBuilder, ServerState, router};
...
/// 确定性内存 [`ChannelReader`]。
///
/// `Clone` 共享同一份状态（`Arc`），所以夹具可以一手把它交给 [`WrokBotApplication`]、
...
impl Outcome {
    ...
    fn from_typed(result: Result<AppReply, wrokbot_contracts::error::AppError>) -> Self {
        match result {
...
fn http_route_of(command: &AppCommand) -> Option<String> {
    match command {
        ...
        // `ApplicationService`（见 `wrokbot_server::http::health`）。所以
...
impl Fixture {
    fn new() -> Self {
        ...
        let service: Arc<dyn ApplicationService> =
            Arc::new(WrokBotApplication::new(reader.clone()));
...
async fn both_legs_share_one_application_instance() {
    ...
    let other: Arc<dyn ApplicationService> =
        Arc::new(WrokBotApplication::new(FakeChannels::new(dataset())));
