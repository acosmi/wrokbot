...
use wrokbot_desktop::{DesktopTauriProtocol, InProcessTransport};
use wrokbot_domain::identity::session::TrustedOrigins;
use wrokbot_infra::auth::config::default_session_lifetime;
use wrokbot_server::auth::{SensitiveWriteSecurity, SingleUserAuthResolver};
use wrokbot_server::{SINGLE_USER_ACTOR_ID, ServerBuilder, router};
...
fn admin_auth() -> AuthContext {
    AuthContext::for_test(
        DeploymentId::new("wrokbot-local"),
        TenantId::new("wrokbot-local"),
...
fn dist() -> PathBuf {
    let root = std::env::temp_dir().join(format!(
        "wrokbot-sandboxed-transport-parity-{}-{}",
        std::process::id(),
