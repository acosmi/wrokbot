...
use wrokbot_contracts::ids::{DeploymentId, ThreadId};
use wrokbot_contracts::intelligence::{
...
use wrokbot_contracts::memory::MemoryKind;
use serde::Serialize;
use serde_json::{Value, json};
use sha2::{Digest, Sha256};

/// 固定 legacy WrokBot source commit。
...
fn validate_runs(thread: &IntelligenceThreadExport) -> Result<(), IntelligenceImportError> {
    ...
    for run in &thread.runs {
        ...
        let event_start_valid = match (run.started_at, events.first()) {
            (Some(_), Some(event)) => {
                event.event_type == wrokbot_contracts::command::ThreadRunEventKind::Started
            }
            (None, Some(event)) => {
                run.status == IntelligenceRunStatus::Cancelled
                    && events.len() == 1
                    && event.event_type == wrokbot_contracts::command::ThreadRunEventKind::Cancelled
            }
            (_, None) => false,
        };
...
fn validate_memories(thread: &IntelligenceThreadExport) -> Result<(), IntelligenceImportError> {
        ...
        if let Some(parent) = memory.supersedes_id.as_deref() {
            *successor_counts.entry(parent).or_default() += 1;
            if by_id[parent].status
                != wrokbot_contracts::intelligence::IntelligenceMemoryStatus::Superseded
    ...
    for memory in &thread.memories {
        if (memory.status == wrokbot_contracts::intelligence::IntelligenceMemoryStatus::Superseded)
            != (successor_counts
