...
use wrokbot_contracts::auth::{AuthContext, AuthGeneration};
...
use wrokbot_contracts::tool::{ToolCommitState, ToolInvocation, ToolResult};
use wrokbot_domain::audit::hash::Sha256Digest;
use wrokbot_domain::audit::payload::{AuditFact, AuditIdentifier, AuditLabel, AuditPayload};
use wrokbot_domain::policy::{PolicyContext, PolicyDecision};
use wrokbot_domain::tool::approval::{ApprovalTarget, PolicyVersionTag};
use wrokbot_domain::tool::args::{ToolArguments, ToolArgumentsError};
use wrokbot_domain::tool::commit::{CommitState, IdempotencyKey};
use wrokbot_domain::tool::metadata::{ApprovalClass, Effect, ToolMetadata, ToolName};
use wrokbot_domain::tool::pipeline::{
...
pub enum ToolAuditDraftError {
    ...
    Identifier(#[from] wrokbot_domain::audit::payload::AuditFieldError),
    /// payload 字段重复/无效。
    #[error("tool_audit_payload_invalid")]
    Payload(#[from] wrokbot_domain::audit::payload::AuditPayloadError),
}
...
) -> Result<Vec<AuditFact>, wrokbot_domain::audit::payload::AuditFieldError> {
    let mut facts = vec![
...
pub struct AuthorizedToolCall {
    ...
    run: RunId,
    thread: ThreadId,
    auth_generation: wrokbot_contracts::auth::AuthGeneration,
...
impl ExecutableToolCall {
    ...
    #[must_use]
    pub const fn auth_generation(&self) -> wrokbot_contracts::auth::AuthGeneration {
...
mod tests {
    ...
    use wrokbot_domain::policy::context::{ActorRef, BotRef, PageRef, ToolRef};
    use wrokbot_domain::policy::{ActionPolicy, CompiledActionPolicy, PolicyMode, evaluate};
    use wrokbot_domain::tool::approval::ApprovalInvalidation;
    ...
    fn auth() -> AuthContext {
        AuthContext::for_test(
            ...
            ActorId::new("actor-1"),
            [Role::User],
            wrokbot_contracts::auth::AuthGeneration::new(9),
            false,
        )
    }
