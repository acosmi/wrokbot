...
use wrokbot_contracts::ids::{ActorId, ChannelId};
use wrokbot_contracts::text::trim_ecmascript;
use wrokbot_domain::identity::groups::{
...
pub const MANAGED_AGENT_IN_PROCESS_ENDPOINT: &str = "wrokbot-internal://managed-agent";
...
/// 官方内置本地默认包（`examples/wrokbot`，见
/// `wrokbot-server::config::launch::LOCAL_PACKAGE_DIRECTORY`）固定使用的 tenant id。
/// 它是产品随发行物自带的第一方参考配置本身，不是第三方对外冒用，因此单独豁免
/// `tenant_id` 品牌限定检查；`product_name` 仍然照常校验，裸 `wrokbot` 之类的
/// tenant id 或 product name 继续保持限定。
const RESERVED_FIRST_PARTY_TENANT_ID: &str = "wrokbot-local";

fn validate_public_brand(tenant_id: &str, product_name: &str) -> Result<(), TenantPackageError> {
    const RESTRICTED_MARKS: [&str; 6] = ["wrokbot", "copilotkit", "codex", "openai", "grok", "xai"];
    let tenant = tenant_id.to_ascii_lowercase();
    let product = product_name.to_ascii_lowercase();
    let tenant_restricted = tenant_id != RESERVED_FIRST_PARTY_TENANT_ID
        && RESTRICTED_MARKS.iter().any(|mark| tenant.contains(mark));
    if tenant_restricted || RESTRICTED_MARKS.iter().any(|mark| product.contains(mark)) {
...
mod tests {
    use super::*;
    use wrokbot_domain::identity::groups::{GroupClaimPath, GroupNormalization, IdpGroupMapping};
    ...
    fn rejects_imports_selectors_and_unsupported_variables() {
        ...
        let mut files = valid_files();
        files.brand = "tenant: { id: wrokbot, product_name: WrokBot }".to_owned();
        assert_eq!(
    ...
    #[test]
    fn reserved_first_party_local_tenant_id_is_exempt_but_product_name_still_restricted() {
        let mut files = valid_files();
        files.brand = "tenant: { id: wrokbot-local, product_name: Wrok Bot }".to_owned();
        assert!(validate_tenant_package(files).is_ok());

        let mut files = valid_files();
        files.brand = "tenant: { id: wrokbot-local, product_name: WrokBot }".to_owned();
        assert_eq!(
            validate_tenant_package(files).unwrap_err(),
            TenantPackageError::BrandRestricted
        );
        ...
        let mut files = valid_files();
        files.brand = "tenant: { id: wrokbot-local-clone, product_name: Neutral }".to_owned();
        assert_eq!(
            validate_tenant_package(files).unwrap_err(),
            TenantPackageError::BrandRestricted
        );
