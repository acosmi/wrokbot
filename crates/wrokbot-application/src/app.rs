//! [`WrokBotApplication`] —— [`ApplicationService`] 的具体实现，把端口与 use case 接起来。
//!
...
use wrokbot_contracts::command::{AppCommand, AppReply, SubscriptionRequest};
...
///
/// 它对具体数据源一无所知：`R` 是任何满足 [`ChannelReader`] 的类型。`wrokbot-server` 与
/// `wrokbot-desktop` 各自注入 `wrokbot-infra` 的实现，测试注入内存 fake —— 三条路径穿的
/// 是同一份业务代码，这正是 §24 G1「ApplicationService 经 Axum/Tauri 结果一致」的前提。
pub struct WrokBotApplication<
    R,
...
impl<R, P, A, K, C, J, T, M, B> WrokBotApplication<R, P, A, K, C, J, T, M, B> {
    /// 注入 people/auth 原子端口。
    #[must_use]
    pub fn with_people<Q>(self, people: Q) -> WrokBotApplication<R, Q, A, K, C, J, T, M, B> {
        WrokBotApplication {
    ...
    #[must_use]
    pub fn with_audit<Q>(self, audit: Q) -> WrokBotApplication<R, P, Q, K, C, J, T, M, B> {
        WrokBotApplication {
            channels: self.channels,
            people: self.people,
            audit,
    ...
    #[must_use]
    pub fn with_policy<Q>(self, policies: Q) -> WrokBotApplication<R, P, A, Q, C, J, T, M, B> {
        WrokBotApplication {
            channels: self.channels,
            people: self.people,
            audit: self.audit,
    ...
    pub fn with_tools<Q, L>(
        ...
        control: Q,
        journal: L,
    ) -> WrokBotApplication<R, P, A, K, Q, L, T, M, B> {
        WrokBotApplication {
            channels: self.channels,
            people: self.people,
            audit: self.audit,
    ...
    #[must_use]
    pub fn with_threads<Q>(self, threads: Q) -> WrokBotApplication<R, P, A, K, C, J, Q, M, B> {
        WrokBotApplication {
            channels: self.channels,
            people: self.people,
            audit: self.audit,
    ...
    #[must_use]
    pub fn with_memory<Q>(self, memory: Q) -> WrokBotApplication<R, P, A, K, C, J, T, Q, B> {
        WrokBotApplication {
            channels: self.channels,
            people: self.people,
            audit: self.audit,
    ...
    pub fn with_agent_callback_tokens<Q>(
        self,
        callback_tokens: Q,
    ) -> WrokBotApplication<R, P, A, K, C, J, T, M, Q> {
        WrokBotApplication {
            channels: self.channels,
            people: self.people,
            audit: self.audit,
...
#[async_trait]
impl<R, P, A, K, C, J, T, M, B> ApplicationService for WrokBotApplication<R, P, A, K, C, J, T, M, B>
where
...
mod tests {
    ...
    use wrokbot_contracts::command::{AppEvent, BeginThreadRun, ThreadRunAnchor, ThreadRunStarted};
    ...
    use wrokbot_contracts::policy::{ActionPolicyDocument, ActionPolicyMode};
    ...
    fn app() -> WrokBotApplication<FakeChannelReader, FakePeopleAdministration> {
        WrokBotApplication::new(
    ...
    fn execute_dispatches_every_command_variant() {
        ...
        assert_eq!(
            ...
            AppReply::Health(wrokbot_contracts::command::HealthReport { ok: true })
        );
        ...
        let (channel_reply, _) = capture(service.execute(
            ...
            AppCommand::GetVisibleChannel {
                channel_id: wrokbot_contracts::ids::ChannelId::new("c-1"),
            },
        ));
    ...
    fn a_roleless_authenticated_actor_is_not_rejected() {
        let roleless = AuthContext::for_test(
            ...
            ActorId::new("actor-1"),
            [],
            wrokbot_contracts::auth::AuthGeneration::new(1),
            false,
        );
    ...
    fn begin_thread_run_reaches_the_port_through_application_service_execute() {
        let service =
            WrokBotApplication::new(FakeChannelReader::empty()).with_threads(FixedThreadBegin);
    ...
    fn approval_subscription_is_typed_and_fail_closed_without_the_production_port() {
        let service = WrokBotApplication::new(FakeChannelReader::empty());
        let (result, captured) = capture(service.subscribe(
            auth_for("actor-1"),
            SubscriptionRequest::ToolApprovalActivity,
        ));
