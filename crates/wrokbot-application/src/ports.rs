...
//!
//! 这是六边形架构里那根决定依赖方向的轴：依赖箭头是 `wrokbot-infra -> wrokbot-application`。
...
use wrokbot_contracts::error::{AppError, IdentityConflictReason};
...
///
/// 查询同时接收 deployment、tenant 与 actor，三者都来自权威 [`AuthContext`](wrokbot_contracts::auth::AuthContext)，
...
pub trait ThreadDirectory: Send + Sync {
    ...
    ) -> Result<wrokbot_contracts::command::ThreadConversationSnapshot, ThreadDirectoryError> {
        Err(ThreadDirectoryError::Unavailable)
    }
}
...
mod tests {
    use super::*;
    use wrokbot_contracts::error::{AuditKind, ErrorCode};
