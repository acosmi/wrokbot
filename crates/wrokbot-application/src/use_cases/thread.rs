...
pub async fn begin_thread_run<D: ThreadDirectory>(
    ...
    if !wrokbot_contracts::command::valid_selected_skill_slugs(&command.selected_skill_slugs) {
        return Err(AppError::MalformedPayload {
...
mod tests {
    ...
    fn begin_command() -> BeginThreadRun {
        BeginThreadRun {
            ...
            run_id: wrokbot_contracts::ids::RunId::new("run-1"),
            bot_id: wrokbot_contracts::ids::BotId::new("bot-1"),
    ...
    async fn begin_injects_authoritative_scope_and_returns_the_durable_receipt() {
        ...
        assert_eq!(
            ...
            &[BeginThreadRunRequest {
                auth_generation: wrokbot_contracts::auth::AuthGeneration::new(7),
                deployment: DeploymentId::new("dep-authoritative"),
                tenant: TenantId::new("tenant-authoritative"),
                actor: ActorId::new("actor-authoritative"),
                command,
        ...
        let invalid = [
            ("model_selection", {
                ...
                value.model_selection =
                    Some(wrokbot_contracts::model_connections::RunModelSelection {
            ...
            ("run_id", {
                let mut value = begin_command();
                value.run_id = wrokbot_contracts::ids::RunId::new("");
                value
            }),
            ("bot_id", {
                let mut value = begin_command();
                value.bot_id = wrokbot_contracts::ids::BotId::new("");
                value
            }),
            ("message", {
                let mut value = begin_command();
            ...
            ("channel_id", {
                ...
                value.anchor = ThreadRunAnchor::Channel {
                    channel_id: wrokbot_contracts::ids::ChannelId::new(""),
                };
                value
            }),
        ];
    ...
    async fn cancel_injects_authoritative_scope_and_returns_only_the_durable_ack() {
        let expected = ThreadRunCancellation {
            thread_id: ThreadId::new("550e8400-e29b-81d4-a716-446655440000"),
            run_id: wrokbot_contracts::ids::RunId::new("run-1"),
            state: wrokbot_contracts::command::ThreadRunCancellationState::Requested,
        };
    ...
    async fn malformed_cancel_identities_never_reach_the_port() {
        let invalid = [
            ...
            ("run_id", {
                let mut command = cancel_command();
                command.run_id = wrokbot_contracts::ids::RunId::new("");
                command
            }),
            ("run_id", {
                let mut command = cancel_command();
                command.run_id = wrokbot_contracts::ids::RunId::new("bad\0run");
                command
            }),
        ];
        for (field, command) in invalid {
