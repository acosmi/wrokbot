//! `wrokbot-application` —— **唯一业务入口**：所有 use case 的收口点。
//!
...
//! - 编排 use case：调 `wrokbot-domain` 做纯判定，调 `wrokbot-infra` / `wrokbot-agent` /
//!   `wrokbot-computer` 的 port 执行 effect，划事务边界。
...
//! 明确**不**负责：
//!
//! - 认证本身、传输 framing、输入大小限制、错误到 HTTP/IPC 的映射 —— 那是 `wrokbot-server`
//!   与 `wrokbot-desktop` 这两个 transport 的活。
...
//!
//! 具体实现 [`WrokBotApplication`] 把上面四层接起来，是 transport 唯一需要构造的类型。
//!
//! # 端口在这里定义，不在 infra 定义
//!
//! 六边形架构的方向盘：`wrokbot-application` **定义** [`ChannelReader`]，`wrokbot-infra`
...
pub use app::WrokBotApplication;
