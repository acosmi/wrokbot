//! `wrokbot-domain` —— 纯领域层：领域状态、不变量、policy 类型、Agent reducer。
//!
...
//!   `policy-cel-corpus`：`owner: wrokbot-domain`，`target: … -> wrokbot_domain::policy::cel`）。
//!   Phase 0 时本段曾写作「CEL 求值器在 `wrokbot-infra`」，那是一句没有台账支撑的推断，
...
//! - use case 编排与事务边界 —— 在 `wrokbot-application`。
//! - SQL、连接池、HTTP、provider 协议细节 —— 在 `wrokbot-infra`。
