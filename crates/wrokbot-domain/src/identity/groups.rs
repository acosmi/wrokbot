...
//! materialized membership**。`wrokbot-application` 的 `ports::ChannelReader` 已经把这条写进
...
///
/// 它不在 `wrokbot_contracts::ids` 的十五个名字里（§5.3 的表钉死了那张清单），所以先落在
...
/// materialized membership（`wrokbot-application::ports::ChannelReader`）。本类型是那些行
/// 该长什么样的**计划**。
///
