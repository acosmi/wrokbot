...
//!
//! 领域层不碰事务（`wrokbot-domain` 的 crate 文档：SQL、连接池在 `wrokbot-infra`）。
...
/// 封闭角色全集。
///
/// 与 `wrokbot_contracts::auth::Role` 的变体一一对应，由
...
mod tests {
    ...
    fn seed_role_uses_the_normalized_admin_floor_and_defaults_to_user() {
        let floor = floor_of(&[" admin@wrokbot.test "]);
        assert_eq!(seed_role(&floor, &email("Admin@WrokBot.test")), Role::Admin);
        assert_eq!(seed_role(&floor, &email("member@wrokbot.test")), Role::User);
    }
