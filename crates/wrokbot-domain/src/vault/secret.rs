...
//!
//! # 为什么复用 `wrokbot_contracts::telemetry::Redacted` 而不是造第二个
//!
//! `Redacted` 已经把三件事做对了，而且是**刻意**不实现某些 trait 换来的：`Debug` / `Display`
//! 只输出 [`Redacted` 的固定占位][wrokbot_contracts::telemetry::REDACTED_PLACEHOLDER]；
...
use wrokbot_contracts::telemetry::{REDACTED_PLACEHOLDER, Redacted};
...
mod tests {
    ...
    ///
    /// 手法照抄 `wrokbot-contracts` 的 `auth.rs::SerializeProbe`：一个 inherent 方法（更优先）
