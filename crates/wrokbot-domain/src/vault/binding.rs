...
use wrokbot_contracts::ids::{ActorId, BotId, TenantId};
...
/// KEK / master key 的版本号。
///
/// 用 `u32` 而不是字符串：与 `wrokbot_contracts::ids` 里两个 generation 计数器同一条理由 ——
...
///
/// # 为什么它在 vault 里而不在 `wrokbot-contracts::ids`
///
/// D-2 复算时它的全部消费者都在 `wrokbot-domain::vault`，没有第一条真实跨层契约。W-7b/R59
