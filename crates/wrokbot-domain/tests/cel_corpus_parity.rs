...
//!
//! [`PolicyContext`] 刻意不实现 `Deserialize`（理由见 `wrokbot_domain::policy::context` 的模块
...
use wrokbot_domain::policy::CompiledRule;
use wrokbot_domain::policy::cel::ResultKind;
use wrokbot_domain::policy::context::{
    ActorRef, BotRef, ElementRef, FileRef, Intent, McpEffect, McpRef, PageRef, PolicyContext,
    ToolRef,
};
use wrokbot_domain::policy::preflight::{
...
///
/// 每一行都是本轮在本机实跑 `cargo test -p wrokbot-domain --test cel_corpus_parity` 得到的，
...
fn divergences_are_exactly_the_recorded_ledger() {
    ...
    for entry in entries {
        ...
        let candidate = match wrokbot_domain::policy::cel::compile(expression) {
...
fn disabling_the_regex_feature_keeps_method_form_matches_in_the_error_class() {
    ...
    let context = contexts
        ...
        .expect("context 必须存在");
    let compiled =
        wrokbot_domain::policy::cel::compile(entry["expression"].as_str().expect("expression"))
            .expect("这条表达式语法合法");
    ...
    // 不是因为 matches 整个不可用。
    let global_form = wrokbot_domain::policy::cel::compile("matches(element.name, \"sub.*\")")
        .expect("全局形式语法合法");
