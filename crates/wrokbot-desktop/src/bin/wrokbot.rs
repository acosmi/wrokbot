...
fn main() -> std::process::ExitCode {
    eprintln!("wrokbot_desktop_launcher_platform_unsupported");
    std::process::ExitCode::FAILURE
}

#[cfg(target_os = "macos")]
fn run() -> Result<(), String> {
    use wrokbot_desktop::desktop_release::{
        TrustedDesktopReleaseDigest, VerifiedDesktopRelease, validate_product_context,
    };
    use wrokbot_desktop::register_desktop_local_runtime;
    ...
    if args.mode == Mode::Check {
        println!("wrokbot_release_resources_verified");
        return Ok(());
    }
        ...
        match arg.to_str() {
                ...
                if parsed.mode == Mode::Launch && parsed.resource_root.is_none() =>
            {
                ...
                if args.next().is_some() {
                    return Err("wrokbot_arguments_invalid".to_owned());
                }
                return Ok(parsed);
            }
            ...
            Some("--resource-root") if parsed.resource_root.is_none() => {
                let path = args
                    ...
                    .ok_or_else(|| "wrokbot_arguments_invalid".to_owned())?;
                parsed.resource_root = Some(path);
            }
            _ => return Err("wrokbot_arguments_invalid".to_owned()),
        }
    }
    Ok(parsed)
}
...
fn bundled_resource_root() -> Result<std::path::PathBuf, String> {
    let executable = std::env::current_exe()
        .map_err(|_| "wrokbot_release_resource_root_unavailable".to_owned())?;
    ...
    let contents = macos
        ...
        .ok_or_else(|| "wrokbot_release_resource_root_unavailable".to_owned())?;
...
mod tests {
    ...
    fn generated_context_matches_reviewed_zero_window_product_config() {
        ...
        wrokbot_desktop::desktop_release::validate_product_context(&context).unwrap();
