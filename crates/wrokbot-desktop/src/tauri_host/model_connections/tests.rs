...
impl Fixture {
    ...
    fn with_application(
        ...
        std::fs::write(root.join("index.html"), "<!doctype html><html lang=\"en\"><head><script type=\"module\" src=\"/wrokbot-bootstrap.mjs\"></script></head><body></body></html>").unwrap();
...
async fn credential_framing_budget_preserves_sixteen_kib_key_limit_and_zeroizes_body() {
    ...
    input["apiKey"] =
        json!("x".repeat(wrokbot_contracts::credential_admin::MAX_CREDENTIAL_TOKEN_BYTES));
    ...
    input["apiKey"] =
        json!("x".repeat(wrokbot_contracts::credential_admin::MAX_CREDENTIAL_TOKEN_BYTES + 1));
    assert_error(
