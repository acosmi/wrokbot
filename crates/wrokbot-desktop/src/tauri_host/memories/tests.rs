...
use wrokbot_contracts::command::{ChannelSummary, SubscriptionRequest};
...
impl Fixture {
    fn new(memory: MemoryPort) -> Self {
        let application: Arc<dyn ApplicationService> =
            Arc::new(WrokBotApplication::new(EmptyChannels).with_memory(memory.clone()));
    ...
    fn with_application(memory: MemoryPort, application: Arc<dyn ApplicationService>) -> Self {
        ...
        std::fs::write(root.join("wrokbot-bootstrap.mjs"), "export {};").unwrap();
