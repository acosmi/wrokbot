...
pub const RELEASE_MANIFEST_FILE: &str = "wrokbot-release.json";
...
impl ReleaseManifest {
    fn parse(bytes: &[u8]) -> Result<Self, DesktopReleaseError> {
        ...
        if manifest.schema != "wrokbot-desktop-release"
        ...
        if tenant_inventory.keys().copied().collect::<BTreeSet<_>>()
            ...
            || !manifest
                ...
                .files
                .iter()
                .any(|file| file.path == "wrokbot-bootstrap.mjs")
        {
            return Err(DesktopReleaseError::Manifest);
        }
...
mod tests {
    ...
    const VALID_INDEX: &str = "<!doctype html><html lang=\"en\"><head><script type=\"module\" src=\"/wrokbot-bootstrap.mjs\"></script></head><body></body></html>";
    static NEXT: AtomicU64 = AtomicU64::new(0);
    struct TempRoot(PathBuf);
    impl TempRoot {
        fn new() -> Self {
            let root = std::env::temp_dir().canonicalize().unwrap().join(format!(
                "wrokbot-release-unit-{}-{}",
                std::process::id(),
    ...
    fn tenant_texts() -> [&'static str; 5] {
        [
            ...
            include_str!("../../../examples/wrokbot/agents.yaml"),
            include_str!("../../../examples/wrokbot/channels.yaml"),
            include_str!("../../../examples/wrokbot/model.yaml"),
            include_str!("../../../examples/wrokbot/knowledge.yaml"),
        ]
    }
    ...
    fn fixture() -> Value {
        json!({
            "schema":"wrokbot-desktop-release", "schema_version":1,
            ...
            "ui":{"root":"ui","files":[file("index.html",VALID_INDEX.as_bytes()),file("wrokbot-bootstrap.mjs",b"export {};")]},
    ...
    fn write_fixture(root: &Path) -> TrustedDesktopReleaseDigest {
        ...
        fs::write(root.join("ui/wrokbot-bootstrap.mjs"), b"export {};").unwrap();
    ...
    fn product_acl_is_exactly_three_local_commands_and_no_precreated_windows() {
        ...
        assert_eq!(
            ...
            json!([
                "allow-wrokbot-structured-events-open",
                "allow-wrokbot-structured-events-close",
                "allow-wrokbot-window-chrome"
            ])
        );
    }
}
