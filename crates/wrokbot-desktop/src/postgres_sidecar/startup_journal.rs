...
/// advance to `exit_confirmed` only after [`wrokbot_macos_process::evidence_process_is_absent`].
        ...
        Ok(metadata) => {
            ...
            match (record.phase, record.child_observation.as_deref()) {
                ...
                (
                    ...
                    wrokbot_macos_process::evidence_process_is_absent(&evidence).map_err(
                        |error| match error {
                            wrokbot_macos_process::ProcessObservationError::ObservationChanged => {
