//! `wrokbot-desktop` —— Desktop transport：Tauri、window ACL、in-process、sidecar/update。
//!
...
//! fencing。Batch73再把closed wire上收到`wrokbot-contracts`，注册exact open/close command：open
...
//!    到同label的新窗口；
//! 5. unary command仍只把typed [`AppCommand`](wrokbot_contracts::command::AppCommand)交给
...
//! # 认证：G1 刻意没有
//!
//! [`WindowIdentity`] 只能由一个既有的 [`AuthContext`](wrokbot_contracts::auth::AuthContext)
...
// 与 `wrokbot-application` 同口径，用 deny 而不是 warn。
#![deny(missing_docs)]
