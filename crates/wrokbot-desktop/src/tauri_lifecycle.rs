...
/// assertion at the host call site, matching [`wrokbot_contracts::auth::AuthContextBuilder`].
...
mod tests {
    ...
    fn navigation_is_exactly_the_platform_internal_custom_origin() {
        for allowed in [
            "wrokbot://localhost/",
            "wrokbot://localhost/channel/id?x=1#message",
        ] {
        ...
        for rejected in [
            "https://localhost/",
            "wrokbot://remote.invalid/",
            "wrokbot://user@localhost/",
            "wrokbot://localhost:444/",
            "other://localhost/",
            "file:///tmp/secret",
        ] {
            assert!(
                !allows_local_navigation("wrokbot", &Url::parse(rejected).unwrap()),
        ...
        assert!(allows_local_navigation_for(
            true,
            "wrokbot",
            &Url::parse("http://wrokbot.localhost/channel/id").unwrap(),
        ));
        for rejected in [
            "https://wrokbot.localhost/",
            "http://wrokbot.localhost:81/",
            "http://user@wrokbot.localhost/",
            "http://other.localhost/",
        ] {
            assert!(
                !allows_local_navigation_for(true, "wrokbot", &Url::parse(rejected).unwrap(),),
        ...
        assert!(!allows_local_navigation_for(
            false,
            "wrokbot",
            &Url::parse("http://wrokbot.localhost/").unwrap(),
        ));
    }
    ...
    fn mock_runtime_creates_bound_local_window_and_destroyed_event_unbinds_it() {
        ...
        assert_eq!(window.url().unwrap().as_str(), "wrokbot://localhost/");
