...
use wrokbot_application::tenant::package::{LoadedTenantPackage, TenantPackageError};
...
use wrokbot_infra::db::desktop_local::DesktopLocalDatabaseError;
...
fn startup_failure_diagnostic(error: &DesktopLocalRuntimeError) -> String {
    let mut diagnostic = format!("wrokbot_desktop_startup_failed code={}", error.code());
...
pub(crate) struct PreparedDesktopLocalRuntime {
    ...
    #[cfg(test)]
    application: Arc<dyn wrokbot_application::ApplicationService>,
    #[cfg(test)]
    pool: wrokbot_infra::db::pool::DatabasePool,
}
...
impl PreparedDesktopLocalRuntime {
    ...
    pub(crate) fn application(&self) -> &Arc<dyn wrokbot_application::ApplicationService> {
        &self.application
    }
    ...
    pub(crate) fn pool(&self) -> &wrokbot_infra::db::pool::DatabasePool {
        &self.pool
    }
...
fn observe_exit(stage: &'static str, outcome: &'static str) {
    ...
    eprintln!("wrokbot_desktop_exit stage={stage} outcome={outcome}");
}
...
pub(crate) async fn prepare_desktop_local_runtime(
    ...
    let assembly = match assemble_postgres_application(PostgresApplicationAssemblyInput {
        ...
        screen_sessions: Arc::new(wrokbot_application::NoScreenSessionAdministration),
...
mod tests {
    ...
    use wrokbot_application::{AppEventStream, ApplicationService};
    ...
    fn lifecycle() -> (
        ...
        (
            Arc::new(DesktopWindowLifecycle::new("wrokbot", Arc::clone(&protocol)).unwrap()),
            protocol,
            root,
        )
    }
    ...
    fn r279_data_plane_errors_preserve_cleanup_and_missing_database_classes() {
        for error in [
            ...
            assert_eq!(
                ...
                "wrokbot_desktop_startup_failed code=desktop_local_runtime_failure_cleanup_failed\n"
            );
        }
        ...
        assert_eq!(
            ...
            "wrokbot_desktop_startup_failed code=desktop_local_runtime_recovery_required\n"
        );
        ...
        assert_eq!(
            ...
            "wrokbot_desktop_startup_failed code=desktop_local_runtime_data_plane_failed\n"
        );
    }
    ...
    fn startup_diagnostic_is_closed_bounded_and_preserves_only_os_category() {
        ...
        assert_eq!(
            with_os_source,
            "wrokbot_desktop_startup_failed code=desktop_local_runtime_os_secret_store_reconciliation_required os_code=desktop_os_secret_store_auth_failed\n"
        );
        ...
        assert_eq!(
            diagnostic,
            "wrokbot_desktop_startup_failed code=desktop_local_runtime_sidecar_failed\n"
        );
    }
    ...
    fn async_prepare_result_uses_one_non_masking_diagnostic_write() {
        ...
        assert_eq!(
            captured,
            b"wrokbot_desktop_startup_failed code=desktop_local_runtime_recovery_required\n"
        );
