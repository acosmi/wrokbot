...
use wrokbot_application::{UiPreferenceAdministration, UiPreferenceAdministrationError};
use wrokbot_contracts::auth::AuthContext;
use wrokbot_contracts::ui::{UiLocale, UiPreferences, UiTheme, UpdateUiPreferences};

const FILE_HEADER: &str = "wrokbot-ui-preferences-v1";
...
mod tests {
    ...
    fn parser_is_closed_and_round_trips_independent_fields() {
        ...
        assert!(parse("wrokbot-ui-preferences-v1\ntheme=sepia\nlocale=en\n").is_err());
        assert!(parse("wrokbot-ui-preferences-v1\ntheme=dark\nlocale=en\nextra=x\n").is_err());
    }
    ...
    async fn local_store_merges_and_replaces_without_leaving_temp_files() {
        let root = std::env::temp_dir().join(format!(
            "wrokbot-desktop-ui-preferences-{}-{}",
            std::process::id(),
