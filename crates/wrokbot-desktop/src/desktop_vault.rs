...
use wrokbot_infra::db::desktop_local::DesktopLocalDatabaseOrigin;
use wrokbot_infra::db::desktop_vault_canary::{
...
impl PreparedDesktopLocalDataPlane {
    ...
    pub(crate) async fn verify_application_key_material<S: OsSecretStore + ?Sized>(
        ...
        package: &wrokbot_application::tenant::package::LoadedTenantPackage,
...
mod tests {
    ...
    fn macos_private_keychain_persists_master_and_cross_restart_vault_open() {
        ...
        let root = std::env::temp_dir().join(format!(
            "wrokbot-desktop-vault-keychain-{}-{}",
            std::process::id(),
    ...
    fn windows_credential_manager_persists_master_and_cross_restart_vault_open() {
        ...
        let _ = wrokbot_windows_sandbox::delete_generic_credential(&target);
        ...
        assert!(wrokbot_windows_sandbox::delete_generic_credential(&target).unwrap());
    }
