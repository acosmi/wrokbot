...
/// `wrokbot-application` 在同一处做了同样的选择。
...
mod tests {
    ...
    use wrokbot_application::{ChannelReader, WrokBotApplication, PortError};
    use wrokbot_contracts::command::{ChannelSummary, HealthReport};
    use wrokbot_contracts::ids::{ActorId, BotId, ChannelId};
    ...
    // -----------------------------------------------------------------------
    // 一个真实业务实现（不是 mock）：真的 WrokBotApplication + 内存 port
    // -----------------------------------------------------------------------
    ...
    impl ChannelReader for FakeChannels {
        async fn list_visible_channels(
            ...
            _actor: &ActorId,
            _limit: u32,
            _cursor: Option<wrokbot_application::ChannelCursor>,
    ...
    fn application() -> Arc<dyn ApplicationService> {
        Arc::new(
            WrokBotApplication::new(FakeChannels::with_one_row())
    ...
    /// （跨 transport 的另一半由 `wrokbot-testkit` 做）。
    #[tokio::test]
