...
use wrokbot_contracts::budget::RunCostBudgetPreference;
...
use wrokbot_contracts::error::{AppError, SensitiveWriteReason};
...
use wrokbot_contracts::people::CurrentUserResponse;
use wrokbot_contracts::remote_interrupt::{RemoteInterruptAnswer, RemoteInterruptResolved};
use wrokbot_contracts::sandboxed::SaveSandboxedComponentRequest;
use wrokbot_contracts::tool::ToolApprovalDecision;
use wrokbot_contracts::ui::{UiLocale, UiPreferences, UiTheme, UpdateUiPreferences};
...
impl DesktopTauriProtocol {
    ...
    pub async fn handle(&self, label: &str, mut request: Request<Vec<u8>>) -> Response<Vec<u8>> {
        ...
        #[cfg(all(feature = "desktop-local-runtime", target_os = "macos"))]
        if path == wrokbot_contracts::desktop::local_confirmation::LOCAL_CONFIRMATION_PATH {
            return self
    ...
    async fn credential_administration(
        ...
        if !authority
            .auth
            .has_role(wrokbot_contracts::auth::Role::Admin)
        {
            request.body_mut().fill(0);
            return error_response(AppError::ForbiddenRole {
                required: wrokbot_contracts::auth::Role::Admin,
            });
        }
        let path = request.uri().path().to_owned();
...
#[tauri::command]
async fn wrokbot_structured_events_open(
    webview: Webview,
...
#[tauri::command]
fn wrokbot_structured_events_close(
    webview: Webview,
    protocol: State<'_, Arc<DesktopTauriProtocolSlot>>,
...
pub(crate) fn register_tauri_protocol_slot(
    ...
    let builder = builder
        ...
        .invoke_handler(tauri::generate_handler![
            wrokbot_structured_events_open,
            wrokbot_structured_events_close,
            window_chrome::wrokbot_window_chrome
        ]);
...
fn validate_index(index: &str) -> Result<(), TauriHostError> {
    ...
    if scripts != 1
        ...
        || !(lower.contains(" src=\"/wrokbot-bootstrap.mjs\"")
            || lower.contains(" src=\"./wrokbot-bootstrap.mjs\""))
    {
        return Err(TauriHostError::InvalidBundle);
    }
...
mod tests {
    ...
    use wrokbot_application::cursor::ChannelCursor;
    use wrokbot_application::{
        ...
        ComponentRuntimeScope, McpConnectionAdministration, McpConnectionError, WrokBotApplication,
    ...
    use wrokbot_contracts::auth::{AuthGeneration, Role};
    ...
    use wrokbot_contracts::ids::{ActorId, BotId, DeploymentId, TenantId};
    use wrokbot_contracts::mcp::{
    ...
    use wrokbot_contracts::tool::{PendingToolApprovals, ToolApprovalResolved};
    ...
    impl futures_core::Stream for NeverAppStream {
        type Item = wrokbot_contracts::command::AppEvent;
    ...
    #[async_trait]
    impl wrokbot_application::ApplicationService for BlockingSubscriptionService {
        async fn execute(
            &self,
            _auth: AuthContext,
            _command: AppCommand,
        ) -> Result<AppReply, AppError> {
            Ok(AppReply::Health(wrokbot_contracts::command::HealthReport {
                ok: true,
            }))
        }
        ...
        ) -> Result<wrokbot_application::AppEventStream, AppError> {
    ...
    impl ComponentAdministration for FakeComponents {
        ...
        async fn update_component_governance(
            ...
            let mut component = ComponentRecord {
                ...
                kind: wrokbot_contracts::components::CompiledComponentKind::Card,
    ...
    fn protocol_root() -> PathBuf {
        ...
        fs::write(
            ...
            "<!doctype html><html lang=\"en\"><head><script type=\"module\" src=\"/wrokbot-bootstrap.mjs\"></script></head><body></body></html>",
        )
        .unwrap();
        fs::write(root.join("wrokbot-bootstrap.mjs"), "export {};").unwrap();
        root
    }
    ...
    fn verified_assets_keep_checked_bytes_after_disk_replacement_and_deletion() {
        ...
        let checked_js = fs::read(root.join("wrokbot-bootstrap.mjs")).unwrap();
        let mut files = BTreeMap::from([
            ("index.html".to_owned(), index.clone()),
            ("wrokbot-bootstrap.mjs".to_owned(), checked_js.clone()),
        ...
        fs::write(root.join("wrokbot-bootstrap.mjs"), "unverified replacement").unwrap();
        fs::write(root.join("injected.js"), "unlisted").unwrap();
        assert_eq!(checked.asset("/wrokbot-bootstrap.mjs").body(), &checked_js);
        assert_eq!(
    ...
    fn verified_image_mime_is_closed_and_non_wasm_budgets_stay_small() {
        ...
        let mut files = BTreeMap::from([
            ...
            ("wrokbot-bootstrap.mjs".to_owned(), b"export {};".to_vec()),
        ]);
        for (path, _) in [
        ...
        for path in [
            ...
            "/index.html",
            "/../wrokbot-bootstrap.mjs",
            "/%2e%2e/wrokbot-bootstrap.mjs",
        ] {
    ...
    fn protocol_with_mcp(
        ...
        protocol_with_admin_ports(
            mcp,
            Arc::new(wrokbot_application::credential_admin::NoCredentialAdministration),
        )
    }

    fn protocol_with_admin_ports(
        mcp: Arc<dyn McpConnectionAdministration>,
        credentials: Arc<dyn wrokbot_application::credential_admin::CredentialAdministration>,
    ) -> (Arc<DesktopTauriProtocol>, PathBuf) {
        ...
        let application = Arc::new(
            WrokBotApplication::new(channels.clone())
    ...
    fn registration_installs_the_closed_wry_protocol_and_command_handler() {
        ...
        assert!(register_tauri_protocol(builder, "wrokbot", protocol).is_ok());
        std::fs::remove_dir_all(root).unwrap();
    }
    ...
    fn scheme_asset_and_percent_decoders_are_closed() {
        assert_eq!(
            ...
            [
                "wrokbot_structured_events_open",
                "wrokbot_structured_events_close",
                "wrokbot_window_chrome"
            ]
        );
        assert!(valid_scheme("app-ui"));
        for invalid in ["", "WrokBot", "http", "tauri", "has space", "a/b"] {
    ...
    #[async_trait]
    impl wrokbot_application::credential_admin::CredentialAdministration for CredentialProbe {
        async fn list(
            &self,
            auth: &AuthContext,
            _: &CredentialPageRequest,
        ) -> Result<
            wrokbot_contracts::credential_admin::CredentialPage,
            wrokbot_application::credential_admin::CredentialAdministrationError,
        > {
            self.0.lock().unwrap().push((auth.actor().clone(), "list"));
            Ok(wrokbot_contracts::credential_admin::CredentialPage {
                credentials: Vec::new(),
                next_cursor: None,
        ...
        async fn create(
            ...
            wrokbot_contracts::credential_admin::CredentialWritten,
            wrokbot_application::credential_admin::CredentialAdministrationError,
        > {
            self.0
                .lock()
        ...
        async fn revoke(
            ...
            wrokbot_contracts::credential_admin::CredentialRevoked,
            wrokbot_application::credential_admin::CredentialAdministrationError,
        > {
            self.0
                .lock()
            ...
            Ok(wrokbot_contracts::credential_admin::CredentialRevoked {
                id: id.to_owned(),
                revoked_at: time::OffsetDateTime::UNIX_EPOCH,
                external_revocation:
                    wrokbot_contracts::credential_admin::CredentialExternalRevocation::Pending,
            })
        }
    }
