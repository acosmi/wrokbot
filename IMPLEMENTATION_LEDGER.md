...
| V6-PR-054 | "跳到主内容" skip-link 键盘焦点修复 | Copilot完成，待主控验收 | [#70](https://github.com/acosmi/wrokbot/pull/70) | 见 PR #70 提交历史 |
...
054（Copilot 临时实施执行方，非主控亲验）：为 `crates/wrokbot-ui/src/shell/layout.rs` 的 skip-link 增加 `on:click` 显式 `.focus()` 调用，修复原生 hash 跳转不可靠移动键盘焦点到 `tabindex="-1"` 主区域的既有缺口，镜像本 crate 已有的 `restore_focus` 惯用法。`cargo check`（native + wasm32-unknown-unknown 两个目标）、`cargo clippy -D warnings`（native 目标）、`cargo fmt --check`、`cargo test`（216 通过）均通过。wasm32 目标下 `-D warnings` clippy 本身在改动前即因 `http_request.rs`/`features/channels/new.rs` 两处既有问题失败，已用 `git stash` A/B 对比确认与本改动无关，另开 issue #69 归档、未在本 PR 内顺手修复。本 crate 无 `wasm-bindgen-test`/无头浏览器 DOM 测试基座，真实焦点跳转的键盘/屏幕阅读器行为未经自动化验证，留待人工/真机验收确认。
