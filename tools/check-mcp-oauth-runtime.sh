...
files=(
  crates/wrokbot-infra/src/mcp_oauth.rs
  crates/wrokbot-infra/src/mcp_connections.rs
  crates/wrokbot-infra/src/mcp_credentials.rs
)
...
grep -qF 'SafeHttpRequest::mcp(' crates/wrokbot-infra/src/mcp_oauth.rs \
  || fail '401 protected-resource probe no longer uses bounded SafeDialer MCP request'
grep -qF 'bearer_parameter(challenge, "resource_metadata")' crates/wrokbot-infra/src/mcp_oauth.rs \
  || fail 'WWW-Authenticate resource_metadata priority disappeared'
grep -qF 'serializer.append_pair("resource", resource);' crates/wrokbot-infra/src/mcp_oauth.rs \
  || fail 'authorization/code token resource binding disappeared'
grep -qF 'serializer.append_pair("resource", discovery.resource());' crates/wrokbot-infra/src/mcp_oauth.rs \
  || fail 'refresh token resource binding disappeared'
grep -qF 'code_challenge_method", "S256"' crates/wrokbot-infra/src/mcp_oauth.rs \
  || fail 'PKCE S256 authorization binding disappeared'
grep -qF 'metadata.issuer != client.issuer' crates/wrokbot-infra/src/mcp_oauth.rs \
...
[[ $(grep -c '\.with_egress_allowlist' crates/wrokbot-infra/src/mcp_connections.rs) -eq 4 ]] \
...
grep -qF 'coalesce(s.egress_allow_cidrs,ARRAY[]::text[]) AS egress_allow_cidrs' \
  crates/wrokbot-infra/src/store/plugin_user_credential.rs \
...
grep -qF 'FOR SHARE OF s,d' crates/wrokbot-infra/src/store/plugin_user_credential.rs \
  || fail 'post-token refresh rotation no longer locks current server/client authority'
grep -qF 'if request.transport() != "mcp"' crates/wrokbot-infra/src/mcp_oauth.rs \
  || fail 'generic MCP refresh accepted another vendor transport'
grep -qF '|| request.transport() != "google_drive_rest"' \
  crates/wrokbot-infra/src/google_drive_oauth.rs \
...
grep -qF 'openbot-mcp-oauth-state-v1' crates/wrokbot-infra/src/mcp_connections.rs \
  || fail 'OAuth state HMAC purpose separation disappeared'
grep -qF 'openbot-mcp-oauth-attempt-aead-v1' crates/wrokbot-infra/src/mcp_connections.rs \
  || fail 'OAuth attempt AEAD purpose separation disappeared'
grep -qF 'DELETE FROM public.verifications WHERE identifier=$1' crates/wrokbot-infra/src/mcp_connections.rs \
  || fail 'callback no longer burns state before validation/network'
grep -qF "FOR UPDATE SKIP LOCKED" crates/wrokbot-infra/src/mcp_connections.rs \
  || fail 'pending vendor revocation is no longer multi-replica claimed'
grep -qF "'revocation_status','pending'" crates/wrokbot-infra/src/mcp_connections.rs \
  || fail 'local-first disconnect tombstone disappeared'
grep -qF 'const ATTEMPT_VERSION: u8 = 3;' crates/wrokbot-infra/src/mcp_connections.rs \
...
grep -qF 'validate_stored_client' crates/wrokbot-infra/src/mcp_oauth.rs \
  || fail 'admin removal no longer validates retained OAuth client material without network'
grep -qF 'struct RemovedServerRevocationContext' crates/wrokbot-infra/src/mcp_connections.rs \
  || fail 'versioned admin-removal revocation context disappeared'
grep -qF 'removed_server_client_material' crates/wrokbot-infra/src/mcp_connections.rs \
...
if grep -qF 'let material = if removed_server' crates/wrokbot-infra/src/mcp_connections.rs; then
  fail 'removed-server tombstones can fall back to a re-added same-id server'
fi
grep -qF "'revocation_status','operator_required'" crates/wrokbot-infra/src/mcp_connections.rs \
...
grep -qF "split_part(g.ref,'/',1)=\$1" crates/wrokbot-infra/src/mcp_connections.rs \
...
grep -qF 'ADD COLUMN credential_generation bigint' crates/wrokbot-infra/sql/native_0018.sql \
  || fail 'credential generation migration disappeared'
grep -qF 'g.credential_generation=coalesce(s.credential_generation,0)' crates/wrokbot-infra/src/mcp_catalog.rs \
  || fail 'grant visibility no longer binds deployment credential generation'
grep -qF 'outcome == Err(McpClientError::AuthRequired)' crates/wrokbot-infra/src/agent_tools.rs \
  || fail 'controlled OAuth 401 refresh/retry branch disappeared'

authorization_slice=$(sed -n '/pub async fn authorization_plan/,/pub async fn exchange_authorization_code/p' crates/wrokbot-infra/src/mcp_oauth.rs)
...
grep -qF '.field("client_secret", &"[redacted]")' crates/wrokbot-contracts/src/mcp.rs \
